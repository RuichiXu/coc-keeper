...
| 37 | `lib/core/scenario/settlements.js` `extractSettlements`/`settlementMatches`：从剧本原文正则提取 `SC X/Y`、`全员HP-NdM`、`承受X/Y的伤害` 为结算点；SC 走 `coc_sanity_check`（eventId=scenario:<id> 幂等），HP 走 `coc_pc` 扣血；SC 行紧邻“体质检定+承受伤害”时生成带 `damage` 的 linkedGate，`.ra` 结算时按成败扣 1/1d4；`chat-bridge.js` 在叙述落地后确定性结算；`context-builder` 之外的兜底 | Codex 复测《对流》：泄压阀 HP-1d6、直面沸核 SC 1d4/1d10、外出 SC 1/1d6 与体质 1/1d4 全部漏结算；旧检定点提取只认“检定”锚点，`SC` 与 `HP-` 标记被整类漏掉 | 结构分析/导入管线直接输出 `settlements`（含触发场景/条件/表达式），不再从原文正则+语境词推断 | 本轮为确定性兜底；语境词（沸核+观察、泄压阀+蒸汽扑面）仍属启发式 |
| 38 | `lib/shared/chat/chat-bridge.js` `scenarioHasMoyuanSpell`：剧本全文同时含“启墨渊”与“归字主”才允许注入《墨渊》特化文本（十二字咒文展示、最终仪式轮提示、日记核心句防护）；`ending.js` `endingSentenceFor` 对非墨渊剧本只输出通用结局句 | Codex 复测《对流》第 25 轮：`finalBranchChosen` 为 true 时共享代码无条件注入“启墨渊、引魂夜、临神名、归字主”，但《对流》剧本没有该咒文；早期《墨渊》专项兜底被做成共享路径后一直缺少剧本适用性闸门 | 咒文内容由剧本结构数据（关键点 ownText / deepParse endingKeywords）驱动，共享代码不再硬编码任何剧本专属文本 | 本轮为剧本适用性闸门，仍属关键词判断启发式 |
