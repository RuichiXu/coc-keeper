...
| 17 | `lib/shared/chat/chat-bridge.js` 检定点消费 / 失败消费 / `expireSceneGates` | `.ra` 前先匹配检定点并绑定到 selectedGate，成功按 checkpointId 记录 passedCheckpointIds；门禁短路过滤同时看 resolvedChecks 与 passedCheckpointIds；失败记 gate-failed trace；每轮开始按门禁 scene 与当前场景互不包含清理过期门禁 | checkpointId 仍依赖 `findCheckpointMatch` 的启发式匹配；场景包含关系可能误保留（如“三层书房”与“三层书房门外”） | 门禁创建时由场景事实/检定点直接写入 checkpointId；目标失效由 Scene/Plot 事件精确清理 |
