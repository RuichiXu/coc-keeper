...
2. **分块局部条件语义审校** ✅ 已完成：新增 `buildChunkReviewPrompt / buildChunkRevisionPrompt`，loop 第 1 轮对全部有内容的分块跑语义审校（廉价模型、并发、低 reasoning），第 2/3 轮只复审并修复有 high/medium 问题的分块（其余分块保持第 1 轮定稿），分块审校计入 pass 门禁（chunkHigh=0 且 chunkMedium≤2）。
