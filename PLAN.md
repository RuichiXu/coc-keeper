...
7. **运行时冒烟与 Codex 复测修复** ✅ 进行中（`c8da08b`…工作区）：
   ...
   - 第八批（工作区）：r6 修复——judge 消息改块式 content + `toOpenAiMessages` 兼容字符串；judge 观测统一工具执行后落盘（`flat.judgeStats` + trace）；回退再收紧（set-4 删“到达”、HP futureMarkers）。
   - 下一步候选：提交第八批后复测；结局前置校验与收口、权限球/教授位置等事实保真、`coc_branch` 工具错误收口。
