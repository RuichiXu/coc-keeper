...
| 可选长剧本 | 星孩v1.0（117 页） | h2/m0，未达旧 B 级（E 阶段已复跑，见下） |
...
| 剧本 | 字符 | sections | kp/br/结局/边 | 冷启动耗时 | 热缓存耗时 | preflight | rule | review | chunk | 未连线 |
|---|---|---|---|---|---|---|---|---|---|---|
| 对流（短） | 16,298 | 35 | 22/6/5/41 | 493s | 0s | 0h/0m | 0h/0m | 0h/2m | 0h/0m | 0 |
| 两面不是人（中 A） | 63,873 | 65 | 29/3/4/40 | 1175s | 174s | 0h/0m | 0h/0m | 3h/0m | 0h/2m | 0 |
| 盲愚之眼（中 B） | 63,629 | 94 | 42/24/2/52 | 2178s | 0s | 0h/0m | 0h/0m | 0h/0m | 0h/1m | 0 |
| 星孩（长） | 166,486 | 221 | 98/77/5/139 | 3982s | 146s | 0h/0m | 0h/0m | 0h/0m | 0h/1m | 0 |

**两档验收门槛（本次验证后确认）**：

- **硬门禁（必须全绿）**：`preflight h0/m0`、`rule h0/m0`、未连线场景点 = 0。→ 4/4 全部通过 ✅。
- **语义门禁（B 级）**：`review ≤ h0/m2`、`chunk ≤ h0/m2`。→ 对流 ✅、盲愚 ✅、星孩 ✅；两面 ❌（review 3h：复杂多结局最终接线生成空前置，审校抓对）。

**本轮验证推动的修复**（已提交 `e7ff165`）：

- 连通性修复：主线含 `scene_event`、非首主场景补入边、排序与 preflight 开场判定对齐；preflight BFS 始终从开场主场景出发。
- `enrichStoryPrerequisites`：场景节点不再引用同场景检定点作前置（消除“进入房间需先通过房内检定”死锁）。
- 最终接线修复：`label/name→title`、`condition→requires` 别名；最终分支/结局标题回填；branchCondition 补 scene 门控；选项 leadsTo 指向结局标题；结局穷尽约束；接线超时默认 180s。
- 审校稳定性：澄清 `br→end` 的 `keyPointIds` 是合法推进条件；场景清单始终包含最终分支 scene（星孩 review high 归零）。
- 配置：`~/.dsh/coc/config.json` 的 `deepParse.finalTemperature` 1→0。

---

## 当前待办（下一步）

> 2026-09-05 合并后更新：前端重构与后端审校跳过均已合入 main（`228e32f`）。本清单反映当前实际待办。

0. **内置场次与随包数据库** ✅ 已完成：`presets/games/verify-xinghai.json`、`presets/games/verify-liangmian.json` 入库，插件启动时 `seedBundledGames` 缺失才复制。
1. **面板坞真实鼠标可用** ✅ 已完成：`lib/client.js` window capture 提前接管 dock 点击；ui-check 恢复真实 `click()`，28/28。
2. **盲愚之眼拓扑修复（main 直接改）** ✅ 已完成：唯一 chapter hub 补 `topHub->chapterHub` + `开场主场景->topHub`，preflight 0h/0m。
3. **重新导入《盲愚之眼》《对流》** ✅ 已完成：对流 quality.pass=true；盲愚硬门禁全绿，语义审校仍有 1 high（“依赖咒语缺陷（守秘人可选）”作为玩家选项），待最终接线/结局选项确定性化。
3b. **《两面不是人》结构颗粒度修复** ✅ 已完成（确定性侧）：标题粘连（`- ◆`/裸标题/页码独立成行）、`胀妇之死` 裸标题拆成独立 scene_event、`【技能】` 括号检定提取并排除战斗对抗类；单测 58/58，真实文本验证通过。**待办：带缓存重新导入《两面不是人》落盘新结构。**
3c. **盲愚“守秘人可选”结局选项清理**：语义审校 high 1 仍未解决，方向：最终接线/结局选项确定性清理，把“守秘人可选”类条目从玩家选项改为 KP 标志。
4. **E 后续（历史项保留）**：
...
7. **运行时冒烟与 Codex 复测修复** ✅ 进行中（`c8da08b`…`0c64d08`，第六批未提交）：
   ...
   - 第三批（`f8bbb0b`）：自动结算句子级匹配 + sceneTokens + 扣损账本（`damage-ledger.js`），修复 r3 误触发与双扣。
   - 第四批（`6f0b79e`）：地点白名单/路线邻接（`route-index.js`）+ 玩家转移意图场景同步 + 未登记地点守卫 + 场景/路线约束注入。
   - 第五批（`0c64d08`）：r4 修复——外出结算改为实际移动动作、守卫只取地点后缀降误报、条件句/问路识别、删除旧版楼层硬编码兜底、`.ra语言（母语）` 解析与母语→EDU、技能成长必须有成功记录。
   - 第六批（工作区）：r5 修复——场景落地 `stripMenuLines` + 合法路线意图无条件采纳；外出结算改到达型（`arrival`）并删除“出去/前往”触发词；HP 结算 `strongContext` + 短场景词；`coc_pc` 空更新 no-op。
   - 下一步候选：r5 修复提交后复测；结局前置校验与收口、权限球/教授位置等事实保真、`coc_branch` 工具错误收口。
