...
0. **内置场次与随包数据库**：把 `verify-xinghai`、`verify-liangmian` 两个 DB 作为内置场次随仓库分发，并在插件启动时自动播种到 `dataDir/games/`（缺失才复制，不覆盖玩家已有进度）。预计新增 `presets/games/*.json` 与播种逻辑。
1. **面板坞真实鼠标可用**：Playwright 鼠标点击 dock fab 未触发 click 监听，`ui-check` 暂用 `dispatchEvent`；需在 `lib/client.js` 定位原因并修复为真实鼠标可点。
2. **盲愚之眼拓扑修复（main 直接改）**：单章节 `chapter`（`w9-s0 绿人与锡尔伯里山`）的虚拟枢纽只有 `return->hub` 入边，BFS 不可达，preflight `0h/5m`；修复方向：给唯一 chapter hub 补 `topHub->chapterHub`，并保证 `topHub` 从开场主场景可达。
3. **重新导入《盲愚之眼》《对流》**：本地旧版 DB 已删除；修复后重新导入，硬门禁必须 pass，且额外人工/脚本核查非主线边（hub 辐条、支线 hook、跨场景推进边）没有大量遗漏。
4. **E 后续（历史项保留）**：
