...
import { ASSET_KINDS, KNOWLEDGE_LAYERS, buildKnowledgeView, compileByPattern } from "../../core/index.js";
    ...
    if (req.method === "POST") {
      ...
      if (path === "/coc-api/game-setup") {
        ...
        if (scenarioId.length > 0) {
          ...
          try {
            const model = compileByPattern(asset.text ?? "", asset.name);
            ...
            for (const npc of (model.npcs ?? [])) {
              entities.push({ id: npc.id ?? `ent-${entities.length + 1}`, name: npc.name ?? "未命名NPC", type: "npc", desc: npc.description ?? "", scene: (npc.scenes ?? [])[0] ?? "", scenarioId: asset.name });
            }
            for (const loc of (model.locations ?? [])) {
              entities.push({ id: loc.id ?? `ent-${entities.length + 1}`, name: loc.name ?? "未命名地点", type: "location", desc: loc.description ?? "", scene: "", scenarioId: asset.name });
            }
            for (const item of (model.items ?? [])) {
              entities.push({ id: item.id ?? `ent-${entities.length + 1}`, name: item.name ?? "未命名物品", type: "item", desc: item.description ?? "", scene: (item.locationIds ?? [])[0] ?? "", scenarioId: asset.name });
            }
        ...
        if (flat.scenario !== null) {
          try {
            const openingPrompt = [
              ...
              "剧本摘要：" + (flat.scenario.summary ?? "").slice(0, 400),
              "调查员：" + names,
              "要求：营造氛围、交代初始场景，以第二人称面对玩家；不要替玩家做决定，不要推进剧情；必须完整结束、以句号结尾，不要在中途截断。",
            ].join("\n");
