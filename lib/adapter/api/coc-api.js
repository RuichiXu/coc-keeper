...
import { ASSET_KINDS, KNOWLEDGE_LAYERS, buildKnowledgeView, compileByPattern, sanitizeMetaText } from "../../core/index.js";
        ...
        if (scenarioId.length > 0) {
          ...
          try {
            ...
            for (const npc of (model.npcs ?? [])) {
              entities.push({ id: npc.id ?? `ent-${entities.length + 1}`, name: npc.name ?? "未命名NPC", type: "npc", desc: sanitizeMetaText(npc.description ?? ""), scene: (npc.scenes ?? [])[0] ?? "", scenarioId: asset.name, revealed: false });
            }
            for (const loc of (model.locations ?? [])) {
              entities.push({ id: loc.id ?? `ent-${entities.length + 1}`, name: loc.name ?? "未命名地点", type: "location", desc: sanitizeMetaText(loc.description ?? ""), scene: "", scenarioId: asset.name, revealed: false });
            }
            for (const item of (model.items ?? [])) {
              entities.push({ id: item.id ?? `ent-${entities.length + 1}`, name: item.name ?? "未命名物品", type: "item", desc: sanitizeMetaText(item.description ?? ""), scene: (item.locationIds ?? [])[0] ?? "", scenarioId: asset.name, revealed: false });
            }
