      ...
      if (path === "/coc-api/game-setup") {
        ...
        if (scenarioId.length > 0) {
          ...
          try {
            ...
            flat.keyPoints = model.keyPoints.map((kp) => ({ ...kp, scenarioId: asset.name }));
            flat.branches = model.branches.map((b) => ({ ...b, scenarioId: asset.name }));
            flat.entities = model.entities.map((e) => ({ ...e, scenarioId: asset.name }));
        ...
        if (flat.scenario !== null) {
          try {
            const openingPrompt = [
              "你是 CoC 7e 守秘人。为以下调查团写一段 80-160 字的开场白：",
              "剧本：" + flat.scenario.name,
              "剧本摘要：" + (flat.scenario.summary ?? "").slice(0, 600),
              "调查员：" + names,
              "要求：营造氛围、交代初始场景，以第二人称面对玩家；不要替玩家做决定，不要推进剧情。",
            ].join("\n");
