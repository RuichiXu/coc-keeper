 ...
 * @param {object} deps
 */
export function registerStateTools(ctx, deps) {
  // ── coc_scene ───────────────────────────────────────────
  ctx.tools.register(
    defineTool({
      name: "coc_scene",
  ...
  ctx.tools.register(
    defineTool({
      name: "coc_entity",
      description:
        "管理剧情中的可交互实体（KP/面板用）：NPC、地点、物品、组织等。add 添加、update 修改、remove 删除、list 列出、reveal 揭示（对玩家可见）。剧本导入时会按标记草拟实体，可在此基础上校对；默认不向玩家视图透露实体。",
      parameters: {
        ...
        entity: {
          ...
          description: "add/update 的实体：{id?, type: npc|location|item|org|other, name, desc?, state?, scene?, revealed?}",
        },
