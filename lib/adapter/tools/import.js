 ...
 */
import { compileByPattern, ASSET_KINDS, sanitizeMetaText } from "../../core/index.js";
    ...
    ctx.tools.register({
      ...
      async execute(args, execCtx) {
        ...
        if (toolName === "coc_import") {
          // 清理剧本结构中的模组元叙事措辞（“是模组主要探索场景”等），
          // 并确保实体默认不向玩家视图泄露（revealed 显式由 KP 控制）。
          flat.keyPoints = (flat.keyPoints ?? []).map((kp) => ({
            ...kp,
            desc: sanitizeMetaText(kp.desc ?? kp.description ?? ""),
          }));
          flat.branches = (flat.branches ?? []).map((b) => ({
            ...b,
            desc: sanitizeMetaText(b.desc ?? b.description ?? ""),
          }));
          flat.entities = (flat.entities ?? []).map((e) => ({
            ...e,
            desc: sanitizeMetaText(e.desc ?? e.description ?? ""),
            revealed: e.revealed === true,
            playerDesc: e.playerDesc ?? "",
            playerState: e.playerState ?? "",
          }));
          // 让运行期 WorldState 也立即采用清理后的实体（否则 commitSession 会投影回旧值）
