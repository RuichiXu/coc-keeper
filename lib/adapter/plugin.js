...
const PRESET_GAMES_DIR = join(moduleDir, "..", "..", "presets", "games");
...
function safeGameId(id) {
  // 保留中文：历史数据中存在 verify-对流 这类含中文的场次 id，
  // 旧规则会把中文压成连字符，导致 stateKey 与磁盘文件名不一致，
  // 场次读不到数据、也删不掉。文件名与资产 id 本就允许中文。
...
// stateKey 在 apply 内定义（需要 dataDir 做目录级自愈回退）。
...
function seedBuiltinAssets(assetStore) {
  ...
  // 清理历史重复调查员卡（宽松同名保留最新一张，其余删除）
  const normalize = (value) => String(value).toLowerCase().replace(/[\s·"'“”‘’.。,_-]/g, "");
  const seen = new Map();
  for (const asset of assetStore.list(ASSET_KINDS.INVESTIGATOR)) {
    const key = normalize(asset.name);
    const existing = seen.get(key);
    if (existing === undefined) {
      seen.set(key, asset);
    } else {
      const keep = new Date(existing.updatedAt ?? 0) >= new Date(asset.updatedAt ?? 0) ? existing : asset;
      const remove = keep === existing ? asset : existing;
      assetStore.delete(ASSET_KINDS.INVESTIGATOR, remove.id);
      seen.set(key, keep);
      console.log(`[coc-keeper] 已清理重复调查员卡：${remove.name}（保留 ${keep.name}）`);
    }
  }
}
...
// 内置场次：随包分发的游戏 DB，缺失时才播种到 dataDir/games/。
function seedBundledGames(dataDir) {
  if (!existsSync(PRESET_GAMES_DIR)) return;
  const gamesDir = join(dataDir, "games");
  mkdirSync(gamesDir, { recursive: true });
  for (const file of readdirSync(PRESET_GAMES_DIR)) {
    if (!file.endsWith(".json")) continue;
    const gameId = file.slice(0, -".json".length);
    const target = join(gamesDir, `${safeGameId(gameId)}.json`);
    if (existsSync(target)) continue;
    try {
      copyFileSync(join(PRESET_GAMES_DIR, file), target);
      console.log(`[coc-keeper] 已播种内置场次：${gameId}`);
    } catch (error) {
      console.log(`[coc-keeper] 内置场次播种失败：${gameId}（${String(error?.message ?? error)}）`);
    }
  }
}
...
export function apply(ctx, config) {
  ...
  // stateKey 自愈：safeGameId 规范化后的文件名不存在时，按 games/ 目录扫描
  // 原始 id 或文件内 id 精确匹配的文件。修复「下拉框能看到场次，但数据为空
  // 且无法删除」这类 id 与磁盘文件名不一致的问题。
  function stateKey(gameId) {
    const clean = safeGameId(gameId);
    const primary = join("games", `${clean}.json`);
    if (existsSync(join(dataDir, primary))) return primary;
    const gamesDir = join(dataDir, "games");
    if (existsSync(gamesDir)) {
      ...
      for (const file of readdirSync(gamesDir)) {
        ...
        if (fileId === rawId) return join("games", file);
        try {
          const flat = JSON.parse(readFileSync(join(gamesDir, file), "utf8"));
          if (flat !== null && typeof flat === "object" && (flat.id === rawId || flat.core?.id === rawId)) {
  ...
  // 1) 内置场次播种：presets/games 缺失时才复制，绝不覆盖玩家已有进度。
  seedBundledGames(dataDir);

  // 2) 旧布局迁移：dataDir/*.json → games/<id>.json + assets/**。
  ...
  // 1) 先装配 legacy：仅保留 systemPrompt 注入与内置规则自动导入。
  //    legacy 工具定义全部丢弃；模型/API/聊天工具均由 shared 提供。
