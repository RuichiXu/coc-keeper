...
var S = { digest: null, entries: [], seq: 0, tab: tabPref(), busy: false, error: "", importView: null };
			...
			function mountPanel() {
				...
				gameSelect.addEventListener("change", function () {
					if (NET_UI._rerenderTimer !== undefined && NET_UI._rerenderTimer !== null) {
						clearTimeout(NET_UI._rerenderTimer);
						NET_UI._rerenderTimer = null;
					}
					NET_UI.search = "";
					NET_UI.scene = "all";
					NET_UI.type = "all";
				...
				var NET_UI = { search: "", type: "all", scene: "all", foldChecks: true, zoom: 1, selected: null, focus: null, issueSeverity: "all", layout: "scene", showReturnEdges: false };
				...
				function buildSceneOverview(nodeById, edgeList, ui) {
					// 场景总览：把同一 scene 的关键点聚合成一个大节点，展示场景间关系。
					function seqOf(node) { var s = Number(node && node.seq); return isFinite(s) ? s : 1e9; }
					var byScene = new Map();
					nodeById.forEach(function (node) {
						if (node.type !== "kp" || node.virtual === true) return;
						...
						var scene = String(node.scene || "");
						if (scene.length === 0) scene = "未命名场景";
						if (!byScene.has(scene)) byScene.set(scene, []);
						byScene.get(scene).push(node);
					});
					var sceneNodes = [];
					var sceneByTitle = new Map();
					var auxNodes = new Map();
					byScene.forEach(function (members, scene) {
						var minSeq = 1e9;
						members.forEach(function (m) {
							var s = seqOf(m);
							if (s < minSeq) minSeq = s;
						});
						if (!isFinite(minSeq)) minSeq = 1e9;
						var node = { type: "kp", id: "scene:" + scene, title: scene, scene: scene, virtual: false, reachability: "strict", seq: minSeq, _sceneAgg: true, _kpCount: members.length, _memberKps: members };
						sceneNodes.push(node);
						sceneByTitle.set(scene, node);
						auxNodes.set(netNodeId("kp", node.id), node);
					});
					sceneNodes.sort(function (a, b) { return seqOf(a) - seqOf(b) || String(a.id).localeCompare(String(b.id)); });
					// 大节点网格：6 列，横向推进，超宽换行
					var COLS = 6;
					var SLOT_W = 200;
					var SLOT_H = 150;
					...
					var maxX = 0;
					var maxY = 0;
					sceneNodes.forEach(function (n, i) {
						var col = i % COLS;
						var row = Math.floor(i / COLS);
						n._x = START_X + col * SLOT_W;
						n._y = START_Y + row * SLOT_H;
						if (n._x > maxX) maxX = n._x;
						if (n._y > maxY) maxY = n._y;
					});
					// 场景间边：跨场景 plotEdge 聚合去重
					var edgeMap = new Map();
					edgeList.forEach(function (edge) {
						if (edge.kind !== "edge" && edge.kind !== "leads") return;
						var f = edge.from;
						var t = edge.to;
						if (!f || !t || f.type !== "kp" || t.type !== "kp") return;
						var sf = String(f.scene || "");
						var st = String(t.scene || "");
						if (sf.length === 0) sf = "未命名场景";
						if (st.length === 0) st = "未命名场景";
						if (sf === st) return;
						var a = sceneByTitle.get(sf);
						var b = sceneByTitle.get(st);
						if (!a || !b || a === b) return;
						var key = netNodeId(a.type, a.id) + "->" + netNodeId(b.type, b.id);
						var rec = edgeMap.get(key);
						if (rec !== undefined) rec._sceneCount += 1;
						else edgeMap.set(key, { from: a, to: b, label: "", requires: [], kind: "scene", fallback: edge.fallback === true, _sceneCount: 1 });
					});
					var visEdges = Array.from(edgeMap.values());
					visEdges.forEach(function (edge) {
						if (edge._sceneCount > 1) edge.label = "×" + edge._sceneCount;
						if ((edge.to._x || 0) <= (edge.from._x || 0) + 40) edge._detour = true;
					});
					return {
						visible: sceneNodes,
						visEdges: visEdges,
						svgWidth: Math.max(760, maxX + 240),
						svgHeight: Math.max(380, maxY + 220),
						auxNodes: auxNodes,
					};
				}
				function buildSkeletonView(nodeById, edgeList, ui) {
					...
					var laneSpacing = maxSpokes > 8 ? 50 : 66;
				...
				function renderNetContent(box, dp, meta) {
					...
					// 默认布局：kp 节点 > 60，或已存在虚拟枢纽节点时进骨架总览；否则保持场景视图。
					// 用户手动点布局按钮后（_layoutUser）不再自动切换；数据未就绪时下一次渲染会重算。
					if (NET_UI._layoutUser !== true) {
						var kpCountSet = new Set();
						var hasVirtualHub = false;
						digestKeyPoints().forEach(function (kp) { if (kp && kp.id) kpCountSet.add(String(kp.id)); if (kp && kp.virtual === true) hasVirtualHub = true; });
						(dp.keyPoints || []).forEach(function (kp) { if (kp && kp.id) kpCountSet.add(String(kp.id)); if (kp && kp.virtual === true) hasVirtualHub = true; });
						NET_UI.layout = (kpCountSet.size > 60 || hasVirtualHub) ? "skeleton" : "scene";
					}
					if (NET_UI.layout === "flow") NET_UI.layout = "skeleton";

					// 搜索/筛选输入 100ms debounce 后再重建，避免逐键整图重绘
					function scheduleNetRerender() {
						if (NET_UI._rerenderTimer !== undefined && NET_UI._rerenderTimer !== null) clearTimeout(NET_UI._rerenderTimer);
						NET_UI._rerenderTimer = setTimeout(function () {
							NET_UI._rerenderTimer = null;
							renderNetContent(box, dp, meta);
						}, 100);
					}
					...
					var sceneModeBtn = el("button", { type: "button" }, "场景总览");
					var skelModeBtn = el("button", { type: "button" }, "骨架总览");
					if (NET_UI.layout === "scene") sceneModeBtn.style.background = "rgba(60,90,160,.45)";
					if (NET_UI.layout === "skeleton") skelModeBtn.style.background = "rgba(60,90,160,.45)";
					sceneModeBtn.addEventListener("click", function () { NET_UI.layout = "scene"; NET_UI._layoutUser = true; renderNetContent(box, dp, meta); });
					skelModeBtn.addEventListener("click", function () { NET_UI.layout = "skeleton"; NET_UI._layoutUser = true; renderNetContent(box, dp, meta); });
					toolbar.append(sceneModeBtn, skelModeBtn);
					var search = el("input", { type: "text", placeholder: "搜索节点标题/id/场景", spellcheck: "false" });
					search.value = NET_UI.search;
					search.addEventListener("input", function () { NET_UI.search = search.value.trim(); scheduleNetRerender(); });
					...
					typeSel.addEventListener("change", function () { NET_UI.type = typeSel.value; scheduleNetRerender(); });
					...
					sceneSel.addEventListener("change", function () { NET_UI.scene = sceneSel.value; scheduleNetRerender(); });
					toolbar.append(sceneSel);
					if (NET_UI.layout === "scene") {
						toolbar.append(el("span", { class: "coc-mini" }, "每个节点 = 一个场景 · 边 = 场景间剧情联系"));
					} else if (NET_UI.layout === "skeleton") {
						var retLabel = el("label", null, "");
						var retInput = el("input", { type: "checkbox" });
						retInput.checked = NET_UI.showReturnEdges === true;
						retInput.addEventListener("change", function () { NET_UI.showReturnEdges = retInput.checked; renderNetContent(box, dp, meta); });
						retLabel.append(retInput, document.createTextNode("显示返回边"));
						toolbar.append(retLabel);
						toolbar.append(el("span", { class: "coc-mini" }, "左主线 · 中枢纽/辐条 · 右汇聚"));
					} else {
					...
					// 大网络（节点+边 > 200）性能降级：不画边标签/悬停 title、不自动开详情、
					// 节点与边的点击统一走 svg 事件委托（见下方 svg click 监听）。
					var bigNet = nodeById.size + edgeList.length > 200;
					var flowBig = flowMode && nodeById.size + edgeList.length > 120;
					var perfBig = bigNet || flowBig;
					var auxNetNodes = new Map();
					var edgeByEndpoint = new Map();

					var flowMode = NET_UI.layout === "flow";
					var skeletonMode = NET_UI.layout === "skeleton";
					var visible = [];
					var foldedCount = 0;
					if (!flowMode && !skeletonMode) {
						nodeById.forEach(function (node) {
					...
					if (flowMode) {
						...
						// 骨架总览：主线 / 枢纽辐条 / 返回汇聚（详见 buildSkeletonView）
						var skel = buildSkeletonView(nodeById, edgeList, NET_UI);
						visible = skel.visible;
						visEdges = skel.visEdges;
						auxNetNodes = skel.auxNodes;
						svgWidth = skel.svgWidth;
						svgHeight = skel.svgHeight;
					} else {
						// 场景总览：场景级聚合节点 + 场景间关系（详见 buildSceneOverview）
						var sceneView = buildSceneOverview(nodeById, edgeList, NET_UI);
						visible = sceneView.visible;
						visEdges = sceneView.visEdges;
						auxNetNodes = sceneView.auxNodes;
						svgWidth = sceneView.svgWidth;
						svgHeight = sceneView.svgHeight;
					}
					...
					graphCard.append(el("h4", null, (flowMode ? "演进拓扑" : skeletonMode ? "骨架总览" : "场景总览") + (foldedCount > 0 ? "（已折叠 " + foldedCount + " 个检定分支）" : "") + " · " + visible.length + " 节点 / " + visEdges.length + " 边"));
					...
					if (skeletonMode) legend.append(el("span", { style: "border-bottom:2px solid rgba(255,220,130,.9);padding-bottom:1px" }, "金色=主线必经 · 双箭头=枢纽可往返 · 淡色细线=辐条列内边 · 淡虚线=推断汇聚边"));
					if (NET_UI.layout === "scene") legend.append(el("span", { style: "color:#9fe0c0" }, "▭ 场景聚合节点（角标=场景内关键点数）"));
					...
					defs.append(marker);
					// 反向箭头：骨架总览的双向边用 marker-start 表示「可返回」
					var markerStart = svgEl("marker", { id: "coc-net-arrow-start", markerWidth: "8", markerHeight: "8", refX: "1", refY: "4", orient: "auto", markerUnits: "strokeWidth" });
					markerStart.append(svgEl("path", { d: "M8,0 L0,4 L8,8 z", fill: "#8fa4d4" }));
					...
					// 节点/边点击统一走 svg 事件委托：避免大网络上逐元素 addEventListener。
					// 边按 data-from/data-to 反查、节点按 data-type/data-id 反查。
					svg.addEventListener("click", function (ev) {
						var target = ev.target;
						if (!target || typeof target.closest !== "function") return;
						var edgeHit = target.closest(".edge");
						if (edgeHit !== null && svg.contains(edgeHit)) {
							var eKey = (edgeHit.getAttribute("data-from") || "") + "\n" + (edgeHit.getAttribute("data-to") || "");
							var hitEdge = edgeByEndpoint.get(eKey);
							if (hitEdge !== undefined) { renderNetEdgeDetail(graphCard, hitEdge, nodeById); return; }
						}
						var nodeHit = target.closest(".node");
						if (nodeHit !== null && svg.contains(nodeHit)) {
							var nKey = netNodeId(nodeHit.getAttribute("data-type") || "", nodeHit.getAttribute("data-id") || "");
							var hitNode = nodeById.has(nKey) ? nodeById.get(nKey) : auxNetNodes.get(nKey);
							if (hitNode !== undefined && hitNode != null) renderNetNodeDetail(graphCard, hitNode, nodeById, edgeList, dp);
						}
					});

					// hover 高亮：悬停节点/边时，只高亮相关入边出边，其余变淡。
					svg.addEventListener("mouseover", function (ev) {
						var target = ev.target;
						if (!target || typeof target.closest !== "function") return;
						var nodeHit = target.closest(".node");
						var edgeHit = target.closest(".edge");
						var focusKeys = new Set();
						var hoverIsNode = false;
						if (nodeHit !== null && svg.contains(nodeHit)) {
							hoverIsNode = true;
							focusKeys.add(netNodeId(nodeHit.getAttribute("data-type") || "", nodeHit.getAttribute("data-id") || ""));
						} else if (edgeHit !== null && svg.contains(edgeHit)) {
							focusKeys.add(edgeHit.getAttribute("data-from") || "");
							focusKeys.add(edgeHit.getAttribute("data-to") || "");
						} else {
							return;
						}
						Array.prototype.forEach.call(svg.querySelectorAll(".node"), function (g) {
							var k = netNodeId(g.getAttribute("data-type") || "", g.getAttribute("data-id") || "");
							var on = focusKeys.has(k);
							g.classList.toggle("dim", !on);
							g.classList.toggle("focus", on);
						});
						Array.prototype.forEach.call(svg.querySelectorAll(".edge"), function (p) {
							var f = p.getAttribute("data-from") || "";
							var t = p.getAttribute("data-to") || "";
							var on = hoverIsNode ? (focusKeys.has(f) || focusKeys.has(t)) : (focusKeys.has(f) && focusKeys.has(t));
							p.classList.toggle("dim", !on);
							p.classList.toggle("focus", on);
						});
					});
					...
					var showEdgeLabels;
					if (perfBig) showEdgeLabels = false;
					else if (flowMode) showEdgeLabels = visEdges.length <= 120;
					else if (skeletonMode) showEdgeLabels = visEdges.length <= 24;
					else showEdgeLabels = NET_UI.focus === "final" || visEdges.length <= 24;
					visEdges.forEach(function (edge) {
						var x1 = edge.from._x + 18;
						var y1 = edge.from._y;
						var x2 = edge.to._x - 18;
						var y2 = edge.to._y;
						var classes = "edge" + (Array.isArray(edge.requires) && edge.requires.length > 0 ? " has-req" : "") + (edge.fallback === true ? " fallback" : "") + (edge._inner === true ? " skel-inner" : "") + (edge._return === true ? " return" : "") + (edge._main === true ? " main" : "");
						var edgeD;
						if (edge._detour === true) {
							// 回流/跨车道边：从源右侧绕到目标右侧外侧走廊，再拐入目标左侧，不穿中间节点。
							var detourX = Math.max(x1, x2) + 90;
							edgeD = "M " + x1 + " " + y1 + " L " + (x1 + 18) + " " + y1 + " L " + detourX + " " + y1 + " L " + detourX + " " + y2 + " L " + (x2 - 18) + " " + y2;
						} else if (flowBig) {
							edgeD = "M " + x1 + " " + y1 + " L " + x2 + " " + y2;
						} else {
							edgeD = "M " + x1 + " " + y1 + " C " + (x1 + (x2 - x1) * 0.5) + " " + y1 + ", " + (x1 + (x2 - x1) * 0.5) + " " + y2 + ", " + x2 + " " + y2;
						}
						var path = svgEl("path", { class: classes, d: edgeD, "marker-end": "url(#coc-net-arrow)" });
						if (edge._bidir === true) path.setAttribute("marker-start", "url(#coc-net-arrow-start)");
						var eFromKey = netNodeId(edge.from.type, edge.from.id);
						var eToKey = netNodeId(edge.to.type, edge.to.id);
						path.setAttribute("data-kind", edge.kind || "");
						path.setAttribute("data-from", eFromKey);
						path.setAttribute("data-to", eToKey);
						var endpointKey = eFromKey + "\n" + eToKey;
						if (!edgeByEndpoint.has(endpointKey)) edgeByEndpoint.set(endpointKey, edge);
					...
					visible.forEach(function (node) {
						var group = svgEl("g", { class: "node" + (node.virtual === true ? " virtual" : "") + (node._main === true ? " main" : "") + (node._sceneAgg === true ? " scene-agg" : ""), transform: "translate(" + node._x + "," + node._y + ")", "data-id": String(node.id), "data-type": node.type });
						var shape;
						if (node.type === "kp") {
