					...
					function loadInvestigatorChoices() {
						...
						post("/coc-api/assets", { kind: "investigators", action: "list" }).then(function (json) {
							...
							players.forEach(function (inv) {
								...
								var info = esc(inv.name) + "｜" + esc(inv.occupation || "无职业") + "｜HP " + inv.hp + " SAN " + inv.san;
								label.append(cb, el("span", null, info));
								cb.addEventListener("change", function () {
									if (cb.checked) { if (!ws.characterAssetIds.includes(inv.id)) ws.characterAssetIds.push(inv.id); }
									else { ws.characterAssetIds = ws.characterAssetIds.filter(function (x) { return x !== inv.id; }); }
								});
								pcList.append(label);
							});
