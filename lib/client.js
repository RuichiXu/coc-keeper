	...
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			function mountPanel() {
				...
				head.append(el("b", null, "🎲 CoC 面板"));
				var gameInput = el("input", { type: "text", value: gameId(), title: "游戏 ID", spellcheck: "false" });
				head.append(gameInput);
				...
				var TAB_DEFS = [["chat", "聊天"], ["status", "状态"], ["plot", "剧情"], ["chars", "人物"], ["ents", "实体"], ["import", "导入"], ["settings", "设置"]];
				var tabButtons = {};
				...
				// 各面板容器
				var panels = {};
				["chat", "status", "plot", "chars", "ents", "import", "settings"].forEach(function (key) {
				...
				function showTab(key) {
					S.tab = key;
					setTabPref(key);
					Object.keys(panels).forEach(function (k) { panels[k].style.display = k === key ? "flex" : "none"; });
					Object.keys(tabButtons).forEach(function (k) { tabButtons[k].classList.toggle("on", k === key); });
					if (key !== "chat") renderPanel(key);
				}
				...
				gameInput.addEventListener("change", function () {
					setGame(gameInput.value.trim() || "default");
					resetSession();
					poll(true);
				});
				...
				function renderPanel(key) {
					if (key === "status") renderStatusPanel();
					else if (key === "plot") renderPlotPanel();
					else if (key === "chars") renderCharsPanel();
					else if (key === "ents") renderEntsPanel();
					else if (key === "import") renderImportPanel();
					else if (key === "settings") renderSettingsPanel();
				}
