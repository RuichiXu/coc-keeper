				...
				function renderEntsPanel() {
					...
					Object.keys(groups).forEach(function (type) {
						...
						groups[type].forEach(function (e) {
							...
							revealBtn.addEventListener("click", function () {
								tool("coc_entity", { action: "update", entityId: e.id, entity: { revealed: e.revealed !== true } }).then(function () { poll(true); });
							});
							...
							var row = el("div", { class: "coc-row" });
							row.append(stateInput, saveState, revealBtn, toAsset, del);
							item.append(row);
							card.append(item);
						});
						box.append(card);
