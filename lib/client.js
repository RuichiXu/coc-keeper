...
`
			...
			function mountPanel() {
				...
				function renderNetPanel() {
					...
					var requestedAsset = S.digest && typeof S.digest.scenarioId === "string" && S.digest.scenarioId.trim() ? S.digest.scenarioId : "";
					if (box._dispose) box._dispose();
					box.replaceChildren(el("div", { class: "coc-empty", role: "status" }, "正在加载剧情网络…"));
					// 剧情网络与剧本资产绑定：当前场次绑定了剧本资产时直接按资产读取，
					// 同一剧本在不同场次看到同一张网络；未绑定/资产缺失时后端会回退场次数据。
