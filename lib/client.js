	...
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			function mountPanel() {
				...
				["status", "plotInner", "ents", "chars", "import", "settings"].forEach(function (key) {
					var box = el("div", { class: "coc-subpanel", "data-subpanel": key });
					panels[key] = box;
				});
				panels.plot.append(panels.status, panels.plotInner);
				panels.debug.append(panels.ents, panels.chars, panels.import, panels.settings);
				...
				function openGameWizard() {
					...
					next1.addEventListener("click", function () {
						...
						step2.querySelectorAll(".coc-wiz-pc-list").forEach(function (n) { n.remove(); });
						step2.querySelectorAll(".coc-wiz-scen").forEach(function (n) { n.remove(); });
						var scenInfo = el("div", { class: "coc-wiz-scen coc-mini" }, hintText);
					...
					step2.append(pcList);
					var aiHint = el("div", { class: "coc-mini", style: "margin-top:6px" });
					step2.append(aiHint);
					...
					function loadInvestigatorChoices() {
						...
						post("/coc-api/assets", { kind: "investigators", action: "list" }).then(function (json) {
							...
							pcList.textContent = "";
							ws.investigators.forEach(function (inv) {
								var label = el("label", { class: "coc-wiz-pc" });
								var cb = el("input", { type: "checkbox", style: "width:15px;height:15px" });
								var info = esc(inv.name) + "｜" + esc(inv.occupation || "无职业") + "｜HP " + inv.hp + " SAN " + inv.san + (inv.aiControlled ? "｜🤖 AI" : "");
								label.append(cb, el("span", null, info));
								cb.addEventListener("change", function () {
									if (cb.checked) { if (inv.aiControlled) ws.aiInvestigatorIds.push(inv.id); else ws.characterAssetIds.push(inv.id); }
									else {
										ws.characterAssetIds = ws.characterAssetIds.filter(function (x) { return x !== inv.id; });
										ws.aiInvestigatorIds = ws.aiInvestigatorIds.filter(function (x) { return x !== inv.id; });
									}
									var total = ws.characterAssetIds.length + ws.aiInvestigatorIds.length;
									aiHint.textContent = total === 0 ? "尚未选择调查员" : "已选 " + ws.characterAssetIds.length + " 名玩家调查员 + " + ws.aiInvestigatorIds.length + " 名 AI 调查员";
								});
								pcList.append(label);
							});
				...
				function renderDebugPanel() {
					...
					box.textContent = "";
					var row = el("div", { class: "coc-row" });
					[["chars", "人物"], ["import", "导入"], ["ents", "实体"], ["settings", "设置"]].forEach(function (pair) {
					...
					panels.chars.style.display = "none";
					panels.settings.style.display = "none";
					box.append(panels.import, panels.ents, panels.chars, panels.settings);
			...
			function mountPlayerPanel() {
				...
				function renderPlayer(data, appendLog) {
					...
					if (log.length > 0) {
						...
						logCard.append(el("h4", null, "最近动态" + (data.logLength !== undefined ? "（" + Math.min(playerLogOffset + log.length, data.logLength) + "/" + data.logLength + "）" : "")));
						log.forEach(function (entry) {
							var line = el("div", { class: "pp-msg " + (entry.kind || "") });
							line.textContent = (entry.kind === "user" ? (entry.player || "玩家") + "：" : "KP：") + entry.text;
							logCard.append(line);
						});
						if (data.logHasMore === true) {
							var moreBtn = el("button", { type: "button", style: "margin-top:6px" }, "加载更多");
							moreBtn.addEventListener("click", function () { playerLogOffset += log.length; pollPlayer(true); });
							logCard.append(moreBtn);
						}
						body.append(logCard);
					}
				}
				function pollPlayer(appendLog) {
					var qs = "/coc-api/player-view?game=" + encodeURIComponent(gameId());
					if (appendLog === true) qs += "&after=" + playerLogOffset + "&limit=20";
					else qs += "&after=0&limit=20";
