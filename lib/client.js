		...
		factory: function () {
			...
			function refreshGameSelects() {
				return api("/coc-api/games").then(function (json) {
					...
					var current = gameId();
					var list = json.data || [];
					// 当前 localStorage 里的场次 id 已不存在时（例如数据修复后
					// id 从 verify 改为 verify-对流），自动切到列表第一项，否则
					// 会一直用旧 id 请求 /coc-api/state 得到空数据，删除也落空。
					var currentExists = list.some(function (g) { return g.id === current; });
					if (!currentExists && list.length > 0) {
						current = list[0].id;
						setGame(current);
					}
					GAME_SELECTS.forEach(function (sel) {
						var signature = JSON.stringify([current, list]);
...
#coc-keeper-panel .coc-net-asset-label{font-size:11.5px;color:#9db0d8}
#coc-keeper-panel .coc-net-empty-wrap{display:grid;gap:10px;justify-items:start}
...
#coc-help-dialog{padding:0;width:min(940px,94vw);height:min(760px,90vh);max-height:90dvh;box-sizing:border-box;border:1px solid #40515e;border-radius:18px;background:#101923;color:#e6ebf8;box-shadow:0 24px 100px #0009;font:14px/1.75 -apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif;overflow:hidden}
#coc-help-dialog[open]{display:flex;flex-direction:column}
#coc-help-dialog::backdrop{background:rgba(2,6,16,.72)}
#coc-help-dialog *{box-sizing:border-box}
#coc-help-dialog .coc-help-header{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:20px 28px;border-bottom:1px solid #304351;background:#16232e}
#coc-help-dialog h2{font-size:22px;line-height:1.4;margin:2px 0 0}
#coc-help-dialog .coc-help-kicker{font-size:10px;letter-spacing:.14em;color:#edca80}
#coc-help-dialog button{font:inherit;color:inherit;border:1px solid #40515e;border-radius:8px;background:#1c2b38;padding:7px 12px;cursor:pointer}
#coc-help-dialog button:hover{background:#293c4c}
#coc-help-dialog :focus-visible{outline:2px solid #edca80;outline-offset:3px}
#coc-help-dialog .coc-help-header button{font-size:12px;flex:none}
#coc-help-dialog .coc-help-layout{display:grid;grid-template-columns:180px minmax(0,1fr);flex:1;min-height:0}
#coc-help-dialog nav{display:flex;flex-direction:column;gap:6px;padding:18px 12px;border-right:1px solid #304351;overflow:auto;overscroll-behavior:contain}
#coc-help-dialog nav button{display:flex;gap:12px;align-items:center;text-align:left;border-color:transparent;background:transparent;white-space:nowrap}
#coc-help-dialog nav button span{color:#889dac;font-size:11px;font-variant-numeric:tabular-nums}
#coc-help-dialog nav button[aria-current]{color:#ffdc82;background:#342f23;border-color:#67573a}
#coc-help-dialog article{padding:28px 32px;overflow:auto;overscroll-behavior:contain;scrollbar-gutter:stable;outline-offset:-3px;overflow-wrap:anywhere}
#coc-help-dialog h3{font-size:26px;line-height:1.4;margin:6px 0 10px;color:#f5f2e9}
#coc-help-dialog .coc-help-intro{color:#9fb0bd;margin-bottom:26px}
#coc-help-dialog section{padding-top:20px;margin-top:20px;border-top:1px solid #293b48}
#coc-help-dialog h4{font-size:15px;color:#f0dba9;margin:0 0 10px}
#coc-help-dialog p{margin:10px 0}
#coc-help-dialog :is(ul,ol){padding-left:22px;margin:8px 0}
#coc-help-dialog li{padding-left:4px;margin:10px 0}
#coc-help-dialog li::marker{color:#d3b876}
#coc-help-dialog .coc-help-note{padding:12px 16px;border-left:3px solid #c7a45f;border-radius:0 8px 8px 0;background:#212b30;color:#dbd8c8}
@media(max-width:600px){#coc-help-dialog{width:96vw;height:92vh;max-height:92dvh;border-radius:12px}#coc-help-dialog .coc-help-header{padding:14px 18px}#coc-help-dialog .coc-help-layout{grid-template-columns:minmax(0,1fr);grid-template-rows:auto minmax(0,1fr)}#coc-help-dialog nav{flex-direction:row;padding:10px;border-right:0;border-bottom:1px solid #304351}#coc-help-dialog nav button{flex:none;gap:6px;font-size:12px}#coc-help-dialog article{padding:20px}#coc-help-dialog h3{font-size:22px}}
@media(prefers-reduced-motion:reduce){#coc-keeper-panel *{transition:none!important;scroll-behavior:auto!important}}
`
			].join("\n");

			// 内置手册按章节渲染；与 USER_GUIDE.md 的操作路径同步维护。
			var helpDialog = null;
			...
			function openHelpManual() {
				if (helpDialog) return;
				var chapters = [
					{ title: "认识面板", intro: "主持游戏、整理剧情、阅读解析，各有自己的工作区。", blocks: [
						{ title: "四个工作区", items: ["主持：对话、快捷明骰/暗骰、待处理检定与 KP 指令。", "剧情：状态总览管理场景、时间、人物、物品和任务；剧情结构跟进关键点、分支和提醒。", "解析：阅读网络图、查找结局和条件、查看质量报告、校对草稿。", "调试：导入、实体、人物、卡库、运行、契约和设置。"] },
						{ title: "Keeper 与玩家视图", text: "Keeper 面板用于主持与管理；玩家视图显示已公开信息并可输入行动。两者共用当前场次。在其中一个面板切换游戏，另一个也会切换。" },
						{ title: "随时查阅", text: "标题栏的「帮助」打开本手册。选择左侧目录阅读各章；窄屏下目录显示在上方。点击关闭或按 Esc 返回原页面，手册不会切换游戏或工作区。" }
					] },
					{ title: "第一次开团", intro: "先准备资料，再用三个步骤建立新场次。", blocks: [
						{ title: "准备模型与资料", ordered: true, items: ["调试 → 设置：检查 LLM 配置，保存并测试连接。已有可用配置时可直接继续。", "调试 → 导入：上传剧本、规则、人物卡，或粘贴文本；选择类型，等待导入完成。", "调试 → 卡库：确认已有剧本和调查员资产，供不同场次复用。"] },
						{ title: "新建场次向导", ordered: true, items: ["点击标题栏「＋」，填写场次名称并选择剧本；也可不选剧本创建空场次。列表未更新时点「刷新列表」。", "为玩家调查员点击「加入」，可选多张；需要时从下拉框选择 AI 调查员。不选人物继续会提示创建空场次。", "核对信息，点击「创建并开始」；看到开场白后点击「进入场次 →」。"] },
						{ title: "资料格式与卡库", text: "PDF 需有可提取的文字，扫描件暂不支持 OCR；DOC 提取失败时可改用 DOCX、TXT 或粘贴文本。卡库的「开新场次」预选该剧本打开向导，人物或实体的「加入场次」则加入当前游戏。", note: "导入前先核对标题栏中的场次。长剧本解析可能耗时较久，进度与错误信息会显示在导入页。" }
					] },
					{ title: "主持与剧情", intro: "把玩家行动、检定结果和结构化状态连起来。", blocks: [
						{ title: "对话与快捷骰", items: ["主持页输入行动，Enter 发送，Shift+Enter 换行。阅读历史消息时不会被新消息强制拉回底部。", "快捷骰填写骰式（默认 d100）、目标技能值、难度和说明，再选择明骰或暗骰。明骰公开，暗骰供 KP 秘密判定。", "已有待处理检定时优先操作对应检定卡，让结果关联到当前检定。宽面板的检定区在右侧，窄面板中在下方。"] },
						{ title: "KP 指令", text: "自然语言管理指令先点「解析预览」，核对将执行的操作，再「确认执行」。玩家行动直接从对话输入。" },
						{ title: "剧情的两个子页", items: ["状态总览：编辑概述、当前场景、时间与 KP 模式，管理人物数值、物品和任务。", "剧情结构：揭示关键点、标记分支抵达、选择分支选项、登记提醒。这些操作会修改当前场次进展。"] }
					] },
					{ title: "剧情网络", intro: "从左向右读主线，用检查栏追踪分支、条件与结局。", blocks: [
						{ title: "两种总览与图例", items: ["骨架总览：看主线、并行调查、分支、汇聚和结局，检定细节通过索引查看。", "场景总览：以矩形聚合场景关键点，保留场景间关系；点击聚合节点查看成员关键点。", "金色表示主线，圆形为关键点，菱形为分支，星形为结局；带「自」的分支是自动判定。↩ 表示可返回。", "悬停节点或边高亮关联线路；点击查看详情。检查栏 → 显示与图例可开启「显示返回边」。"] },
						{ title: "移动与缩放", items: ["滚轮以鼠标位置为中心缩放；「＋」「−」也可调整比例。拖拽空白画布平移。", "「适应画布」查看整图；「100%」恢复原始比例和起始位置。小于 30% 时暂时隐藏文字，放大恢复。", "点击图下方迷你导航跳到对应位置；聚焦后可用左右方向键移动。", "收起检查栏可扩大画布；窄面板中检查栏覆盖在图上。"] },
						{ title: "搜索与结局导航", text: "搜索标题、ID 或场景，组合类型与场景筛选。匹配节点突出显示，其他节点淡化；点检查栏的结果查看详情并尽可能定位。被折叠的检定节点仍能从索引找到。", note: "清除筛选后，检查栏显示「结局导航」。逐个打开结局核对条件；图中有结局不代表当前游戏已经满足它的前置条件。" }
					] },
					{ title: "质量与校对", intro: "先读报告和条件，再保存并确认采用的解析版本。", blocks: [
						{ title: "读懂质量报告", text: "解析 → 检查栏 → 质量统计与审校报告：查看来源、草稿/确认状态、结构、规则、审校与分块记录；展开列表可按严重程度筛选问题和建议。", note: "0 条问题记录不等于执行过语义审校。确定性检查干净时，后端可能跳过语义审校；暂无报告也不能视为通过。「已确认生效」只表示采用保存版本。" },
						{ title: "校对编辑与确认生效", ordered: true, items: ["展开同名区域，先保留一份原 JSON，再修改节点、边、结局或条件。", "点击「保存草稿」，等待成功并核对刷新后的内容。", "确认无误后点击「确认生效」。此按钮只确认已保存版本，不会自动保存输入框中的改动。"] },
						{ title: "结构编辑", text: "章节与场景层级在「剧本结构编辑」中修改，使用「保存结构编辑」。这会改变剧本结构；缺少数据时按提示重新导入或解析。当前校对使用 JSON，拖动画布只改变观看位置，不修改剧情连接。" }
					] },
					{ title: "资料与调试", intro: "管理资料、人物和设置，也能在测试场次中排查运行问题。", blocks: [
						{ title: "调试页入口", items: ["导入：文件或文本导入，查看进度与已导入全文。", "实体：管理 NPC、地点、物品等；编辑状态，揭示或隐藏。揭示时填写适合玩家阅读的内容。", "人物：查看当前人物属性和物品，更新 HP、SAN、MP、LUCK；新增可用导入、卡库或向导。", "卡库：复用剧本、调查员卡与实体资产；创建场次或加入当前场次。", "运行：查看检定、门禁与事件诊断，跳转测试剧情点或导出夹具复现问题。", "契约：查看、编辑草稿，保存后确认生效，供运行时校验使用。", "设置：保存和测试 LLM 配置，管理内置规则导入。"] },
						{ title: "会影响数据的操作", text: "「剧情点跳转（测试）」会重设剧情状态，请在测试场次使用。删除剧本资产会一并删除引用它的场次；只删除一个游戏时，用标题栏「··· → 删除当前场次」。" }
					] },
					{ title: "玩家与面板", intro: "切换场次、整理窗口，给玩家展示公开信息。", blocks: [
						{ title: "玩家视图", text: "显示公开的场景、人物、实体、线索与最近动态，也能输入行动。同一页面仍可打开 Keeper；共享屏幕时只选玩家视图，避免展示暗骰和未揭示信息。" },
						{ title: "标题栏与面板坞", items: ["场次下拉框切换游戏；「＋」新建；「⟳」刷新；「帮助」打开本手册。", "「⛶」最大化/还原；「···」重置位置与大小或删除当前场次；「−」最小化到面板坞。", "拖动标题栏移动面板；Keeper 边缘/右下角可调整大小，位置与尺寸自动记忆。", "右下角 🧩 面板坞可打开/隐藏面板，也能恢复最小化的 Keeper。玩家标题栏按钮折叠内容，再点展开。"] },
						{ title: "更新与场次", text: "两个面板共用当前场次，刷新浏览器会记住所选游戏。Keeper 约 2.5 秒、玩家视图约 3 秒更新；操作后可等待下一次更新或刷新页面。" }
					] },
					{ title: "常见问题", intro: "从当前场次、页面入口和界面提示开始排查。", blocks: [
						{ title: "面板与资料", items: ["找不到 Keeper：从右下角面板坞恢复；仍未挂载时检查插件是否启用，重启服务后刷新浏览器。", "窗口位置或尺寸不合适：更多菜单 → 重置位置与大小。", "向导没有剧本或人物：先导入，在卡库确认，再刷新向导列表。", "解析页没有网络：核对当前场次和导入结果；已有文本不一定已有可用深度解析。"] },
						{ title: "网络与校对", items: ["图上文字消失：缩放可能小于 30%，放大或搜索节点。", "找不到结局：清除关键词、类型和场景筛选，再看结局导航。", "修改未生效：检查 JSON，按「保存草稿 → 核对 → 确认生效」操作。", "聊天或导入失败：看界面错误，到调试 → 设置检查配置并测试连接。"] },
						{ title: "反馈问题", text: "注明 DSH 或独立网页版、浏览器、场次、所在 Tab/网络视图及复现步骤。截图可包含尺寸、缩放与错误提示；分享前移除密钥、访问令牌及不适合公开的剧本内容。" }
					] }
				];
				var dialog = el("dialog", { id: "coc-help-dialog", "aria-labelledby": "coc-help-title" });
				var header = el("header", { class: "coc-help-header" });
				var title = el("div");
				title.append(el("span", { class: "coc-help-kicker" }, "COC KEEPER · GUIDE"), el("h2", { id: "coc-help-title" }, "使用手册"));
				var close = el("button", { type: "button", "aria-label": "关闭使用手册" }, "关闭 · Esc");
				header.append(title, close);
				var layout = el("div", { class: "coc-help-layout" });
				var nav = el("nav", { "aria-label": "手册目录" });
				var article = el("article", { tabindex: "-1", "aria-labelledby": "coc-help-chapter-title" });
				var buttons = [];
				function showChapter(index, focus) {
					...
					fragment.append(el("span", { class: "coc-help-kicker" }, "使用指南 / " + String(index + 1).padStart(2, "0")), el("h3", { id: "coc-help-chapter-title" }, chapter.title), el("p", { class: "coc-help-intro" }, chapter.intro));
					chapter.blocks.forEach(function (block) {
						var section = el("section"); section.append(el("h4", null, block.title));
						if (block.text) section.append(el("p", null, block.text));
						if (block.items) {
							var list = el(block.ordered ? "ol" : "ul");
							block.items.forEach(function (text) { list.append(el("li", null, text)); }); section.append(list);
						}
						if (block.note) section.append(el("p", { class: "coc-help-note" }, block.note));
						fragment.append(section);
					});
					article.replaceChildren(fragment); article.scrollTop = 0;
					if (focus) article.focus({ preventScroll: true });
				}
				chapters.forEach(function (chapter, index) {
					var button = el("button", { type: "button" });
					button.append(el("span", { "aria-hidden": "true" }, String(index + 1).padStart(2, "0")), document.createTextNode(chapter.title));
					button.addEventListener("click", function () { showChapter(index, true); });
					buttons.push(button); nav.append(button);
				});
				layout.append(nav, article); dialog.append(header, layout);
				close.addEventListener("click", closeHelpManual);
				dialog.addEventListener("keydown", function (event) {
					if (event.key !== "Tab") return;
					var active = document.activeElement, last = buttons[buttons.length - 1];
					if (active === article || (event.shiftKey ? active === close : active === last)) {
						event.preventDefault(); (event.shiftKey ? last : close).focus();
					}
				});
				dialog.addEventListener("cancel", function (event) { event.preventDefault(); closeHelpManual(); });
				dialog.addEventListener("close", function () { if (helpDialog === dialog) helpDialog = null; dialog.remove(); });
				helpDialog = dialog;
				showChapter(0, false); document.body.append(dialog); dialog.showModal(); buttons[0].focus();
			}
			...
			function mountPanel() {
				...
				var helpBtn = el("button", { type: "button", title: "使用手册", "aria-label": "使用手册" }, "帮助");
				helpBtn.addEventListener("click", openHelpManual);
				head.append(helpBtn);
				...
				var NET_UI = { search: "", type: "all", scene: "all", selected: null, views: {}, issueSeverity: "all", layout: "scene", showReturnEdges: false, asset: "" };
				...
				function renderDpEditorCard(box, dp, meta) {
					...
					var assetId = meta && meta.asset ? meta.asset : (S.digest && S.digest.scenarioId) || "";
					if (assetId) card.append(el("p", { class: "coc-mini" }, "剧情网络已与剧本资产绑定：保存/确认会写入资产，同剧本的其他场次也会读到。"));
					...
					saveBtn.addEventListener("click", function () {
						...
						saveBtn.disabled = true; status.textContent = "保存中…";
						var payload = { deepParse: parsed, status: "draft", source: "manual" };
						if (assetId) payload.asset = assetId;
						post("/coc-api/deep-parse", payload).then(function (json) {
							saveBtn.disabled = false;
							if (!json.ok) { status.textContent = "保存失败：" + (json.error || "未知错误"); return; }
							status.textContent = assetId ? "草稿已保存到剧本资产。" : "草稿已保存。";
							kpResetAndPoll && kpResetAndPoll();
						});
					});
					confirmBtn.addEventListener("click", function () {
						...
						post("/coc-api/deep-parse", payload).then(function (json) {
							...
							status.textContent = assetId ? "已确认生效（已写入剧本资产）。" : "已确认生效。";
							kpResetAndPoll && kpResetAndPoll();
						});
					});
				...
				function createNetModel(dp, base, overlay) {
					// base：解析时落盘在剧本资产里的完整结构（keyPoints/branches），
					//       节点集合由它 + deepParse 接线节点决定，与具体场次无关。
					// overlay：仅当“跟随当前场次”且场次剧本与查看资产一致时传入，
					//          只叠加执行状态（revealed/reached），不增删节点。
					function baseKeyPoints() { return base ? base.keyPoints || [] : []; }
					function baseBranches() { return base ? base.branches || [] : []; }
					function overlayKeyPoints() { return overlay ? overlay.keyPoints || [] : []; }
					function overlayBranches() { return overlay ? overlay.branches || [] : []; }
					...
					var baseKpCount = baseKeyPoints().length;
					baseKeyPoints().forEach(function (kp, index) { addNode("kp", kp.id, kp.title, { scene: kp.scene, desc: kp.desc, revealed: false, virtual: kp.virtual === true, reachability: kp.reachability, order: kp.order, seq: index, flowRole: kp.flowRole, kind: kp.kind, parentId: kp.parentId }); });
					(dp.keyPoints || []).forEach(function (kp, index) { addNode("kp", kp.id, kp.title, { scene: kp.scene, desc: kp.desc, revealed: false, virtual: kp.virtual === true, reachability: kp.reachability, order: kp.order, seq: baseKpCount + index, flowRole: kp.flowRole, kind: kp.kind, parentId: kp.parentId }); });
					baseBranches().forEach(function (b) { addNode("br", b.id, b.title, { scene: b.scene, desc: b.desc, options: b.options || [], reached: false, checkpointBranch: b.checkpointBranch === true, autoChoose: b.autoChoose === true }); });
					...
					// 执行状态覆盖：只更新已存在的节点，不引入场次节点。
					overlayKeyPoints().forEach(function (kp) {
						var node = nodeById.get(netNodeId("kp", kp.id));
						if (node && kp.revealed === true) node.revealed = true;
					});
					overlayBranches().forEach(function (b) {
						var node = nodeById.get(netNodeId("br", b.id));
						if (node && b.reached === true) node.reached = true;
					});
					...
					baseBranches().concat(dp.branches || []).forEach(function (branch) {
						(branch.options || []).forEach(function (option) {
				...
				var netRequest = 0;
				function renderNetAssetSelect(container, currentAssetId) {
					// 解析页独立的剧本切换：不依赖场次下拉，直接切换剧本资产。
					var assetSel = el("select", { "aria-label": "切换剧本资产", style: "max-width:240px" });
					assetSel.append(el("option", { value: "__current__" }, "跟随当前场次"));
					var assetStatus = el("span", { class: "coc-mini", role: "status" }, "");
					container.append(el("span", { class: "coc-net-asset-label" }, "剧本"), assetSel, assetStatus);
					assetSel.addEventListener("change", function () {
						NET_UI.asset = assetSel.value === "__current__" ? "" : assetSel.value;
						NET_UI.search = ""; NET_UI.scene = "all"; NET_UI.type = "all";
						NET_UI.selected = null; NET_UI.views = {}; NET_UI._layoutUser = false;
						renderNetPanel();
					});
					post("/coc-api/assets", { kind: "scenarios", action: "list" }).then(function (json) {
						if (json.ok) {
							...
							items.forEach(function (asset) {
								var statusText = asset.deepParseStatus === "confirmed" ? "已确认" : asset.deepParseStatus === "draft" ? "草稿" : "无解析";
								assetSel.append(el("option", { value: asset.id }, asset.name + "（" + statusText + "）"));
							});
							assetSel.value = currentAssetId || "__current__";
							assetStatus.textContent = items.length > 0 ? items.length + " 个剧本" : "暂无剧本资产";
						} else {
							assetStatus.textContent = "剧本列表加载失败";
						}
					});
					return assetSel;
				}
				function renderNetPanel() {
					...
					// 解析页有自己的剧本切换：NET_UI.asset 显式选择时优先；
					// 未选择时跟随当前场次剧本，场次无剧本再按场次读取。
					var requestedAsset = NET_UI.asset
						|| (S.digest && typeof S.digest.scenarioId === "string" && S.digest.scenarioId.trim() ? S.digest.scenarioId : "");
					if (box._dispose) box._dispose();
					box.replaceChildren(el("div", { class: "coc-empty", role: "status" }, "正在加载剧情网络…"));
					// 剧情网络与剧本资产绑定：按资产读取，同一剧本在不同场次看到同一张网络。
					var url = requestedAsset
						? "/coc-api/deep-parse?asset=" + encodeURIComponent(requestedAsset)
						: "/coc-api/deep-parse?game=" + encodeURIComponent(requestedGame);
					api(url).then(function (json) {
						...
						if (!data || !data.deepParse) {
							var emptyWrap = el("div", { class: "coc-net-empty-wrap" });
							var emptyToolbar = el("div", { class: "coc-net-toolbar" });
							renderNetAssetSelect(emptyToolbar, data && data.asset ? data.asset : "");
							var emptyMsg = el("div", { class: "coc-empty" }, data && data.assetName
								? "剧本「" + data.assetName + "」尚无深度解析。可前往调试 → 导入重新解析该剧本，或在上方切换其他剧本。"
								: "尚无深度解析。前往调试 → 导入，载入剧本后生成。");
							var importBtn = el("button", { type: "button" }, "前往导入");
							importBtn.addEventListener("click", function () { debugTab = "import"; showTab("debug"); });
							emptyWrap.append(emptyToolbar, emptyMsg, importBtn);
							box.replaceChildren(el("h3", null, "剧情网络"), emptyWrap, renderStructureCard(box));
							return;
						}
				...
				function createNetLayout(dp, base, overlay, mode) {
					var model = createNetModel(dp, base, overlay);
				...
				function renderNetStatus(parent, dp, meta) {
					...
					parent.append(el("span", { class: "coc-mini" }, meta.assetName
						? "来源：剧本资产「" + meta.assetName + "」"
						: "来源：" + (meta.source || "未知")));
				...
				function renderNetContent(box, dp, meta) {
					if (box._dispose) box._dispose();
					// 解析页独立的剧本切换：查看的资产与当前场次剧本一致时才叠加
					// 场次运行时摘要（revealed/reached 等），否则只看资产纯结构。
					var netAssetId = meta && meta.asset ? meta.asset : "";
					// overlay：仅“跟随当前场次”且场次剧本与查看资产一致时使用。
					var overlay = (!netAssetId || (S.digest && S.digest.scenarioId === netAssetId)) ? S.digest : null;
					// base：资产里解析时落盘的完整结构，与场次无关；资产无结构时回退场次副本。
					var base = (meta && Array.isArray(meta.assetKeyPoints))
						? { keyPoints: meta.assetKeyPoints, branches: meta.assetBranches || [] }
						: (overlay ? { keyPoints: overlay.keyPoints || [], branches: overlay.branches || [] } : { keyPoints: [], branches: [] });
					var hasAssetStructure = (Array.isArray(base.keyPoints) && base.keyPoints.length > 0) || (Array.isArray(base.branches) && base.branches.length > 0);
					var model = createNetModel(dp, base, overlay), cache = new Map(), view, camera, filterTimer;
					if (!NET_UI._layoutUser) NET_UI.layout = "skeleton";
					var header = el("div", { class: "coc-net-header" });
					header.append(el("h3", null, "剧情网络"));
					if (meta && meta.assetName) header.append(el("span", { class: "coc-badge", title: "剧情网络与剧本资产绑定，修改会写入资产并在同剧本场次间共享" }, "📖 " + meta.assetName));
					if (!hasAssetStructure) header.append(el("span", { class: "coc-mini", style: "color:#ffd54f;margin-left:8px" }, "资产暂无完整剧情结构，当前仅显示 deepParse 小网（参考）"));
					...
					// 剧本切换：不依赖场次下拉，直接在解析页切换剧本资产。
					renderNetAssetSelect(toolbar, netAssetId);
					var modeButtons = [];
					...
					// 剧本结构编辑写入当前场次，只在查看当前场次剧本时显示。
					if (overlay !== null) netDisclosure(inspector, "剧本结构编辑", function (content) { content.append(renderStructureCard(box)); });
					function draw() {
						var start = performance.now();
						if (camera) camera.dispose();
						if (!cache.has(NET_UI.layout)) cache.set(NET_UI.layout, createNetLayout(dp, base, overlay, NET_UI.layout));
				...
				function renderImportPanel() {
					...
					var targetInfo = el("div", { class: "coc-mini", style: "margin-bottom:8px;color:#8bc34a" });
					targetInfo.textContent = "目标场次：" + gameId() +
						(S.digest && S.digest.scenario ? "（已有剧本：" + S.digest.scenario.name + "）" : "（当前无剧本）") +
						"；剧本会写入当前场次，需要独立场次请先“新建场次”。";
					card.append(targetInfo);
				...
				card.append(btnRow);
					runBtn.addEventListener("click", function () {
						...
						if ((kind === "scenario" || kind === "auto") && S.digest && S.digest.scenario) {
							var existingName = S.digest.scenario.name || "未命名";
							if (!confirm("当前场次已有剧本「" + existingName + "」。继续导入会把内容写入当前场次，可能追加或覆盖已有剧情结构。\n\n如果这是另一个剧本，建议先「新建场次」再导入。\n\n仍要导入到当前场次？")) return;
						}
