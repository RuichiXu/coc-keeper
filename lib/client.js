			...
			function mountPanel() {
				...
				function renderQualityIssues(box, q) {
					...
					toggle.addEventListener("click", function () {
						...
						box.replaceChildren(); renderQualityIssues(box, q);
					});
					...
					[["all", "全部"], ["high", "high"], ["medium", "medium"], ["low", "low"]].forEach(function (pair) {
						...
						btn.addEventListener("click", function () { NET_UI.issueSeverity = pair[0]; box.replaceChildren(); renderQualityIssues(box, q); });
				...
				function renderNetStatus(parent, dp, meta) {
					...
					var q = dp.quality || {};
					[["preflight", "结构"], ["rule", "规则"], ["review", "审校"], ["chunk", "分块"]].forEach(function (pair) {
				...
				function renderNetContent(box, dp, meta) {
					...
					netDisclosure(inspector, "质量统计与审校报告", function (content) { renderNetStatus(content, dp, meta); renderQualityIssues(content, dp.quality || {}); });
