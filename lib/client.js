...
var S = { digest: null, entries: [], seq: 0, tab: tabPref(), busy: false, error: "", importView: null };
			...
			function mountPanel() {
				...
				function buildSkeletonView(nodeById, edgeList, ui) {
					// 骨架总览：左列主线（strict）→ 中列虚拟枢纽（hub）+ 辐条（spoke）→ 右列汇聚节点。
					// 只画 hub→首辐条（双向箭头表示可返回）、辐条列内边、返回点 / 末辐条 → 汇聚的边，其余边隐藏。
					function orderOf(node) { var o = Number(node && node.order); return isFinite(o) ? o : 1e9; }
					...
					var strictNodes = pool.filter(function (n) { return n.reachability === "strict" && n.virtual !== true; }).sort(byOrder);
					...
					// 5. 每个 hub 的辐条：plotEdges 中 from=kp:hub.id 且 to 是非虚拟 kp，按 order 升序；
					//    过滤返回点（virtual / -return 结尾），并按 hub 顺序先去重认领，避免辐条重复入列。
					...
					// 汇聚节点：有枢纽时取 strict 中第一个不被任何 hub 连接的节点，否则 kp-hub-final，否则临时虚拟节点；
					// 无枢纽（退化三层）时取主线最后一个 strict（故事终点），避免把开场节点当汇聚造成倒流。
					...
					if (hubNodes.length === 0) {
						convergence = strictNodes.length > 0 ? strictNodes[strictNodes.length - 1] : pseudoConv();
					} else if (freeStrict.length > 0) {
						convergence = freeStrict[0];
					} else {
						for (var fi = 0; fi < hubNodes.length; fi += 1) {
							if (String(hubNodes[fi].id) === "kp-hub-final") {
								convergence = hubNodes.splice(fi, 1)[0];
								// kp-hub-final 提升为汇聚后重算认领，它原来的辐条回归主线/隐藏
								claim = claimSpokes(hubNodes);
								spokesByHub = claim.byHub;
								spokeSet = claim.claimed;
								break;
							}
						}
						if (convergence === null) convergence = pseudoConv();
					}
					// 主线列：strict 且未充当辐条 / 汇聚节点（每节点只放一处）
					var mainNodes = strictNodes.filter(function (n) { return n !== convergence && !spokeSet.has(n); });
					...
					function pushConvEdge(from) {
						var edge = findPlotEdge(from, convergence);
						if (edge === null) edge = { from: from, to: convergence, label: "", requires: [], kind: "skel", fallback: true };
						skelEdges.push(edge);
					}

					// 6. 布局坐标与边筛选
					mainNodes.forEach(function (node, i) { place(node, 80, 40 + i * 56); });
					var hubCount = hubNodes.length;
					var convX;
					if (hubCount > 0) {
						hubNodes.forEach(function (hub, hi) {
							var colX = 260 + hi * 170;
							// hub 放在本列第一个辐条前面
							place(hub, colX, 40);
							var spokes = spokesByHub.get(hub);
							var rets = returnsByHub.get(hub) || [];
							spokes.forEach(function (s, si) { place(s, colX, 40 + 52 * (si + 1)); });
							rets.forEach(function (r, ri) { place(r, colX, 40 + 52 * (spokes.length + 1) + 8 + ri * 40); });
							// 只画 hub → 第一个辐条，双向箭头表示可返回
							if (spokes.length > 0) {
								var e0 = findPlotEdge(hub, spokes[0]);
								if (e0 === null) e0 = { from: hub, to: spokes[0], label: "", requires: [], kind: "skel", fallback: true };
								e0._bidir = true;
								skelEdges.push(e0);
							}
							// 辐条列内的原始边（普通 / fallback、含辐条→返回点），浅色绘制
							var spokeIdx = new Set(spokes);
							edgeList.forEach(function (edge) {
								if (edge.kind !== "edge") return;
								if (!spokeIdx.has(edge.from)) return;
								if (spokeIdx.has(edge.to) || rets.indexOf(edge.to) >= 0) {
									edge._inner = true;
									skelEdges.push(edge);
								}
							});
							// 末辐条与本 hub 返回点 → 汇聚
							if (spokes.length > 0) pushConvEdge(spokes[spokes.length - 1]);
							rets.forEach(function (r) { pushConvEdge(r); });
						});
						convX = 260 + hubCount * 170 + 90;
						place(convergence, convX, 40);
						orphanReturns.forEach(function (r, ri) { place(r, convX, 40 + (ri + 1) * 40); pushConvEdge(r); });
					} else {
						// 7. hubCount 为 0：退化为 主线 / 支线（conditional）/ 汇聚 三层
						var others = pool.filter(function (n) {
							return n.virtual !== true && n.reachability !== "strict" && n !== convergence;
						}).sort(byOrder);
						others.forEach(function (node, i) { place(node, 260, 40 + i * 52); });
						convX = 520;
						place(convergence, convX, 40);
						returnNodes.forEach(function (r, ri) { place(r, convX, 40 + (ri + 1) * 40); });
						var placedSet = new Set(visible);
						edgeList.forEach(function (edge) {
							if (placedSet.has(edge.from) && placedSet.has(edge.to)) skelEdges.push(edge);
						});
						var hasConvIn = new Set();
						skelEdges.forEach(function (e) { if (e.to === convergence) hasConvIn.add(e.from); });
						others.forEach(function (n) { if (!hasConvIn.has(n)) pushConvEdge(n); });
						returnNodes.forEach(function (r) { if (!hasConvIn.has(r)) pushConvEdge(r); });
						if (mainNodes.length > 0) {
							var lastMain = mainNodes[mainNodes.length - 1];
							if (!hasConvIn.has(lastMain)) pushConvEdge(lastMain);
						}
					}
					return {
						visible: visible,
						visEdges: skelEdges,
						svgWidth: Math.max(720, convX + 200),
						svgHeight: Math.max(240, maxY + 110),
						auxNodes: auxNodes,
					};
				}
				...
				function renderNetContent(box, dp, meta) {
					...
					// 默认布局按关键点规模选择：kp 节点 > 60 时进骨架总览，否则保持场景视图。
					// 用户手动点布局按钮后（_layoutUser）不再自动切换；数据未就绪时下一次渲染会重算。
					if (NET_UI._layoutUser !== true) {
						var kpCountSet = new Set();
						digestKeyPoints().forEach(function (kp) { if (kp && kp.id) kpCountSet.add(String(kp.id)); });
						(dp.keyPoints || []).forEach(function (kp) { if (kp && kp.id) kpCountSet.add(String(kp.id)); });
						NET_UI.layout = kpCountSet.size > 60 ? "skeleton" : "scene";
					}
					...
					var skelModeBtn = el("button", { type: "button" }, "骨架");
					...
					flowModeBtn.addEventListener("click", function () { NET_UI.layout = "flow"; NET_UI._layoutUser = true; renderNetContent(box, dp, meta); });
					skelModeBtn.addEventListener("click", function () { NET_UI.layout = "skeleton"; NET_UI._layoutUser = true; renderNetContent(box, dp, meta); });
					toolbar.append(sceneModeBtn, flowModeBtn, skelModeBtn);
					...
					if (NET_UI.layout === "scene") {
						...
						toolbar.append(el("span", { class: "coc-mini" }, "骨架总览：左主线 · 中枢纽/辐条 · 右汇聚"));
					} else {
					...
					digestKeyPoints().forEach(function (kp) { addNode("kp", kp.id, kp.title, { scene: kp.scene, desc: kp.desc, revealed: kp.revealed === true, virtual: kp.virtual === true, reachability: kp.reachability, order: kp.order, flowRole: kp.flowRole, kind: kp.kind, parentId: kp.parentId }); });
					(dp.keyPoints || []).forEach(function (kp) { addNode("kp", kp.id, kp.title, { scene: kp.scene, desc: kp.desc, revealed: false, virtual: kp.virtual === true, reachability: kp.reachability, order: kp.order, flowRole: kp.flowRole, kind: kp.kind, parentId: kp.parentId }); });
					...
					if (flowMode) {
						...
						sceneOrder.forEach(function (scene) {
							...
							yCursor += BAND_TITLE_H + rows * NODE_SLOT_H + 10;
						});
						svgWidth = Math.max(720, 64 + MAX_PER_ROW * NODE_SLOT_W + 64);
						// 高度封顶 720px，超出部分靠视口平移/缩放查看
						svgHeight = Math.min(720, Math.max(240, yCursor + 120));
					}
					...
					graphCard.append(el("h4", null, (flowMode ? "演进拓扑" : skeletonMode ? "骨架总览" : "网络拓扑") + (foldedCount > 0 ? "（已折叠 " + foldedCount + " 个检定分支）" : "") + " · " + visible.length + " 节点 / " + visEdges.length + " 边"));
					...
					if (skeletonMode) legend.append(el("span", null, "双箭头=枢纽可往返 · 淡色细线=辐条列内边 · 淡虚线=推断汇聚边"));
					...
					var showEdgeLabels;
					if (bigNet) showEdgeLabels = false;
					...
					visEdges.forEach(function (edge) {
						...
						var y2 = edge.to._y;
						var classes = "edge" + (Array.isArray(edge.requires) && edge.requires.length > 0 ? " has-req" : "") + (edge.fallback === true ? " fallback" : "") + (edge._inner === true ? " skel-inner" : "");
					...
					visible.forEach(function (node) {
						var group = svgEl("g", { class: "node" + (node.virtual === true ? " virtual" : ""), transform: "translate(" + node._x + "," + node._y + ")", "data-id": String(node.id), "data-type": node.type });
						var shape;
						if (node.type === "kp") {
