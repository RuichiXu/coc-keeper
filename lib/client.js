	...
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			var STYLE = [
				...
				"#coc-keeper-panel .coc-net-svg{width:100%;height:auto;min-height:240px;border:1px solid rgba(120,150,215,.25);border-radius:11px;background:radial-gradient(circle at 20% 15%,rgba(72,104,176,.14),rgba(6,12,30,.35) 65%)}",
				...
				"#coc-keeper-panel .coc-net-svg .edge{stroke:rgba(140,170,230,.5);stroke-width:1.3;fill:none}",
			...
			function mountPanel() {
				...
				var NET_UI = { search: "", type: "all", scene: "all", foldChecks: true, zoom: 1, selected: null, focus: "final" };
				...
				function fmtCondChips(parent, cond, nodeById) {
					...
					function chip(text, cls) { parent.append(el("span", { class: "coc-cond" + (cls ? " " + cls : "") }, text)); }
					if (cond.scene) chip("📍 场景=" + cond.scene);
					if (Array.isArray(cond.entryEvidence) && cond.entryEvidence.length > 0) chip("🔎 文本证据：" + cond.entryEvidence.join(" / "));
				...
				function renderNetContent(box, dp, meta) {
					...
					var scenes = [];
					digestKeyPoints().forEach(function (kp) { if (kp.scene && scenes.indexOf(kp.scene) < 0) scenes.push(kp.scene); });
					(dp.keyPoints || []).forEach(function (kp) { if (kp.scene && scenes.indexOf(kp.scene) < 0) scenes.push(kp.scene); });
					(dp.branches || []).forEach(function (b) { if (b.scene && scenes.indexOf(b.scene) < 0) scenes.push(b.scene); });
					...
					digestKeyPoints().forEach(function (kp) { addNode("kp", kp.id, kp.title, { scene: kp.scene, desc: kp.desc, revealed: kp.revealed === true }); });
					(dp.keyPoints || []).forEach(function (kp) { addNode("kp", kp.id, kp.title, { scene: kp.scene, desc: kp.desc, revealed: false }); });
					digestBranches().forEach(function (b) { addNode("br", b.id, b.title, { scene: b.scene, desc: b.desc, options: b.options || [], reached: b.reached === true, checkpointBranch: b.checkpointBranch === true }); });
					(dp.branches || []).forEach(function (b) { addNode("br", b.id, b.title, { scene: b.scene, desc: b.desc, options: b.options || [], reached: false, checkpointBranch: b.checkpointBranch === true, finalChoice: b.finalChoice === true }); });
					...
					(dp.plotEdges || []).forEach(function (edge) {
						var from = netResolveNode(nodeById, edge.from);
						var to = netResolveNode(nodeById, edge.to);
						if (from === null || to === null) return;
						edgeList.push({ from: from, to: to, label: edge.label || "", requires: edge.requires || [], kind: "edge" });
					});
					digestBranches().concat(dp.branches || []).forEach(function (branch) {
						(branch.options || []).forEach(function (option) {
							...
							var dup = edgeList.some(function (e) { return e.kind === "edge" && e.from === from && e.to === to; });
							if (!dup) edgeList.push({ from: from, to: to, label: option.label || "", requires: [], kind: "leads" });
						});
					});
					...
					nodeById.forEach(function (node) {
						...
						if (NET_UI.focus === "final") {
							var isFinal = node.type === "end" || (node.type === "br" && (node.finalChoice === true || String(node.id).indexOf("br-final") === 0));
							if (!isFinal) return;
						}
						visible.push(node);
					});
					...
					// 布局：按场景分「横向条带」，条带内节点横向排布、超宽自动换行；
					// 条带纵向堆叠，适配面板窄宽度，避免几十列被压缩成一条线。
					...
					var sceneOrder = Array.from(sceneNodeMap.keys());
					var BAND_TITLE_H = 18;
					var NODE_SLOT_W = 112;
					var NODE_SLOT_H = 50;
					var MAX_PER_ROW = 5;
					...
					var svgWidth = 720;
					var svgHeight = NET_UI.focus === "final" ? Math.max(160, yCursor + 30) : Math.max(200, yCursor + 46);

					var graphCard = el("div", { class: "coc-card" });
					graphCard.append(el("h4", null, "网络拓扑" + (foldedCount > 0 ? "（已折叠 " + foldedCount + " 个检定分支）" : "") + " · " + visible.length + " 节点 / " + visEdges.length + " 边"));
					...
					var svg = svgEl("svg", { class: "coc-net-svg", viewBox: "0 0 " + svgWidth + " " + svgHeight, preserveAspectRatio: "xMidYMin meet", width: "100%", height: String(svgHeight) });
					var defs = svgEl("defs");
					...
					visEdges.forEach(function (edge) {
						...
						var y2 = edge.to._y;
						var classes = "edge" + (Array.isArray(edge.requires) && edge.requires.length > 0 ? " has-req" : "");
						...
						if (edge.label) {
							...
							if (edge.label !== targetTitle && edge.label !== sourceTitle) {
								...
								label.textContent = shortLabel(edge.label, "");
								svg.append(label);
							}
						}
					});
					...
					visible.forEach(function (node) {
						var group = svgEl("g", { class: "node", transform: "translate(" + node._x + "," + node._y + ")" });
						var shape;
						if (node.type === "kp") {
						...
						group.append(shape);
						var label = svgEl("text", { y: "28", "text-anchor": "middle" });
						label.textContent = shortLabel(node.title, node.id);
						group.append(label);
						var title = svgEl("title");
						title.textContent = node.type.toUpperCase() + " " + node.id + " · " + node.title + (node.scene ? " @" + node.scene : "");
						group.append(title);
						group.addEventListener("click", function () { renderNetNodeDetail(graphCard, node, nodeById, edgeList); });
						svg.append(group);
					});

					graphCard.append(svg);
					box.append(graphCard);

					var firstEnding = (dp.endings || [])[0];
					if (firstEnding) {
						var firstNode = netResolveNode(nodeById, "end:" + firstEnding.id);
						if (firstNode !== null) renderNetNodeDetail(graphCard, firstNode, nodeById, edgeList);
					}
				}
				function renderNetNodeDetail(container, node, nodeById, edgeList) {
