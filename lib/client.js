...
var S = { digest: null, entries: [], seq: 0, tab: tabPref(), busy: false, error: "", importView: null };
			...
			function mountPanel() {
				...
				function buildSkeletonView(nodeById, edgeList, ui) {
					...
					// 6. 主线中轴：freeStrict + 虚拟枢纽 按剧情 order 混排，从左向右。
					...
					// 7. 放置主线中轴（从左向右）并串主线。
					var mainStep = 150;
					spineItems.forEach(function (item, idx) { place(item, 70 + idx * mainStep, mainLaneY); });
					for (var si = 0; si < spineItems.length - 1; si += 1) {
						pushEdge(makeEdge(spineItems[si], spineItems[si + 1]));
					}

					// 8. 每个枢纽：向上/下分出并行车道，车道从左向右走，最后汇入该枢纽之后的下一个主线节点。
					var finalConv = null;
				hubNodes.forEach(function (hub) {
						var spokes = spokesByHub.get(hub) || [];
						if (spokes.length === 0) return;
						var rets = returnsByHub.get(hub) || [];
						var conv = nextMainAfter(hub);
						if (conv === null) {
							if (finalConv === null) finalConv = pseudoConv();
							conv = finalConv;
						}
						var laneCount = spokes.length;
						var spacing = laneCount > 6 ? 52 : 64;
						var laneTop = mainLaneY - ((laneCount - 1) * spacing) / 2;
						var baseX = hub._x + 130;
						spokes.forEach(function (first, si) {
							var laneY = laneTop + si * spacing;
							var lane = chainLane(first);
							lane.forEach(function (n, ni) { place(n, baseX + ni * 130, laneY); });
							var firstEdge = makeEdge(hub, lane[0]);
							...
							var retsForLane = (si === 0) ? rets : [];
							if (retsForLane.length > 0) {
								retsForLane.forEach(function (r, ri) {
									place(r, baseX + lane.length * 130 + 40, laneY + ri * 34 - 34);
									var toRet = makeEdge(last, r);
									toRet._inner = true;
									pushEdge(toRet);
									pushEdge(makeEdge(r, conv));
								});
							} else {
								pushEdge(makeEdge(last, conv));
							}
						});
					});
					if (finalConv !== null) {
						place(finalConv, maxX + 220, mainLaneY);
						var lastSpine = spineItems[spineItems.length - 1];
						if (lastSpine !== undefined && lastSpine !== finalConv) pushEdge(makeEdge(lastSpine, finalConv));
						orphanReturns.forEach(function (r, ri) { place(r, finalConv._x - 90, mainLaneY + 40 + ri * 40); pushEdge(makeEdge(r, finalConv)); });
					} else {
						orphanReturns.forEach(function (r, ri) { place(r, maxX + 150, mainLaneY + 40 + ri * 40); });
					}

					return {
						visible: visible,
						visEdges: skelEdges,
						svgWidth: Math.max(760, maxX + 280),
						svgHeight: Math.max(360, maxY + 140),
						auxNodes: auxNodes,
					};
				}
