...
`
			...
			function mountPanel() {
				...
				function renderNetContent(box, dp, meta) {
					...
					var netDigest = (!netAssetId || (S.digest && S.digest.scenarioId === netAssetId)) ? S.digest : null;
					// 场次不存在或查看的是其他资产时，用资产自带结构作为图模型输入，
					// 保证剧情网络展示完整 keyPoints/branches（而非仅 deepParse 小网）。
					var assetDigest = (meta && Array.isArray(meta.assetKeyPoints))
						? { keyPoints: meta.assetKeyPoints, branches: meta.assetBranches || [], scenarioId: netAssetId }
						: null;
					var modelDigest = netDigest !== null ? netDigest : assetDigest;
					var model = createNetModel(dp, modelDigest), cache = new Map(), view, camera, filterTimer;
					...
					if (netDigest !== null) netDisclosure(inspector, "剧本结构编辑", function (content) { content.append(renderStructureCard(box)); });
					function draw() {
						var start = performance.now();
						if (camera) camera.dispose();
						if (!cache.has(NET_UI.layout)) cache.set(NET_UI.layout, createNetLayout(dp, modelDigest, NET_UI.layout));
