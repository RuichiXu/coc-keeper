	...
	window.__ModuleLoader__.load({
		...
		factory: function (require) {
			...
			var STYLE = [
				...
				"#coc-keeper-panel .coc-net-svg{display:block;width:100%;height:auto;min-height:240px;border:1px solid rgba(120,150,215,.25);border-radius:11px;background:radial-gradient(circle at 20% 15%,rgba(72,104,176,.14),rgba(6,12,30,.35) 65%)}",
			...
			function mountPanel() {
				...
				function renderNetContent(box, dp, meta) {
					...
					function addEdge(kind, from, to, label, requires) {
						if (from === null || to === null || from === to) return false;
						var key = kind + "|" + netNodeId(from.type, from.id) + "|" + netNodeId(to.type, to.id) + "|" + (label || "");
						if (edgeKeys.has(key)) return false;
						edgeKeys.add(key);
						edgeList.push({ from: from, to: to, label: label || "", requires: requires || [], kind: kind });
						return true;
					}
					(dp.plotEdges || []).forEach(function (edge) {
						addEdge("edge", netResolveNode(nodeById, edge.from), netResolveNode(nodeById, edge.to), edge.label || "", edge.requires || []);
					});
					...
					var svgWidth = 720;
					var svgHeight = NET_UI.focus === "final" ? Math.max(200, yCursor + 50) : Math.max(240, yCursor + 120);
					...
					var showEdgeLabels = NET_UI.focus === "final" || visEdges.length <= 24;
