...
var S = { digest: null, entries: [], seq: 0, tab: tabPref(), busy: false, error: "", importView: null };
			...
			function mountPanel() {
				...
				var NET_UI = { search: "", type: "all", scene: "all", foldChecks: true, zoom: 1, selected: null, focus: "final", issueSeverity: "all", layout: "scene", showReturnEdges: false };
				...
				function buildSkeletonView(nodeById, edgeList, ui) {
					...
					for (var si = 0; si < placedMain.length - 1; si += 1) {
						...
						pushEdge(makeEdge(from, placedMain[si + 1]));
					}
				...
				function renderNetContent(box, dp, meta) {
					...
					svg.addEventListener("mouseover", function (ev) {
						...
						Array.prototype.forEach.call(svg.querySelectorAll(".edge"), function (p) {
							...
							var on = focusKeys.has(f) && focusKeys.has(t);
					...
					visEdges.forEach(function (edge) {
						...
						var y2 = edge.to._y;
						var classes = "edge" + (Array.isArray(edge.requires) && edge.requires.length > 0 ? " has-req" : "") + (edge.fallback === true ? " fallback" : "") + (edge._inner === true ? " skel-inner" : "") + (edge._return === true ? " return" : "");
						var edgeD = flowBig
							? "M " + x1 + " " + y1 + " L " + x2 + " " + y2
