...
var S = { digest: null, entries: [], seq: 0, tab: tabPref(), busy: false, error: "", importView: null };
			...
			function mountPanel() {
				...
				function buildSkeletonView(nodeById, edgeList, ui) {
					...
					var laneSpacing = maxSpokes > 8 ? 48 : 60;
					...
					spineItems.forEach(function (item) {
						if (item.virtual === true) {
							...
							spokes.forEach(function (first, si) {
								...
								lane.forEach(function (n, ni) { place(n, baseX + ni * 130, laneY); fanRight = Math.max(fanRight, baseX + ni * 130); });
							...
							// 返回点：每个 hub 最多展示一个返回点（多返回点竖排），避免虚拟节点堆成一片。
							if (rets.length > 0) {
								var retX = fanRight + 50;
								rets.forEach(function (r, ri) {
									place(r, retX, mainLaneY + (ri - (rets.length - 1) / 2) * 64);
									lastPerLane.forEach(function (last) { var e = makeEdge(last, r); e._inner = true; pushEdge(e); });
							...
							xCursor = Math.max(xCursor + 160, fanRight + 110);
						} else {
							place(item, xCursor, mainLaneY);
							placedMain.push(item);
							xCursor += 150;
					...
					// 10. 无主返回点：放到右侧空白处，避免叠在主线。
					if (orphanReturns.length > 0) {
						var orX = Math.max(xCursor, maxX) + 160;
						orphanReturns.forEach(function (r, ri) { place(r, orX, 60 + ri * 48); });
					}
