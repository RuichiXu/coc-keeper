...
export function settlementMatchRules(trigger, kind) {
  ...
  if (kind === "san") {
    ...
    if (/(?:踏出遗迹|离开矿洞|来到洞口|洞口|外部控制室)/.test(source)) {
      // 外出/抵达阶段：anyOf 只放实际移动或抵达动词，context 只放目的地，
      // 二者必须同句；此外标记 arrival——当当前场景已抵达该目的地时，
      // 由场景落地确定性触发，不再依赖“动作词+目的地”同句。
      // “把沸核发射出去”这类把“出去”当补语的句子不含移动动词，
      // 不应触发（Codex r5 第 30 轮）；“寻找外部控制室的安全到达路线”
      // 里的“到达”是路线名词，不列入触发词（Codex r6 第 25 轮）。
      return {
        anyOf: ["踏出遗迹", "离开矿洞", "来到洞口", "走出", "来到", "站到"],
        context: ["外部控制室", "洞口", "矿洞", "岩台", "山脉侧翼"],
        arrival: true,
      };
    }
  }
  ...
  if (kind === "hp") {
    // 蒸汽/热浪是弱语境；“崩/裂/爆/炸/碎/分崩离析/零件/充斥”
    // 才表示装置真的损坏。仅“泄压阀室的门，热浪扑面”不扣血。
    // “喷/涌”是强语境但会误伤“沸核随时会提前失控喷发”这类未来描述，
    // 因此列入 futureMarkers 排除：出现未来/条件标记且没有破坏词时不算发生。
    return {
      anyOf: ["泄压阀", "圆盘", "转盘", "分崩离析", "零件", "蒸汽", "烟雾", "充斥"],
      context: ["蒸汽", "热浪", "烟雾", "零件", "崩", "烫", "灼", "扑面", "涌来", "喷", "充斥"],
      strongContext: ["崩", "裂", "爆", "炸", "碎", "喷", "涌", "分崩离析", "零件", "充斥"],
      futureMarkers: ["随时", "将会", "将要", "即将", "可能", "迟早", "早晚", "会提前", "一旦", "万一", "如果", "若是"],
      destructiveWords: ["崩", "裂", "爆", "炸", "碎", "分崩离析"],
    };
  }
 ...
 * @returns {string[]}
 */
// 通用地点后缀：用于从“泄压阀控制室”这类场景名里剥离出有区分度的
// 短词“泄压阀”，使场景约束在叙述只写短名（“泄压阀”）时仍能命中。
// 单字“室”放最后，保证先匹配“控制室”等更长的后缀。
const GENERIC_LOCATION_SUFFIXES = [
  "控制室", "控制台", "档案馆", "档案室", "办公室", "大厅",
  "旋梯", "拱道", "通道", "岔道", "走廊", "楼层", "舱", "室",
];

/**
 * 从场景名中剥离通用地点后缀，得到有区分度的短词。
 * @param {string} name
 * @returns {string}
 */
export function distinctiveLocationName(name) {
  const source = String(name ?? "").trim();
  if (source.length < 3) return source;
  for (const suffix of GENERIC_LOCATION_SUFFIXES) {
    if (source.endsWith(suffix) && source.length - suffix.length >= 2) {
      const base = source.slice(0, source.length - suffix.length);
      // 剥离后只剩“外部/内部”这类方位词时不作为区分词。
...
export function sceneTokensFor(scene) {
  ...
  if (name !== null && !/(?:这个|眼前|这是)/.test(name[1])) {
    tokens.push(name[1]);
    const distinctive = distinctiveLocationName(name[1]);
    if (distinctive !== name[1] && distinctive.length >= 2) tokens.push(distinctive);
  }
...
export function settlementMatches(settlement, text, currentScene = "") {
  ...
  const strongContext = rules?.strongContext ?? [];
  const futureMarkers = rules?.futureMarkers ?? [];
  const destructiveWords = rules?.destructiveWords ?? [];

  // 到达型结算：当前场景已抵达结算点场景时视为事件发生。
  // 场景落地是确定性状态变化，比“叙述里动作词+目的地同句”更可靠；
  // 反过来，仅讨论“把沸核发射出去”不会命中（anyOf 无移动动词）。
  if (
    rules?.arrival === true &&
    sceneTokens.length > 0 &&
    sceneTokens.some((token) => cur.includes(token))
  ) {
    return true;
  }
  ...
  if (context.length > 0) {
    ...
    // hp 类规则额外要求强破坏词（崩/裂/爆/喷/涌…），避免“热浪扑面”误扣血。
    ...
    for (const sentence of sentences) {
      ...
      const strongOk = strongContext.length === 0 || strongContext.some((word) => sentence.includes(word));
      if (!anyOk || !ctxOk || !strongOk) continue;
      // 未来/条件语气（“沸核随时会提前失控喷发”）不是已发生事件；
      // 但句中同时出现“崩/裂/爆/炸/碎/分崩离析”等破坏词时仍视为发生。
      const futureOnly =
        futureMarkers.length > 0 &&
        futureMarkers.some((word) => sentence.includes(word)) &&
        destructiveWords.length > 0 &&
        !destructiveWords.some((word) => sentence.includes(word));
