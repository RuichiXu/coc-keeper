...
export function applyTopologySkeleton(flat, deepParse) {
  ...
  for (const [parentKey, items] of groups.entries()) {
    ...
    // parentKey 可能是 section id（结构分组）或 kp id（未分配场景关键点按 kp.parentId 分组）。
    const parentKp = parentKey !== "root" ? (kpBySectionId.get(parentKey) ?? kpById.get(parentKey)) : undefined;
