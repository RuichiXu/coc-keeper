    ...
    if (isAuto) {
      ...
      if (asArray(branch?.options).length === 0 && brEndings.length > 0) {
        branch.options = brEndings.map((ending) => ({
          label: nonEmptyString(ending?.requires?.optionLabel) || nonEmptyString(ending?.optionLabel) || nonEmptyString(ending?.title) || "继续",
          leadsTo: `end:${String(ending.id)}`,
        }));
        repaired += 1;
      }
      // 2) finalChoice 置真。
