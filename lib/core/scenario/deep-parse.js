 ...
 */
function coerceConditionShape(cond) {
  if (cond === null || cond === undefined || typeof cond !== "object" || Array.isArray(cond)) return cond;
  const next = { ...cond };
  if (Array.isArray(next.checkpointGroups)) {
    next.checkpointGroups = next.checkpointGroups
      .map((group) => (Array.isArray(group) ? group : [group]))
      .filter((group) => group.length > 0);
  }
  if (next.not !== undefined && next.not !== null && typeof next.not === "object" && !Array.isArray(next.not)) {
    next.not = coerceConditionShape(next.not);
  }
  return next;
}

function coerceEndingShape(ending) {
  if (ending === null || ending === undefined || typeof ending !== "object") return ending;
  const next = { ...ending };
  next.requires = coerceConditionShape(next.requires);
  next.blockers = asArray(next.blockers).map((blocker) => {
    if (typeof blocker === "string") return { entryEvidence: [blocker] };
    return coerceConditionShape(blocker);
  });
  return next;
}
...
export function parseSkeletonWiringResult(rawText, flat) {
  ...
  if (parsed.deepParse === null) return parsed;
  {
    ...
    parsed.deepParse = normalizeDeepParse({
      ...
      keyPointConditions: asArray(parsed.deepParse.keyPointConditions).map((entry) => {
        const next = {
          ...entry,
          requires: coerceConditionShape(entry?.requires),
          requiresAnyOf: asArray(entry?.requiresAnyOf).map((group) => asArray(group).map(coerceConditionShape)),
        };
        if (next.requiresAnyOf.length === 0) delete next.requiresAnyOf;
        return next;
      }),
      branchConditions: asArray(parsed.deepParse.branchConditions).map((entry) => {
        const next = {
          ...entry,
          requires: coerceConditionShape(entry?.requires),
          requiresAnyOf: asArray(entry?.requiresAnyOf).map((group) => asArray(group).map(coerceConditionShape)),
      ...
      plotEdges: asArray(parsed.deepParse.plotEdges).map((edge) => {
        if (Array.isArray(edge?.requires)) return { ...edge, requires: { entryEvidence: edge.requires.map((item) => String(item)) } };
        return { ...edge, requires: coerceConditionShape(edge?.requires) };
      }),
      endings: asArray(parsed.deepParse.endings).map(coerceEndingShape),
    });
