...
export function repairSkeletonWiringDeepParse(deepParse, flat) {
  ...
  for (const branchId of finalChoiceIds) {
    ...
    const entry = asArray(dp.branchConditions).find((cond) => String(cond?.branchId ?? "") === branchId);
    if (entry === undefined) {
      if (branch !== undefined) {
    ...
    const hasScene =
      ...
      nonEmptyString(entry.requires.scene).length > 0;
    const anyScene = asArray(entry.requiresAnyOf).some(
      (group) => group !== null && group !== undefined && typeof group === "object" && !Array.isArray(group) && nonEmptyString(group.scene).length > 0
    );
    if (!hasScene && !anyScene) {
      const scene = nonEmptyString(branch?.scene);
      if (scene.length > 0) {
        const requires = entry.requires !== undefined && entry.requires !== null && typeof entry.requires === "object" && !Array.isArray(entry.requires)
          ? entry.requires
          : {};
        entry.requires = { scene, ...requires };
