...
export function buildSkeletonWiringPrompt(flat) {
  ...
  // 追加“玩家选择型最终分支”提取结果，并把 id 重整进骨架（避免冲突）。
  {
    const finalChoice = extractFinalChoiceBranches(flat);
    let kpIndex = grounded.keyPoints.length + 1;
    let brIndex = grounded.branches.length + 1;
    for (const kp of finalChoice.keyPoints) {
      grounded.keyPoints.push({ ...kp, id: `kp-${kpIndex}` });
      kpIndex += 1;
    }
    for (const branch of finalChoice.branches) {
      grounded.branches.push({ ...branch, id: `br-${brIndex}` });
      brIndex += 1;
    }
  }
