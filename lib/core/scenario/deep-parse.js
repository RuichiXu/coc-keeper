...
export function repairSkeletonWiringDeepParse(deepParse, flat) {
  ...
  for (const branchId of finalChoiceIds) {
    ...
    // 优先取已有 scene；没有则用骨架分支 scene；再没有保持空对象交给 preflight。
    const existingScene =
    ...
    const scene = existingScene.length > 0 ? existingScene : anyScene.length > 0 ? anyScene : nonEmptyString(branch?.scene);
    if (scene.length > 0) {
      ...
      } else if (Object.keys(entry.requires).length === 1) {
        // 已有 scene，但把同条件里的 keyPointIds/not 等额外键剥离，防止
        // “kp-17 与 not kp-17 互斥门控”把最终分支锁死。
