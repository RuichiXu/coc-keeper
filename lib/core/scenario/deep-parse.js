...
export function buildDeepParsePrompt(flat) {
  ...
  return [
    ...
    `  "endings": [{"id":"end-1","branchId":"br-1","title":"墨渊消散的结局","optionLabel":"逆序念诵（送神）","requires":{"keyPointIds":["kp-7"],"branchChoiceIds":["br-1"]},"blockers":[{"branchChoiceIds":["br-1"]}],"endingKeywords":["墨渊消散","送神"]}]`,
    `}`,
    ``,
    `字段说明与约束：`,
...
export function applyConfirmedDeepParse(flat) {
  ...
  for (const entry of asArray(deepParse?.keyPointConditions)) {
    ...
    if (entry?.requires !== undefined) kp.requires = { ...entry.requires };
    if (entry?.requiresAnyOf !== undefined) {
      kp.requiresAnyOf = asArray(entry.requiresAnyOf).map((group) => ({ ...group }));
    }
  ...
  for (const entry of asArray(deepParse?.branchConditions)) {
    ...
    if (entry?.requires !== undefined) branch.requires = { ...entry.requires };
    if (entry?.requiresAnyOf !== undefined) {
      branch.requiresAnyOf = asArray(entry.requiresAnyOf).map((group) => ({ ...group }));
    }
    if (entry?.autoChooseLabel !== undefined && nonEmptyString(entry.autoChooseLabel).length > 0) {
...
export function syncPlotGraphFromDeepParse(plot, deepParse, story) {
  const synced = plot.syncFromStory(story ?? { keyPoints: [], branches: [] });
