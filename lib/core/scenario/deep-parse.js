...
export function buildSkeletonWiringPrompt(flat) {
  ...
  return [
    ...
    ``,
    `约束：`,
    `- 不要生成 keyPoints / branches 字段；keyPointConditions.keyPointId 与 branchConditions.branchId 必须来自下方骨架。`,
    ...
    `- endings[].branchId 必须来自骨架分支；optionLabel 必须等于该分支某个选项 label；`,
...
export function parseSkeletonWiringResult(rawText, flat) {
  ...
  if (asArray(parsed.deepParse.keyPoints).length > 0 || asArray(parsed.deepParse.branches).length > 0) {
    parsed.issues.push("骨架锁定模式不允许生成 keyPoints / branches，请删除后只输出条件、plotEdges、endings");
...
export function runDeepParsePreflight(deepParse, flat) {
  ...
  if (finalChoiceIds.size > 0 && endings0.length > 0) {
    for (let endingIndex = 0; endingIndex < endings0.length; endingIndex += 1) {
      const ending = endings0[endingIndex];
      if (!finalChoiceIds.has(String(ending?.branchId ?? ""))) {
        issues.push({
          severity: "high",
          where: `endings[${endingIndex}].branchId`,
          problem: `结局必须挂在玩家选择型最终分支上（${[...finalChoiceIds].join(" / ")}），不能挂在技能检定分支：${String(ending?.branchId ?? "")}`,
          suggestion: "把 endings[].branchId 改为最终抉择分支 id，并同步 optionLabel 为其选项 label",
        });
      }
    }
  }
