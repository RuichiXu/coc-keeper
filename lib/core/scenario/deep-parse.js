...
export function runDeepParsePreflight(deepParse, flat) {
  ...
  for (const branch of allBranches) {
    for (let optionIndex = 0; optionIndex < asArray(branch?.options).length; optionIndex += 1) {
      ...
      if (!matchTitle(leadsTo) && !sceneNames.has(leadsTo) && !matchEndingRef(leadsTo)) {
        issues.push({
          severity: "medium",
  ...
  // 结构分析后的 keyPoints 带 kind/flowRole/parentId/order；规则按场景角色分别判定。
  // 提前计算开场主场景，让 BFS 从开场节点出发（即使它被人为加了 hook 入边）。
  const kpMeta = new Map(
    allKp.map((kp) => [String(kp?.id ?? ""), {
  ...
  const mainSceneKps = allKp.filter((kp) => {
    ...
    return meta !== undefined && meta.flowRole === "main" && (meta.kind === "scene" || meta.kind === "scene_event");
  });
  ...
  const hasParentScene = (kp) => {
    ...
    return reachable.has(parentId);
  };
  const sceneKpCount = allKp.filter((kp) => {
    const meta = kpMeta.get(String(kp?.id ?? ""));
    return meta !== undefined && (meta.kind === "scene" || meta.kind === "scene_event");
  }).length;
  const isSceneLike = (kp) => {
    const meta = kpMeta.get(String(kp?.id ?? ""));
    return meta !== undefined && (meta.kind === "scene" || meta.kind === "scene_event");
  };
  ...
  for (const kp of allKp) {
    ...
    const id = `kp:${kpId}`;
    const isGenerated = generatedKpIds.has(kpId);
    const meta = kpMeta.get(kpId);
    const isMainScene = meta !== undefined && meta.flowRole === "main" && (meta.kind === "scene" || meta.kind === "scene_event");
    ...
    if (!reachable.has(id)) {
      ...
      issues.push({
        ...
        suggestion: "补上进入该关键点的分支选项 leadsTo 或 plotEdges（支线/线索场景需要 hook 边挂入）",
      });
    } else if (isSceneLike(kp) && !isOpening && (inDegree.get(id) ?? 0) === 0 && !hasParentScene(kp)) {
      issues.push({
        severity: "medium",
        where: `keyPoints[${kpId}]`,
    ...
    // 主线推进规则：仅 flowRole=main 且非最后主场景的节点，没有非自环出边记 medium。
  ...
  // 没有结构 flowRole 标记的旧数据：沿用“生成的关键点缺入边”提示，但不重复报已报过的场景规则。
  if (sceneKpCount === 0) {
    for (const kp of allKp) {
      const kpId = String(kp?.id ?? "");
      const id = `kp:${kpId}`;
      const isGenerated = generatedKpIds.has(kpId);
      if (!reachable.has(id)) continue;
      if (isGenerated && (inDegree.get(id) ?? 0) === 0) {
        issues.push({
          severity: "medium",
          where: `keyPoints[${kpId}]`,
...
export function repairDeepParseFinalWiring(flat, deepParse = flat?.deepParse) {
  ...
  for (const ending of endings) {
    ...
    const hasDistinguishing =
      req !== null && req !== undefined && typeof req === "object" && !Array.isArray(req) &&
      ["scene", "entryEvidence", "checkpointGroups", "sanityEventIds", "keyPointIds", "not"].some(
...
export function repairDeepParseConnectivity(flat, deepParse = flat?.deepParse) {
  ...
  // 1) 结局场景点归位：标题/关键词命中 ending 的孤立 kp，从 end 拉一条同义边。
  const endingByTitle = new Map();
  ...
  // 2) 主线场景推进边：flowRole=main 的 scene 按 order 顺序，没有出边就
  //    指向下一个 main scene（最后一个除外）。
  const mainScenes = keyPoints
    .filter((kp) => (kp?.kind === "scene" || kp?.kind === "scene_event") && kp?.flowRole === "main")
    .sort(
      (a, b) =>
  ...
  for (let index = 0; index < mainScenes.length; index += 1) {
    ...
    const from = `kp:${String(scene.id)}`;
    const next = mainScenes[index + 1];
    // 非最后主场景且没有出边时，按 order 指向下一个主场景。
    if (next !== undefined && !hasOut.has(from)) {
      addEdge(from, `kp:${String(next.id)}`, "继续调查", []);
    }
    // 非首个主场景且没有入边时，从前一个主场景拉一条兜底入边，
    // 避免并行主线/多个入口场景被 preflight 判为不可达。
    if (index > 0 && !hasIn.has(from)) {
      addEdge(`kp:${String(mainScenes[index - 1].id)}`, from, "继续调查", []);
    }
  }

  // 3) 支线/事件 hook：非 main scene/scene_event 且没有入边的，从前一个
  //    main scene 拉一条 hook 边。
  let lastMainId = null;
  for (const kp of [...keyPoints].sort((a, b) => (Number(a?.order) || 0) - (Number(b?.order) || 0))) {
    if (kp?.kind === "scene" && kp?.flowRole === "main") {
      lastMainId = `kp:${String(kp.id)}`;
      continue;
    }
    const target = `kp:${String(kp.id)}`;
    if (hasIn.has(target)) continue;
    if (lastMainId !== null) addEdge(lastMainId, target, "关联调查", []);
  }
