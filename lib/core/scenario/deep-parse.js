 ...
 * @returns {number}
 */
export function repairDeepParseFinalWiring(flat) {
  const dp = flat?.deepParse;
  ...
  const endingByOptionLabel = new Map();
  const endingByTitle = new Map();
  for (const ending of endings) {
    const label = nonEmptyString(ending?.optionLabel);
    if (label.length > 0) endingByOptionLabel.set(label, ending);
    const title = nonEmptyString(ending?.title);
  ...
  const repairBranch = (branch) => {
    ...
    const isFinal = branch?.finalChoice === true || String(branch?.id ?? "").startsWith("br-final");
    if (!isFinal) return;
    const brTarget = `br:${String(branch.id)}`;
    if (!hasEdgeTo(brTarget)) {
    ...
    for (const option of asArray(branch.options)) {
      ...
      const ending =
        endingByOptionLabel.get(label) ??
        (() => {
          for (const [title, candidate] of endingByTitle) {
            if (label.includes(title) || title.includes(label)) return candidate;
          }
          return undefined;
        })();
      if (ending !== undefined) {
  ...
  for (const branch of flatBranches) repairBranch(branch);
  for (const branch of dpBranches) repairBranch(branch);
  return repaired;
}