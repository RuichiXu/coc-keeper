 ...
 *
 * 纯 Prompt + 解析 + 校验，零 DSH 依赖，不直接调用 LLM。
 * 导入链路通过 deps.callLlmApi 调用后把结果交给本模块解析。
 *
 * 产物结构（deepParse）：
 * {
 *   version: "1.0",
...
import { computeConditionalClosure, inferReachability, isSceneLikeKp } from "./topology-skeleton.js";
 ...
 * @returns {string[]} 问题列表
 */
export function validateConditionObject(cond, label = "condition", options = {}) {
  const issues = [];
  ...
  if (cond.optionLabel !== undefined && cond.optionLabel !== null) {
    ...
    } else if (asArray(cond.branchChoiceIds).length === 0 && options?.insideNot !== true) {
  ...
  if (cond.not !== undefined && cond.not !== null) {
    if (typeof cond.not !== "object" || Array.isArray(cond.not)) {
      ...
      issues.push(...validateConditionObject(cond.not, `${label}.not`, { insideNot: true }));
    }
  }
...
export function normalizeDeepParse(raw) {
  ...
  return {
    ...
    branches: asArray(source.branches).map((entry) => ({
      ...
      ...(entry?.desc !== undefined ? { desc: nonEmptyString(entry.desc) } : {}),
      ...(entry?.finalChoice === true ? { finalChoice: true } : {}),
      ...(entry?.autoChoose === true ? { autoChoose: true } : {}),
      ...(entry?.kind !== undefined ? { kind: nonEmptyString(entry.kind) } : {}),
...
function canonicalizeConditionEntry(entry, idKey) {
  ...
  // 别名：一些模型把条件数组放在 conditions / condition 字段。
  if (entry.requires === undefined && entry.requiresAnyOf === undefined) {
    ...
    } else if (entry.condition !== undefined && entry.condition !== null) {
      if (Array.isArray(entry.condition)) entry.requiresAnyOf = entry.condition;
      else if (typeof entry.condition === "object") entry.requires = entry.condition;
    }
  }
...
function normalizeEdgeEndpoint(value) {
  ...
  if (/^(br|kp|end):/.test(text)) return text;
  if (/^br-/.test(text)) return `br:${text}`;
  if (/^kp-/.test(text)) return `kp:${text}`;
  if (/^end-/.test(text)) return `end:${text}`;
  return text;
}

function canonicalizeEdge(edge) {
  if (edge === null || edge === undefined || typeof edge !== "object" || Array.isArray(edge)) return undefined;
  const out = { from: normalizeEdgeEndpoint(edge.from), to: normalizeEdgeEndpoint(edge.to) };
...
function canonicalizeEnding(ending) {
  ...
  const requiresRaw = ending.requires;
  const requiresOptionLabel =
    requiresRaw !== null && requiresRaw !== undefined && typeof requiresRaw === "object" && !Array.isArray(requiresRaw)
      ? nonEmptyString(requiresRaw.optionLabel)
      : "";
  const title =
    nonEmptyString(ending.title) ||
    nonEmptyString(ending.label) ||
    nonEmptyString(ending.name) ||
    nonEmptyString(ending.optionLabel) ||
    requiresOptionLabel;
  const out = {
...
function canonicalizeBranch(entry) {
  ...
  const out = {
    id: nonEmptyString(entry.id),
    title: nonEmptyString(entry.title) || nonEmptyString(entry.label) || nonEmptyString(entry.name),
  };
  if (entry.scene !== undefined) out.scene = nonEmptyString(entry.scene);
  if (entry.desc !== undefined) out.desc = nonEmptyString(entry.desc);
  if (entry.finalChoice === true) out.finalChoice = true;
  if (entry.checkpointBranch === true) out.checkpointBranch = true;
  if (entry.autoChoose === true) out.autoChoose = true;
  if (entry.kind !== undefined) out.kind = nonEmptyString(entry.kind);
...
export function canonicalizeDeepParse(raw, options = {}) {
  ...
  const plotEdges = asArray(unwrapped.plotEdges)
    .map(canonicalizeEdge)
    .filter((edge) => edge !== undefined)
    .filter((edge) => {
      const key = `${String(edge?.from ?? "")}|${String(edge?.to ?? "")}|${String(edge?.label ?? "")}`;
...
/**
 * 比较两份 deepParse 草稿的“可用程度”，返回更优的一份。
 * 用于资产与场次数据同步时防止旧解析覆盖新解析：
 * - 先比门禁计数（preflight/rule/review/chunk 的 high、medium、low 合计），越低越好；
 * - 门禁平手时比 generatedAt，越新越好；
 * - 仍平手时返回 b（默认认为“候选方”更值得写入）。
 * 任一参数为空时返回另一方；都为空返回 null。
 * @param {object|null|undefined} a 已有草稿（如资产中的 deepParse）
 * @param {object|null|undefined} b 候选草稿（如场次/新导入的 deepParse）
 * @returns {object|null}
 */
export function pickBetterDeepParse(a, b) {
  if (a === null || a === undefined) return b ?? null;
  if (b === null || b === undefined) return a;

  const scoreA = deepParseGateScore(a);
  const scoreB = deepParseGateScore(b);
  if (scoreA.high !== scoreB.high) return scoreA.high < scoreB.high ? a : b;
  if (scoreA.medium !== scoreB.medium) return scoreA.medium < scoreB.medium ? a : b;
  if (scoreA.low !== scoreB.low) return scoreA.low < scoreB.low ? a : b;

  const timeA = Date.parse(String(a?.generatedAt ?? "")) || 0;
  const timeB = Date.parse(String(b?.generatedAt ?? "")) || 0;
  if (timeB > timeA) return b;
  return a;
}

function deepParseGateScore(deepParse) {
  const q = deepParse?.quality ?? null;
  const num = (value) => (Number.isFinite(Number(value)) ? Number(value) : 0);
  if (q === null || typeof q !== "object") {
    return { high: Number.POSITIVE_INFINITY, medium: Number.POSITIVE_INFINITY, low: Number.POSITIVE_INFINITY };
  }
  const hasCategoryCounts =
    q.preflightHigh !== undefined || q.ruleHigh !== undefined || q.reviewHigh !== undefined || q.chunkHigh !== undefined ||
    q.preflightMedium !== undefined || q.ruleMedium !== undefined || q.reviewMedium !== undefined || q.chunkMedium !== undefined;
  if (hasCategoryCounts) {
    return {
      high: num(q.preflightHigh) + num(q.ruleHigh) + num(q.reviewHigh) + num(q.chunkHigh),
      medium: num(q.preflightMedium) + num(q.ruleMedium) + num(q.reviewMedium) + num(q.chunkMedium),
      low: num(q.preflightLow) + num(q.ruleLow) + num(q.reviewLow) + num(q.chunkLow),
    };
  }
  return {
    high: num(q.high),
...
export function repairSkeletonWiringDeepParse(deepParse, flat) {
  ...
  // 1c) 同最终分支结局的 not 排除项补齐：若两个结局的正向前置完全相同，
  // 其中一个已经用 not.keyPointIds 排除某关键点（例如“仪式完成后不可选”），
  // 另一个也应补同样的排除项，避免阶段错配的结局被错误命中。
  {
    const endingGroups = new Map();
    for (const ending of asArray(dp.endings)) {
      const branchId = nonEmptyString(ending?.branchId);
      if (branchId.length === 0 || !finalChoiceIds.has(branchId)) continue;
      if (!endingGroups.has(branchId)) endingGroups.set(branchId, []);
      endingGroups.get(branchId).push(ending);
    }
    for (const endings of endingGroups.values()) {
      const byPositiveKey = new Map();
      for (const ending of endings) {
        const req = ending?.requires;
        const positiveKeys = asArray(req?.keyPointIds).map((id) => String(id)).sort();
        if (positiveKeys.length === 0) continue;
        const key = positiveKeys.join("|");
        if (!byPositiveKey.has(key)) byPositiveKey.set(key, []);
        byPositiveKey.get(key).push(ending);
      }
      for (const siblings of byPositiveKey.values()) {
        if (siblings.length < 2) continue;
        const knownNots = [];
        for (const ending of siblings) {
          const notKeys = asArray(ending?.requires?.not?.keyPointIds).map((id) => String(id));
          for (const key of notKeys) if (!knownNots.includes(key)) knownNots.push(key);
        }
        if (knownNots.length === 0) continue;
        for (const ending of siblings) {
          const req = ending.requires;
          if (req === null || req === undefined || typeof req !== "object" || Array.isArray(req)) continue;
          if (req.not === null || req.not === undefined || typeof req.not !== "object" || Array.isArray(req.not)) req.not = {};
          const currentNots = new Set(asArray(req.not.keyPointIds).map((id) => String(id)));
          let added = false;
          for (const key of knownNots) {
            if (!currentNots.has(key)) { currentNots.add(key); added = true; }
          }
          if (added) {
            req.not.keyPointIds = [...currentNots];
            repairs.push(`结局 ${String(ending?.id ?? ending?.title ?? "")} 补齐 not.keyPointIds：[${knownNots.join(", ")}]`);
          }
        }
      }
    }
  ...
  // 1d) 同最终分支结局互斥兜底：每条带正向前置的结局，排除同分支其他结局
  //     的正向前置关键点（本结局未正向要求的），避免“同时满足多个附加条件”
  //     时互斥失效（如《两面不是人》end-2/3/4 各自缺少 not）。
  {
    const endingGroups = new Map();
    for (const ending of asArray(dp.endings)) {
      const branchId = nonEmptyString(ending?.branchId);
    ...
    for (const endings of endingGroups.values()) {
      ...
      const positiveByEnding = new Map();
      for (const ending of endings) {
        const positiveKeys = asArray(ending?.requires?.keyPointIds).map((id) => String(id)).filter((id) => id.length > 0);
        if (positiveKeys.length > 0) positiveByEnding.set(ending, positiveKeys);
      }
      if (positiveByEnding.size < 2) continue;
      const allPositive = new Set();
      for (const keys of positiveByEnding.values()) for (const key of keys) allPositive.add(key);
      for (const [ending, ownKeys] of positiveByEnding) {
        const req = ending.requires;
        if (req === null || req === undefined || typeof req !== "object" || Array.isArray(req)) continue;
        const ownSet = new Set(ownKeys);
        const missingNots = [...allPositive].filter((key) => !ownSet.has(key));
        if (missingNots.length === 0) continue;
        if (req.not === null || req.not === undefined || typeof req.not !== "object" || Array.isArray(req.not)) req.not = {};
        const currentNots = new Set(asArray(req.not.keyPointIds).map((id) => String(id)));
        let added = false;
        for (const key of missingNots) {
          if (!currentNots.has(key)) { currentNots.add(key); added = true; }
        }
        if (added) {
          req.not.keyPointIds = [...currentNots];
          repairs.push(`结局 ${String(ending?.id ?? ending?.title ?? "")} 互斥补齐 not.keyPointIds：[${missingNots.join(", ")}]`);
        }
      }
    }
  }
  ...
  for (let index = 0; index < asArray(dp.endings).length; index += 1) {
    ...
    const directEdge = asArray(dp.plotEdges).find((edge) => String(edge?.from ?? "") === from && String(edge?.to ?? "") === to);
    // 直接入边的 requires 必须与结局 requires 一致（去掉 branchChoiceIds/optionLabel，
    // 它们由边 label 表达），否则会出现“边可通过但结局条件不满足”。
    const edgeRequires = [];
    const endingReq = ending.requires;
    if (endingReq !== null && endingReq !== undefined && typeof endingReq === "object" && !Array.isArray(endingReq)) {
      const cond = {};
      for (const key of ["scene", "entryEvidence", "checkpointGroups", "sanityEventIds", "keyPointIds", "not"]) {
        if (endingReq[key] !== undefined && endingReq[key] !== null) cond[key] = endingReq[key];
      }
      if (Object.keys(cond).length > 0) edgeRequires.push(cond);
    }
    if (directEdge !== undefined) {
      directEdge.requires = edgeRequires;
      directEdge.label = endingLabel.length > 0 ? endingLabel : nonEmptyString(ending.title);
    } else if (to.length > "end:".length) {
...
export function validateDeepParse(deepParse, flat) {
  ...
  for (let index = 0; index < asArray(deepParse.branches).length; index += 1) {
    ...
    if (nonEmptyString(entry?.title).length === 0) issues.push(`${label}.title 不能为空`);
    const isAutoBranch = entry?.autoChoose === true || entry?.kind === "auto";
    if (asArray(entry?.options).length === 0 && !isAutoBranch) {
      issues.push(`${label}.options 至少需要一个选项`);
    } else if (asArray(entry?.options).length > 0) {
...
export function runDeepParsePreflight(deepParse, flat) {
  ...
  const autoBranchIds = new Set(
    [
      ...branchList0,
      ...asArray(deepParse?.branches),
    ].filter((branch) => branch?.autoChoose === true || branch?.kind === "auto").map((branch) => String(branch?.id ?? "")),
  );
  {
  ...
  // 结构分析后的 keyPoints 带 kind/flowRole/parentId/order；统一在此计算
  // reachability 与开场节点，供条件闭包与分级判定使用。
  const kpMeta = new Map(
    allKp.map((kp) => {
      ...
      return [kpId, {
        ...
        parentId: nonEmptyString(kp?.parentId),
        order: Number(kp?.order ?? 0),
        scene: nonEmptyString(kp?.scene),
        title: nonEmptyString(kp?.title),
        reachability: inferReachability(kp, kpById),
      }];
    })
  );
  const mainSceneKps = allKp.filter((kp) => {
    const meta = kpMeta.get(String(kp?.id ?? ""));
    return meta !== undefined && meta.reachability === "strict" && (meta.kind === "scene" || meta.kind === "scene_event");
  });
  const openingKpIds = new Set();
  if (mainSceneKps.length > 0) {
    const ordered = [...mainSceneKps].sort((a, b) => {
      const aOrder = kpMeta.get(String(a?.id ?? ""))?.order ?? 0;
      const bOrder = kpMeta.get(String(b?.id ?? ""))?.order ?? 0;
      if (aOrder !== bOrder) return aOrder - bOrder;
      return nonEmptyString(a?.title).localeCompare(nonEmptyString(b?.title), "zh-Hans-CN");
    });
    openingKpIds.add(`kp:${String(ordered[0]?.id ?? "")}`);
  }
  ...
  // 显式 end: 端点也视为合法去向（图同步会把 end:<id> 解析到结局节点）。
  const endingRefs = new Set(
    endings.flatMap((ending) => [
      `end:${nonEmptyString(ending?.id)}`,
      `end:${nonEmptyString(ending?.title)}`,
    ]).filter((item) => item.length > "end:".length)
  );
  const matchEndingRef = (text) => {
    const value = nonEmptyString(text);
    return value.length > 0 && (endingRefs.has(value) || /^end:/.test(value));
  };
  // 显式 br: 端点也视为合法去向：两段式最终抉择会用一个最终分支的选项
  // leadsTo 指向另一个最终分支（如 br-final-1 → br-final-2）。
  const matchBranchRef = (text) => {
    const value = nonEmptyString(text);
    return value.length > 0 && /^br:/.test(value) && branchById.has(value.slice(3));
  };

  // 1. 分支选项 leadsTo 必须命中某个节点标题/结局关键词/结局端点（否则运行时只能靠自动补点兜底）。
  for (const branch of allBranches) {
    for (let optionIndex = 0; optionIndex < asArray(branch?.options).length; optionIndex += 1) {
      ...
      if (!matchTitle(leadsTo) && !sceneNames.has(leadsTo) && !matchEndingRef(leadsTo) && !matchBranchRef(leadsTo)) {
        issues.push({
          severity: "medium",
  ...
  const starts = [...new Set([...nodeIds].filter((id) => (inDegree.get(id) ?? 0) === 0).concat([...openingKpIds]))];
  ...
  const closureOpening =
    openingKpIds.size > 0
      ? openingKpIds
      : new Set([...nodeIds].filter((id) => id.startsWith("kp:") && (inDegree.get(id) ?? 0) === 0));
  const closureReachable = computeConditionalClosure({
    openingIds: closureOpening,
    edges: closureEdges,
    branchConditions,
    checkpointIds,
  });
  ...
  const hasOutEdge = (id) => (adjacency.get(id) ?? []).length > 0;
  const hasParentScene = (kp) => {
    const meta = kpMeta.get(String(kp?.id ?? ""));
    if (meta === undefined || meta.parentId.length === 0) return false;
    const parentId = `kp:${meta.parentId}`;
    return closureReachable.has(parentId);
  };

  for (const kp of allKp) {
    const kpId = String(kp?.id ?? "");
    const id = `kp:${kpId}`;
    const meta = kpMeta.get(kpId);
    const reachability = meta?.reachability ?? "conditional";
    const isMainScene = meta !== undefined && reachability === "strict" && (meta.kind === "scene" || meta.kind === "scene_event");
    const isOpening = openingKpIds.has(id);
    const reachableInClosure = closureReachable.has(id);

    // optional：长期悬空可选内容，完全豁免连通性门禁。
    if (reachability === "optional") continue;

    if (!reachableInClosure) {
      const referencedByEnding = endings.some((ending) => asArray(ending?.requires).some((cond) => asArray(cond?.keyPointIds).map(String).includes(kpId)));
      if (reachability === "strict") {
        issues.push({
          severity: referencedByEnding ? "high" : "medium",
          where: `keyPoints[${kpId}]`,
          problem: `关键点在剧情图上不可达：${nonEmptyString(kp?.title)}`,
          suggestion: "补上进入该关键点的分支选项 leadsTo 或 plotEdges（主线节点必须有无条件/条件闭包可达路径）",
        });
      } else {
        issues.push({
          severity: "medium",
          where: `keyPoints[${kpId}]`,
          problem: `支线/条件节点在条件闭包下不可达：${nonEmptyString(kp?.title)}`,
          suggestion: "补上 hook 边（源场景→事件→支线场景）或条件入边；若确为长期悬空内容，标记 reachability=\"optional\"",
        });
      }
      continue;
    }

    // conditional：闭包可达即可，不要求非自环入边/出边。
    if (reachability === "conditional") continue;

    // strict 且可达：保留原主线/场景节点的入边与推进边要求。
    if (isSceneLikeKp(kp) && !isOpening && (inDegree.get(id) ?? 0) === 0 && !hasParentScene(kp)) {
      issues.push({
        severity: "medium",
        where: `keyPoints[${kpId}]`,
        problem: `场景节点没有非自环入边，也没有 hook 入边或可达父场景：${nonEmptyString(kp?.title)}`,
        suggestion: "补上指向它的分支选项 leadsTo 或 plotEdges（hook 边：源场景→事件→支线场景）",
      });
    }

    // 主线推进规则：仅 strict 场景节点且非最后主场景的节点，没有非自环出边记 medium。
    if (isMainScene && !isOpening && !hasOutEdge(id) && mainSceneKps.length > 1) {
      const orderedMain = [...mainSceneKps].sort((a, b) => {
        const aOrder = kpMeta.get(String(a?.id ?? ""))?.order ?? 0;
        const bOrder = kpMeta.get(String(b?.id ?? ""))?.order ?? 0;
        if (aOrder !== bOrder) return aOrder - bOrder;
        return nonEmptyString(a?.title).localeCompare(nonEmptyString(b?.title), "zh-Hans-CN");
      });
      const lastMain = orderedMain[orderedMain.length - 1];
      if (lastMain !== undefined && String(lastMain?.id ?? "") !== kpId) {
        issues.push({
          severity: "medium",
          where: `keyPoints[${kpId}]`,
          problem: `主线场景缺少推进边：${nonEmptyString(kp?.title)}`,
          suggestion: "补一条 plotEdge 从该场景指向原文明确提到的下一可去场景",
        });
      }
    }
  }
...
export function mergeDeepParseDraft(flat, deepParse) {
  ...
  const knownKpById = new Map(keyPoints.map((kp) => [String(kp?.id ?? "").trim(), kp]));
  ...
  for (const kp of asArray(deepParse?.keyPoints)) {
    ...
    const mergedKp = {
      ...
      reachability: inferReachability(kp, knownKpById),
    };
    keyPoints.push(mergedKp);
    knownKpById.set(finalId, mergedKp);
    kpMap.set(rawId, finalId);
    keyPointsAdded += 1;
  }
  ...
  for (const branch of asArray(deepParse?.branches)) {
    ...
    if (finalId.length === 0) {
      ...
      if (existingBranch !== undefined) {
        ...
        if (branch?.autoChoose === true || branch?.kind === "auto") {
          existingBranch.autoChoose = true;
          existingBranch.finalChoice = false;
          existingBranch.options = [];
        }
        const isFinalBranch =
          branch?.finalChoice === true ||
          existingBranch?.finalChoice === true ||
          String(finalId).startsWith("br-final");
        // 最终接线 prompt 会重写最终分支 options（例如把残缺半句改成
        // “毁灭：…”完整选项）；必须同步回 flat.branches，否则运行时
        // 玩家看到的选项与 endings.optionLabel 对不上。
        if (isFinalBranch && asArray(branch?.options).length > 0) {
          existingBranch.options = asArray(branch.options);
          existingBranch.finalChoice = true;
        } else if (asArray(branch?.options).length > 0 && asArray(existingBranch?.options).length === 0) {
          existingBranch.options = asArray(branch.options);
        }
      }
...
/**
 * 最终接线兜底修复：最终分支是玩家选择型节点，必须挂进剧情图并与结局相连。
 * LLM 最终接线（尤其强推理模型）经常返回空或 leadsTo 无法命中任何节点；
 * 这里做确定性修复：
 * - 最终分支若没有入边，从同场景关键点补一条进入边
 * - 最终分支选项按 optionLabel/标题关键词匹配 ending，把 leadsTo 改成 end:<id>
 * 返回修复条数。
 *
 * @param {object} flat
 * @returns {number}
 */
export function repairDeepParseFinalWiring(flat, deepParse = flat?.deepParse) {
  const dp = deepParse;
  if (dp === null || dp === undefined) return 0;
  const endings = asArray(dp.endings);
  if (endings.length === 0) return 0;
  const keyPoints = asArray(flat?.keyPoints);
  const flatBranches = asArray(flat?.branches);
  const dpBranches = asArray(dp.branches);
  const plotEdges = Array.isArray(dp.plotEdges) ? dp.plotEdges : (dp.plotEdges = []);
  let repaired = 0;

  const normTitle = (value) => String(value ?? "").replace(/[：:]\s*$/, "").trim();

  // ── 最终分支识别：finalChoice / id 形态 / 被 endings 引用 / 标题含“最终” ──
  const referencedIds = new Set();
  for (const ending of endings) {
    for (const id of asArray(ending?.requires?.branchChoiceIds)) {
      if (String(id ?? "").length > 0) referencedIds.add(String(id));
    }
  }
  const isFinalBranch = (branch) => {
    if (branch === null || branch === undefined) return false;
    const id = String(branch?.id ?? "");
    if (branch?.finalChoice === true) return true;
    if (/^br-(final|failure|success|ending)/i.test(id)) return true;
    if (referencedIds.has(id)) return true;
    return /最终|结局/.test(String(branch?.title ?? ""));
  };
  const finalBranchIds = new Set();
  for (const branch of [...flatBranches, ...dpBranches]) {
    if (isFinalBranch(branch)) finalBranchIds.add(String(branch.id));
  }
  for (const id of referencedIds) finalBranchIds.add(id);

  // ── 自动判定型分支识别（A6）：显式 autoChoose/kind=auto，或失败结局分支 ──
  const autoBranchIds = new Set();
  for (const id of finalBranchIds) {
    const branch = [...flatBranches, ...dpBranches].find((candidate) => String(candidate?.id ?? "") === id);
    const branchEndings = asArray(deepParse?.endings).filter((ending) => {
      const choiceIds = asArray(ending?.requires?.branchChoiceIds);
      return String(ending?.branchId ?? "") === id || choiceIds.includes(id);
    });
    if (branch?.autoChoose === true || branch?.kind === "auto") {
      autoBranchIds.add(id);
    } else if (
      asArray(branch?.options).length === 0 &&
      branchEndings.length > 0 &&
      /失败|败局|bad\s*end/i.test(String(branch?.title ?? ""))
    ) {
      autoBranchIds.add(id);
    }
  }

  // ── endings 回填 branchId / optionLabel，并按 branchId 分组 ──
  const endingsOfBranch = new Map();
  const endingByLabel = new Map();
  const endingByTitle = new Map();
  for (const ending of endings) {
    const label = nonEmptyString(ending?.requires?.optionLabel) || nonEmptyString(ending?.optionLabel);
    if (label.length > 0) {
      endingByLabel.set(label, ending);
      if (String(ending?.optionLabel ?? "").length === 0) {
        ending.optionLabel = label;
        repaired += 1;
      }
    }
    let title = nonEmptyString(ending?.title);
    if (title.length === 0 && label.length > 0) {
      // 模型常把结局标题省略，只写 optionLabel；结局标题回填为选项 label。
      ending.title = label;
      title = label;
      repaired += 1;
    }
    if (title.length > 0) endingByTitle.set(title, ending);
    for (const keyword of asArray(ending?.endingKeywords)) {
      const kw = nonEmptyString(keyword);
      if (kw.length > 0) endingByTitle.set(kw, ending);
    }

    let branchId = nonEmptyString(ending?.branchId);
    if (branchId.length === 0) {
      const choiceIds = asArray(ending?.requires?.branchChoiceIds);
      if (choiceIds.length === 1) branchId = String(choiceIds[0]);
      else if (choiceIds.length > 1) branchId = String(choiceIds[0]);
    }
    if (branchId.length > 0) {
      if (String(ending?.branchId ?? "") !== branchId) {
        ending.branchId = branchId;
        repaired += 1;
      }
      if (!endingsOfBranch.has(branchId)) endingsOfBranch.set(branchId, []);
      endingsOfBranch.get(branchId).push(ending);
    }

    // 结局除 branchChoiceIds/optionLabel 外没有任何区分条件时，用
    // endingKeywords 作为 entryEvidence 兜底：这些关键词本来就是该结局的
    // 叙述命中词，运行时文本中出现任意一个才允许进入该结局，避免
    // “无条件结局”在条件未满足时被误选（审校 high 的常见来源）。
    const req = ending?.requires;
    const hasDistinguishing =
      req !== null && req !== undefined && typeof req === "object" && !Array.isArray(req) &&
      ["scene", "entryEvidence", "checkpointGroups", "sanityEventIds", "keyPointIds", "branchChoiceIds", "optionLabel", "not"].some(
        (key) => req[key] !== undefined && req[key] !== null
      );
    if (!hasDistinguishing) {
      const keywords = asArray(ending?.endingKeywords).map((item) => nonEmptyString(item)).filter((item) => item.length > 0);
      const evidence = keywords.length > 0 ? keywords : (title.length > 0 ? [title] : []);
      if (evidence.length > 0) {
        ending.requires = { ...(req ?? {}), entryEvidence: evidence };
        repaired += 1;
      }
    }
  }

  const findEndingForOption = (label) => {
    const exact = endingByLabel.get(String(label ?? ""));
    if (exact !== undefined) return exact;
    const text = String(label ?? "");
    for (const [title, candidate] of endingByTitle) {
      if (text.includes(title) || title.includes(text)) return candidate;
    }
    return undefined;
  };

  const branchSceneKp = (branch) => {
    const branchScene = normTitle(branch?.scene);
    if (branchScene.length > 0) {
      return (
        keyPoints.find((kp) => normTitle(kp?.scene) === branchScene) ??
        keyPoints.find((kp) => normTitle(kp?.title) === branchScene)
      );
    }
    // 无 scene 时按分支标题与关键点标题/场景做包含匹配，取第一个。
    const branchTitle = normTitle(branch?.title);
    return keyPoints.find((kp) => {
      const kpTitle = normTitle(kp?.title);
      const kpScene = normTitle(kp?.scene);
      return (
        (branchTitle.length > 0 && (kpTitle.includes(branchTitle) || branchTitle.includes(kpTitle))) ||
        (branchTitle.length > 0 && (kpScene.includes(branchTitle) || branchTitle.includes(kpScene)))
      );
    });
  };

  const hasEdgeTo = (targetId) => plotEdges.some((edge) => String(edge?.to ?? "") === targetId);

  const repairBranch = (branch) => {
    if (branch === null || branch === undefined) return;
    const id = String(branch?.id ?? "");
    if (!finalBranchIds.has(id)) return;

    const brEndings = endingsOfBranch.get(id) ?? [];
    const isAuto = autoBranchIds.has(id);

    // 0) 标题回填：最终分支标题为空时，单选项分支用选项 label，
    //    多选项分支用结局标题/“最终抉择”，自动分支用结局标题/“自动结局”。
    if (nonEmptyString(branch?.title).length === 0) {
      const options = asArray(branch?.options);
      const firstLabel = nonEmptyString(options[0]?.label);
      const endingTitle =
        brEndings
          .map((ending) => nonEmptyString(ending?.title))
          .find((item) => item.length > 0) ?? "";
      const fallbackTitle =
        options.length === 1 && firstLabel.length > 0
          ? firstLabel
          : (endingTitle.length > 0 ? endingTitle : (isAuto ? "自动结局" : "最终抉择"));
    ...
    if (isAuto) {
      // 自动判定型分支：不是玩家选择，不生成 options，也不标记 finalChoice。
      if (branch?.autoChoose !== true) {
        branch.autoChoose = true;
        repaired += 1;
      }
      if (branch?.finalChoice === true) {
        branch.finalChoice = false;
        repaired += 1;
      }
      // 自动分支的结局标记 kind=auto，并保证直接入边 label 使用结局标题。
      for (const ending of brEndings) {
        if (ending?.kind !== "auto") {
          ending.kind = "auto";
          repaired += 1;
        }
      ...
      // 1) options 为空时从 endings 反推（玩家选择型最终分支）。
      if (asArray(branch?.options).length === 0 && brEndings.length > 0) {
        branch.options = brEndings.map((ending) => {
          const label =
          ...
          const leadsTo =
            nonEmptyString(ending?.title) ||
            asArray(ending?.endingKeywords).map((item) => nonEmptyString(item)).find((item) => item.length > 0) ||
            `end:${String(ending.id)}`;
          return { label, leadsTo };
        });
        repaired += 1;
      }
      ...
      // 3) 选项 leadsTo 修正为结局标题/关键词（图同步用标题命中结局节点；
      //    end: 端点只用于 plotEdges，避免 preflight 把 end: 判为未命中）。
      for (const option of asArray(branch?.options)) {
        if (option === null || option === undefined) continue;
        const ending = findEndingForOption(option?.label);
        if (ending !== undefined) {
          const leadsTo =
            nonEmptyString(ending?.title) ||
            asArray(ending?.endingKeywords).map((item) => nonEmptyString(item)).find((item) => item.length > 0) ||
            `end:${String(ending.id)}`;
          if (String(option?.leadsTo ?? "") !== leadsTo) {
            option.leadsTo = leadsTo;
            repaired += 1;
          }
        }
      }
      // 3b) 玩家选择型最终分支必须有 branchCondition 先 reached（scene 门控且不代选）。
      if (!Array.isArray(dp.branchConditions)) dp.branchConditions = [];
      const conds = dp.branchConditions.filter((cond) => String(cond?.branchId ?? "") === id);
      if (conds.length === 0) {
        const sceneKp = branchSceneKp(branch);
        const scene =
          nonEmptyString(branch?.scene) ||
          (sceneKp !== undefined ? nonEmptyString(sceneKp?.scene) || nonEmptyString(sceneKp?.title) : "") ||
          "结局";
        dp.branchConditions.push({ branchId: id, requires: { scene } });
        repaired += 1;
      } else {
        const cond = conds[0];
        if (String(cond?.autoChooseLabel ?? "").length > 0) {
          delete cond.autoChooseLabel;
          repaired += 1;
        }
        const hasScene = cond?.requires?.scene !== undefined && cond?.requires?.scene !== null;
        if (!hasScene) {
          ...
          const scene =
            ...
            (sceneKp !== undefined ? nonEmptyString(sceneKp?.scene) || nonEmptyString(sceneKp?.title) : "") ||
            "结局";
          cond.requires = { ...(cond?.requires ?? {}), scene };
          repaired += 1;
        }
      }
    }
    ...
    if (!hasEdgeTo(`br:${id}`)) {
      const sceneKp = branchSceneKp(branch);
      if (sceneKp !== undefined) {
        plotEdges.push({
          from: `kp:${String(sceneKp.id)}`,
          to: `br:${id}`,
          label: isAuto ? "自动判定" : "进入最终抉择",
          requires: [{ keyPointIds: [String(sceneKp.id)] }],
        });
        repaired += 1;
      }
    }
  ...
  // flat 与 dp 都修：两处都会进入前端网络图与运行时，必须同步。
  for (const branch of [...flatBranches, ...dpBranches]) {
    repairBranch(branch);
  }

  // 4b) 两段式最终分支修复：br:A 的选项 leadsTo 指向 br:B 时，把该选项写入
  //     B 的 branchCondition，并清除 B 结局及其出边上重复的 entryEvidence 前置。
  //     这样运行时先选 A 的该选项才 reached B，B 的结局不再依赖叙述命中词。
  const allFinalBranches = [...flatBranches, ...dpBranches].filter((branch) => finalBranchIds.has(String(branch?.id ?? "")));
  const finalBranchById = new Map(allFinalBranches.map((branch) => [String(branch?.id ?? ""), branch]));
  for (const edge of [...plotEdges]) {
    const from = String(edge?.from ?? "");
    const to = String(edge?.to ?? "");
    if (!from.startsWith("br:") || !to.startsWith("br:")) continue;
    const fromId = from.slice(3);
    const toId = to.slice(3);
    if (!finalBranchIds.has(fromId) || !finalBranchIds.has(toId)) continue;
    const sourceBranch = finalBranchById.get(fromId);
    const targetBranch = finalBranchById.get(toId);
    const option = asArray(sourceBranch?.options).find((opt) => {
      const leadsTo = String(opt?.leadsTo ?? "");
      return leadsTo === to || leadsTo === toId || leadsTo === `br:${toId}`;
    });
    ...
    if (!Array.isArray(dp.branchConditions)) dp.branchConditions = [];
    const cond = dp.branchConditions.find((entry) => String(entry?.branchId ?? "") === toId);
    if (cond !== undefined) {
      cond.requires = {
        ...(cond?.requires ?? {}),
        branchChoiceIds: [fromId],
        optionLabel,
      };
      repaired += 1;
    } else {
      const targetScene = nonEmptyString(targetBranch?.scene) || "结局";
      dp.branchConditions.push({
        branchId: toId,
        requires: { scene: targetScene, branchChoiceIds: [fromId], optionLabel },
      });
      repaired += 1;
    }

    for (const ending of endingsOfBranch.get(toId) ?? []) {
      const req = ending?.requires;
      if (req !== null && req !== undefined && typeof req === "object" && !Array.isArray(req) && req.entryEvidence !== undefined) {
        delete req.entryEvidence;
        repaired += 1;
      }
    }
    for (const targetEdge of plotEdges) {
      if (String(targetEdge?.from ?? "") !== to) continue;
      const requires = asArray(targetEdge?.requires);
      if (requires.length === 0) continue;
      const filtered = requires.filter(
        (req) => req === null || typeof req !== "object" || Array.isArray(req) || req.entryEvidence === undefined
      );
      if (filtered.length !== requires.length) {
        targetEdge.requires = filtered;
        repaired += 1;
      }
    }
  }

  // 4c) 叠加结局兜底：原文标注“叠加于结局一的基础上”时，非基准结局必须
  //     先满足基准结局条件（用基准选项原文做 entryEvidence），并继承基准结局
  //     的关键词，避免玩家未达成基础条件就直达后续结局（《两面不是人》）。
  if (/叠加于结局一的基础上/.test(String(flat?.scenario?.text ?? ""))) {
    for (const [branchId, brEndings] of endingsOfBranch) {
      if (brEndings.length < 2) continue;
      const baseEnding = brEndings[0];
      const baseLabel = nonEmptyString(baseEnding?.optionLabel) || nonEmptyString(baseEnding?.requires?.optionLabel);
      const baseKeywords = asArray(baseEnding?.endingKeywords).map((item) => nonEmptyString(item)).filter((item) => item.length > 0);
      if (baseLabel.length === 0) continue;
      for (const ending of brEndings.slice(1)) {
        const req = ending?.requires;
        if (req === null || req === undefined || typeof req !== "object" || Array.isArray(req)) continue;
        if (req.entryEvidence === undefined || req.entryEvidence === null) {
          req.entryEvidence = [baseLabel];
          repaired += 1;
        }
        const keywords = new Set(asArray(ending?.endingKeywords).map((item) => nonEmptyString(item)).filter((item) => item.length > 0));
        ...
        if (keywordAdded) {
          ending.endingKeywords = [...keywords];
          repaired += 1;
        }
      }
    }
  ...
  // 5) 确保每个 final branch → 其 endings 的边存在且带 label 与推进条件。
  const edgeRequiresForEnding = (ending) => {
    const req = ending?.requires;
    if (req === null || req === undefined || typeof req !== "object" || Array.isArray(req)) return [];
    const cond = {};
    for (const key of ["scene", "entryEvidence", "checkpointGroups", "sanityEventIds", "keyPointIds", "not"]) {
      if (req[key] !== undefined && req[key] !== null) cond[key] = req[key];
    }
    return Object.keys(cond).length > 0 ? [cond] : [];
  };
  for (const [branchId, brEndings] of endingsOfBranch) {
    for (const ending of brEndings) {
      const from = `br:${branchId}`;
      const to = `end:${String(ending.id)}`;
      const label =
        autoBranchIds.has(branchId)
          ? nonEmptyString(ending?.title) || nonEmptyString(ending?.requires?.optionLabel) || nonEmptyString(ending?.optionLabel) || "自动结局"
          : nonEmptyString(ending?.requires?.optionLabel) || nonEmptyString(ending?.optionLabel) || nonEmptyString(ending?.title) || "结局";
      const edgeRequires = edgeRequiresForEnding(ending);
      const existing = plotEdges.find((edge) => String(edge?.from ?? "") === from && String(edge?.to ?? "") === to);
      if (existing !== undefined) {
        if (nonEmptyString(existing?.label).length === 0) {
          existing.label = label;
          repaired += 1;
        }
        if (asArray(existing?.requires).length === 0 && edgeRequires.length > 0) {
          existing.requires = edgeRequires;
          repaired += 1;
        }
      } else {
        plotEdges.push({ from, to, label, requires: edgeRequires });
        repaired += 1;
      }
    }
  }
...
/**
 * 确定性连通性修复：把 preflight 常报的“主线缺少推进边 / 场景点不可达 /
 * 结局场景点孤立”三类问题用确定性规则补边。
 * 返回修复条数。
 *
 * @param {object} flat
 * @param {object} [deepParse]
 * @returns {number}
 */
export function repairDeepParseConnectivity(flat, deepParse = flat?.deepParse) {
  const dp = deepParse;
  if (dp === null || dp === undefined) return 0;
  const keyPoints = asArray(flat?.keyPoints);
  if (keyPoints.length === 0) return 0;
  const plotEdges = Array.isArray(dp.plotEdges) ? dp.plotEdges : (dp.plotEdges = []);
  const edgeKeys = new Set(plotEdges.map((edge) => `${String(edge?.from ?? "")}->${String(edge?.to ?? "")}`));
  let repaired = 0;

  const normTitle = (value) => String(value ?? "").replace(/[：:]\s*$/, "").trim();
  const hasIn = new Set(plotEdges.map((edge) => String(edge?.to ?? "")));
  const hasOut = new Set(plotEdges.map((edge) => String(edge?.from ?? "")));
  const addEdge = (from, to, label, requires = []) => {
    if (from === to) return false;
    const key = `${from}->${to}`;
    if (edgeKeys.has(key)) return false;
    edgeKeys.add(key);
    plotEdges.push({ from, to, label, requires });
    repaired += 1;
    return true;
  };

  // 结局场景点归位：标题/关键词命中 ending 的孤立 kp，从 end 拉一条同义边。
  // 主线/支线连通性缺口由 applyTopologySkeleton 按结构树补充，这里不再做
  // 扁平 order 串链，避免并行幕/可选遭遇/支线被拉成单链。
  const endingByTitle = new Map();
  for (const ending of asArray(dp.endings)) {
    const title = normTitle(ending?.title);
    if (title.length > 0) endingByTitle.set(title, ending);
    for (const word of asArray(ending?.endingKeywords).map((item) => normTitle(item))) {
      if (word.length > 0 && !endingByTitle.has(word)) endingByTitle.set(word, ending);
    }
  }
  for (const kp of keyPoints) {
    const kpTarget = `kp:${String(kp?.id)}`;
    if (hasIn.has(kpTarget) || hasOut.has(kpTarget)) continue;
    const kpTitle = normTitle(kp?.title);
    const ending = endingByTitle.get(kpTitle);
    if (ending !== undefined) {
      addEdge(`end:${String(ending.id)}`, kpTarget, "结局场景点", []);
    }
  }

  // 空前置推进边语义兜底：kp→kp 推进边 requires 为空、且来源场景原文存在
  // “如果…，……（不被/不会/可以就此进入/才可/才能）”这类资格条件时，把条件句
  // 片段作为 entryEvidence 门禁，避免“有资格条件的推进边”被当作无条件直通
  // （如《两面不是人》接受审查→海上盛宴：原文要求“被认定为合作证人”才会受邀）。
  const kpById = new Map(keyPoints.map((kp) => [String(kp?.id ?? ""), kp]));
  const factByHeading = new Map(
    asArray(flat?.scenarioFacts).map((fact) => [normTitle(fact?.heading), fact])
  );
  const hasUsefulRequires = (edge) =>
    asArray(edge?.requires).some(
      (cond) =>
        cond !== null &&
        cond !== undefined &&
        typeof cond === "object" &&
        !Array.isArray(cond) &&
        ["scene", "entryEvidence", "checkpointGroups", "sanityEventIds", "keyPointIds", "branchChoiceIds", "optionLabel", "not"].some(
          (key) => cond[key] !== undefined && cond[key] !== null
        )
    );
  const conditionalEvidence = (text) => {
    const phrases = [];
    const re = /(?:如果|若)([^。；！？]{4,120})/g;
    let match;
    while ((match = re.exec(text)) !== null) {
      const clause = String(match[1]).replace(/\s+/g, "");
      // 只取与资格/信任/邀请/坚持/庇护相关的条件句，避免把战斗、环境等
      // 无关条件句当成“进入下一场景”的门禁证据。
      if (!/(?:信任|邀请|受邀|合作|专案|坚持|庇护|结局|证人|资格)/.test(clause)) continue;
      for (const raw of clause.split(/[，,]/)) {
        const part = raw.trim().replace(/^则/, "").trim();
        if (part.length >= 4 && part.length <= 24 && !/[：:]/.test(part)) phrases.push(part);
      }
    }
  ...
  for (const edge of plotEdges) {
    const from = String(edge?.from ?? "");
    const to = String(edge?.to ?? "");
    if (!from.startsWith("kp:") || !to.startsWith("kp:")) continue;
    // 只处理“资格条件推进边”：label 里有“后前往/后进入/后，/后才/方可/才能”等
    // 条件-结果标记；普通 fallback 边（回到枢纽/前往/关联调查）不在此列。
    if (/hub|return/i.test(from) || /hub|return/i.test(to)) continue;
    if (hasUsefulRequires(edge)) continue;
    const label = normTitle(edge?.label);
    if (!/(?:后前往|后进入|后，|后才|方可前往|才能前往)/.test(label)) continue;
    const fromKp = kpById.get(from.slice(3));
    if (fromKp === undefined) continue;
    const fact = factByHeading.get(normTitle(fromKp?.scene)) ?? factByHeading.get(normTitle(fromKp?.title));
    if (fact === undefined) continue;
    const factText = String(fact?.original ?? "");
    if (!/(?:如果|若)/.test(factText)) continue;
    if (!/(?:不被|不会|可以就此进入|才可|才能|方可)/.test(factText)) continue;
    const evidence = conditionalEvidence(factText);
    if (evidence.length === 0) continue;
    edge.requires = [{ entryEvidence: evidence }];
    repaired += 1;
  }

  // 移除最终分支提取器生成、但未被任何条件/边/结局引用的 kp-final-* 选项节点，
  // 避免它们成为 preflight 的不可达 medium（如《两面不是人》的 kp-final-1/2）。
  {
    const referenced = new Set();
    const collectCond = (cond) => {
      if (cond === null || cond === undefined || typeof cond !== "object" || Array.isArray(cond)) return;
      for (const id of asArray(cond.keyPointIds).map(String)) if (id.length > 0) referenced.add(id);
      for (const id of asArray(cond.not?.keyPointIds).map(String)) if (id.length > 0) referenced.add(id);
    };
    for (const edge of plotEdges) {
      const from = String(edge?.from ?? "");
      const to = String(edge?.to ?? "");
      if (from.startsWith("kp:")) referenced.add(from.slice(3));
      if (to.startsWith("kp:")) referenced.add(to.slice(3));
      for (const cond of asArray(edge?.requires)) collectCond(cond);
    }
    for (const ending of asArray(dp?.endings)) {
      collectCond(ending?.requires);
      for (const cond of asArray(ending?.requiresAnyOf)) collectCond(cond);
    }
    for (const entry of asArray(dp?.keyPointConditions)) {
      collectCond(entry?.requires);
      for (const cond of asArray(entry?.requiresAnyOf)) collectCond(cond);
    }
    for (const entry of asArray(dp?.branchConditions)) {
      collectCond(entry?.requires);
      for (const cond of asArray(entry?.requiresAnyOf)) collectCond(cond);
    }
    const before = flat?.keyPoints?.length ?? 0;
    flat.keyPoints = asArray(flat?.keyPoints).filter(
      (kp) => !/^kp-final-/.test(String(kp?.id ?? "")) || referenced.has(String(kp?.id ?? ""))
    );
    const removed = before - (flat?.keyPoints?.length ?? 0);
    if (removed > 0) repaired += removed;
  }
