...
/**
 * Prompt 用关键点投影：去掉 reachability 等内部门禁字段，避免 schema
 * 扩展改变 LLM 缓存键（确定性字段不进 prompt）。
 */
function promptKeyPoints(value) {
  return asArray(value).map((kp) => {
    if (kp === null || typeof kp !== "object") return kp;
    const clone = { ...kp };
    delete clone.reachability;
    return clone;
  });
}
...
export function buildChunkPrompt(flat, chunk) {
  ...
  return [
    ...
    ``,
    `本场景节点：`,
    JSON.stringify({ keyPoints: promptKeyPoints(chunk.keyPoints), branches: chunk.branches, checkpoints }, null, 2),
    ``,
...
export function buildChunkReviewPrompt(flat, chunk, chunkPart) {
  ...
  return [
    ...
    `medium = 条件过于空泛导致玩家可跳过原文明确要求的检定/事件而直达后续场景、关键点缺少前置导致提前剧透、plotEdges 缺少“骨架中真实存在且原文明确要求”的 requires、把后续节点当作当前节点前置；如果骨架中没有可引用的检定点/关键点来表达该前置，或者原文明确写“KP 可自由裁量/可选遭遇”，不要报 medium（可报 low 建议补充骨架）；`,
    `low = 优化建议。`,
    ...
    ``,
    `本场景节点：`,
    JSON.stringify({ keyPoints: promptKeyPoints(chunk?.keyPoints), branches: asArray(chunk?.branches), checkpoints }, null, 2),
    ``,
    `全部场景速查表：`,
    JSON.stringify(sceneDirectory, null, 2),
    ``,
...
export function buildFinalWiringPrompt(flat, chunks) {
  ...
  return [
    ...
    ``,
    `确定性骨架（可引用 id）：`,
    JSON.stringify({ keyPoints: promptKeyPoints(keyPoints), branches, chunkScenes, checkpointIds }, null, 2),
    ``,
