...
export function buildFinalWiringPrompt(flat, chunks) {
  ...
  return [
    ...
    `约束：`,
    `- 不允许生成 keyPoints。`,
    `- 如果下方骨架已有最终抉择分支（finalChoice=true），必须复用它，不要新增分支；如果没有，才允许在 branches 里补一个最终抉择分支。`,
    `- 最终分支必须有 branchCondition：requires.scene 为其场景，禁止 autoChooseLabel。`,
    ...
    `- 同一最终分支的多个结局用 optionLabel 区分，requires/blockers 互不相同；mutexGroup 相同的结局互斥。`,
    ...
    ``,
    `剧本原文（结局相关部分在最后）：`,
    String(flat?.scenario?.text ?? "").slice(-6000),
  ].join("\n");
}
...
function sanitizeConditionEntry(entry) {
  ...
  return {
    ...entry,
    requires: sanitizeCondition(entry.requires),
    requiresAnyOf: asArray(entry.requiresAnyOf).map((group) => asArray(group).map((cond) => sanitizeCondition(cond))),
  };
}
