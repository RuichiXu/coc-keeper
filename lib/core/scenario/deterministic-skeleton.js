 ...
 */
export function buildCheckpointBranches(flat, keyPointsOverride = null) {
  const branches = [];
  // 检定点只生成“可挂到场景关键点”的分支：facts 等无关键点 section 的检定点
  // 保留在 scenarioCheckpoints（供 coc_check 门禁），但不生成剧情分支，避免出现
  // 无入边、不可达的分支节点。
  const keyPoints =
    Array.isArray(keyPointsOverride)
      ? keyPointsOverride
      : Array.isArray(flat?.keyPoints)
        ? flat.keyPoints
        : [];
  const kpScenes = new Set();
  for (const kp of keyPoints) {
    for (const value of [kp?.title, kp?.scene, kp?.displayName]) {
      const text = nonEmptyString(value);
      if (text.length > 0) kpScenes.add(text);
    }
  }
  for (const checkpoint of Array.isArray(flat?.scenarioCheckpoints) ? flat.scenarioCheckpoints : []) {
    ...
    if (scene.length > 0 && !kpScenes.has(scene)) continue;
...
export function buildDeterministicSkeleton(flat) {
  ...
  return { keyPoints, branches: buildCheckpointBranches(flat, keyPoints) };
}
