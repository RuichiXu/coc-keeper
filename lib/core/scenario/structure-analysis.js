...
export function splitSectionsAtBareHeadingBoundaries(doc, sections) {
  ...
  for (const section of list) {
    ...
    if (tail === undefined || !looksLikeBareHeading(tail.text)) continue;
    // 裸标题后必须紧跟基础标题（如“结局”），才判定为被吞掉的独立小节。
    ...
    if (!looksLikeHeadingBasic(nextLine.text, { requireKeyword: false, strict: false })) continue;
    // 已经以该裸标题独立成节就不重复拆。
    ...
    out.push({
      ...
      kind: section.kind === "chapter" ? "scene" : "scene_event",
      flowRole: section.flowRole ?? "main",
      desc: "",
      level: section.kind === "chapter" ? Math.max(1, Number(section.level) + 1) : Number(section.level) || 1,
