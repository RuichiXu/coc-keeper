...
export function buildStructureWindowPrompt(doc, window, scenarioName, options = {}) {
    ...
    sections: [
      {
        id: "s1",
        title: "原标题（原文标题行，去编号）",
        displayName: "概括名（1-8 字）",
        kind: "chapter|scene|scene_event|facts|module_notes|chapter_notes|rules|appendix|meta",
        level: 1,
        flowRole: "main|side|clue（仅 scene/scene_event，其余为 null）",
        desc: "1-2 句概括",
  ...
  const kindRules = [
    ...
    "- scene_event：场景内的事件节点（接到电话/遭遇/回忆/结局小节等）；",
    "- facts：背景/世界观/人物设定/线索资料等非场景事实；",
    "- module_notes：模组级 KP 须知；",
    "- chapter_notes：章节级 KP 须知；",
    "- rules：特殊机制/规则；",
    "- appendix：附录资料；",
    "- meta：版权/目录/作者信息。",
    "flowRole 只允许在 kind=scene / scene_event 上填：main=主线场景，side=支线可选场景，clue=线索调查场景。",
  ...
  const rules = [
    `你是 CoC 跑团剧本的结构分析师。剧本《${name}》已按行号切成 ${window.total} 个窗口，请只处理第 ${window.index + 1}/${window.total} 个窗口（行号 ${window.startLine}–${window.endLine}）。`,
    "只输出一个 JSON 对象（不要 Markdown 代码块）：",
    JSON.stringify(schemaExample),
    "",
    ...kindRules,
    "约束：",
    `- sections 必须覆盖本窗口全部行号：首个 section 的 startLine 必须是 ${window.startLine}，末个 section 的 endLine 必须是 ${window.endLine}；相邻 section 不得留空档、不得重叠。`,
    "- startLine/endLine 只能是下方文本里出现的行号。",
    "- 层级用 level 表示：1=章/幕/大场景，2=子场景/房间，3=房间内事件；同级的 section 顺序用 order 递增。",
    "- 同名通用标题（如“线索”“信息”）用 displayName 加父级前缀消歧。",
    "- 只输出本窗口内的 sections，不要输出窗口外内容。",
    ...(options.previousSectionTitle
      ? [`- 上一窗口最后一个 section 是「${options.previousSectionTitle}」。如果本窗口第一行不是新标题，它属于上一 section 的正文延续：请把它归入一个与上一 section 同 title 的 section，startLine 填本窗口首行行号。`]
      : []),
    "- title 抄原文标题（去编号），displayName 写概括名，desc 写 1-2 句内容概括。",
    "",
    "本窗口带行号文本：",
...
export function mergeStructureWindowResults(doc, windowResults) {
  ...
  for (const section of merged) {
    ...
    if (stack.length > 0) section.parentId = stack[stack.length - 1].id;
    else section.parentId = null;
    stack.length = level;
