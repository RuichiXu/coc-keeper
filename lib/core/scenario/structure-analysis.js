...
import { inferReachability } from "./topology-skeleton.js";
import {
  FACT_LINE_RE,
  classifyFloor,
  collectKeywords,
...
import {
  ...
  isPageMarkerLine,
  isPageNumberLine,
  looksLikeBareHeading,
  looksLikeEndingHeading,
  looksLikeHeadingBasic,
  looksLikeHeadingForSplit,
...
/**
 * 结构分析候选/窗口切分用的标题判定：编号/附录/结局/冒号/◆ 项目符号标题。
 * 与 scene-facts 的确定性切分共用 heading-rules；此处冒号规则更宽松
 * （不要求地点关键词），因为候选只是交给 LLM 参考，允许略多。
 */
function looksLikeHeading(line) {
  return looksLikeHeadingBasic(line, { requireKeyword: false, strict: false });
}
...
export function cleanScenarioText(text) {
  ...
  const rawLines = sourceLines.map((line) => {
    ...
    return isPageMarkerLine(trimmed) ? "" : trimmed;
  });
  ...
  const nextSignificantText = (fromIndex) => {
    for (let cursor = fromIndex + 1; cursor < rawLines.length; cursor += 1) {
      const nextText = rawLines[cursor].trim();
      if (nextText.length > 0) return nextText;
    }
    return null;
  };

  // 清洗上下文里的标题判定：基础标题总是标题；裸露短标题在后随
  // 标题/页码/页标记/另一裸露短标题时也视为标题（防止相邻短标题互相粘连）。
  const isHeadingForClean = (line, index) => {
    if (looksLikeHeading(line)) return true;
    return looksLikeHeadingForSplit(line, nextSignificantText(index), { requireKeyword: false, strict: false });
  };
  ...
  for (let index = 0; index < rawLines.length; index += 1) {
    ...
    const lineIsHeading = isHeadingForClean(textLine, index);

    // 纯页码行与目录点线行独立成显示行，绝不参与残句合并（避免目录粘连）。
    if (isPageNumberLine(textLine) || isDashedTocLine(textLine)) {
      pushPending();
      displayLines.push({ lineNo, rawFrom: lineNo, rawTo: lineNo, text: textLine });
      displayLineByNo.set(lineNo, displayLines[displayLines.length - 1]);
      pending = null;
      continue;
    }

    if (pending === null) {
      pending = { lineNo, rawFrom: lineNo, rawTo: lineNo, text: textLine, isHeading: lineIsHeading };
      continue;
    }
...
export function buildStructureWindowPrompt(doc, window, scenarioName, options = {}) {
  ...
  const rules = [
    ...
    compactExample,
    `字段：i=局部id；t=原标题(去编号)；n=概括名(1-8字)；k=类型；l=层级(1章幕/2场景房间/3房间内事件)；f=flowRole；d=1-2句概括；s=起始行；e=结束行；o=顺序。`,
    ...
    `level 只表示标题层级，不表示剧情从属：连续多个 sc 应并列，不要为了凑层级把后一个 sc 设成前一个 sc 的子级；se 只挂在它真正发生的 sc 下；ch 作为容器可以包含 sc/se。`,
...
export function mergeStructureWindowResults(doc, windowResults) {
  ...
  // 场景类型父子关系纠偏：level 栈只表达标题层级，不等于剧情从属。
  // - scene 不挂在 scene 下（场景之间不应互为父子）
  // - scene_event 不挂在 scene_event 下
  // - scene_event 只挂在「地点型」scene 下；若父 scene 是结局/尾声等
  //   事件型标题，则事件独立成点，避免“结局2/子夜书房”被归到“结局1”下。
  for (const section of merged) {
    ...
    if (parent.kind === "scene_event") {
      section.parentId = null;
      continue;
    }
    if (parent.kind === "scene" && section.kind === "scene") {
      section.parentId = null;
      continue;
    }
    if (parent.kind === "scene" && section.kind === "scene_event") {
      if (!isPlaceLikeSceneTitle(parent.title)) section.parentId = null;
      continue;
    }
    if (parent.kind !== "chapter" && parent.kind !== "scene") {
      section.parentId = null;
    }
  }
...
function isPlaceLikeSceneTitle(title) {
  const text = String(title ?? "");
  if (/结局|尾声|终局|END|TRUE\s*END|BAD\s*END|GOOD\s*END|开幕|导入|开始|选择/.test(text)) return false;
  return /[层楼房馆宅邸室厅店所社园院窖廊梯]|地窖|书房|卧室|餐厅|厨房|图书|温室|庭院|走廊|房间|一楼|二楼|三楼|四楼/.test(text);
}
 ...
 */
/**
 * 确定性兜底：把“被上一节吞掉的裸露短标题”切回独立 section。
 *
 * 例如原文“胀妇之死”单独一行、下一行是“结局”，窗口 LLM 可能把“胀妇之死”
 * 并入上一节末尾；这里在 applyStructureAnalysis 之前把该行拆成独立的
 * scene_event section，保证高潮事件能成为剧情节点。
 *
 * 只处理 scene/scene_event 节尾的“非基础标题的裸标题”，避免把附录/展示材料
 * 等标题误拆成剧情节点。
 */
export function splitSectionsAtBareHeadingBoundaries(doc, sections) {
  const list = Array.isArray(sections) ? sections : [];
  if (list.length === 0) return list;

  const displayByStart = new Map(doc.displayLines.map((line) => [line.lineNo, line]));
  const indexByLineNo = new Map(doc.displayLines.map((line, index) => [line.lineNo, index]));
  ...
  for (const section of list) {
    ...
    const tail = displayByStart.get(Number(section.endLine));
    if (tail === undefined) continue;
    if (!looksLikeBareHeading(tail.text)) continue;
    // 基础标题（编号/附录/结局/冒号/◆）交给正常结构识别，不在这里拆。
    if (looksLikeHeadingBasic(tail.text, { requireKeyword: false, strict: false })) continue;
    // 裸标题后必须紧跟独立结局标题（“结局”“END 1”等），才判定为被吞掉的
    // 高潮/终局小节；其他基础标题后接裸标题的情形交给窗口 LLM 判断。
    const tailIndex = indexByLineNo.get(tail.lineNo);
    const nextLine = tailIndex !== undefined ? doc.displayLines[tailIndex + 1] : undefined;
    if (nextLine === undefined) continue;
    if (!looksLikeEndingHeading(nextLine.text)) continue;
    // 已经以该裸标题独立成节就不重复拆。
    if (String(section.title ?? "").trim() === tail.text && Number(section.startLine) === tail.lineNo) continue;

    const prevLine = tailIndex > 0 ? doc.displayLines[tailIndex - 1] : undefined;
    const prevEnd = prevLine !== undefined ? prevLine.rawTo : tail.lineNo - 1;
    if (prevEnd < Number(section.startLine)) continue;

    section.endLine = prevEnd;
    out.push({
      id: `bare-${tail.lineNo}`,
      title: tail.text,
      displayName: tail.text,
      kind: "scene_event",
      flowRole: section.flowRole ?? "main",
      desc: "",
      level: Number(section.level) || 1,
      parentId: section.parentId ?? null,
      startLine: tail.lineNo,
      endLine: tail.lineNo,
      page: section.page ?? null,
      order: Number(section.order) + 0.5,
      note: "",
    });
  }

  out.sort((a, b) => Number(a.startLine) - Number(b.startLine) || Number(a.order) - Number(b.order));
  return out;
}

export function applyStructureAnalysis(flat, doc, result) {
  const sections = computeSectionTexts(doc, splitSectionsAtBareHeadingBoundaries(doc, result.sections ?? []));
  ...
  for (const section of sections) {
    if (!SCENE_KINDS.has(section.kind)) continue;
    // scene 本身就是场景；scene_event 才继承最近的地点型 scene 作为场景归属。
    const parentScene = section.kind === "scene" ? null : nearestAncestorOfKind(section, "scene");
    ...
    keyPoints.push({
      ...
      kind: section.kind,
      flowRole: section.flowRole,
      reachability: inferReachability({
        kind: section.kind,
        flowRole: section.flowRole,
        parentId: section.kind === "scene_event" && parentScene !== null ? String(parentScene.id ?? "") : null,
      }, new Map([[String(parentScene?.id ?? ""), { flowRole: parentScene?.flowRole ?? "main" }]])),
      parentId: parentKpId,
  ...
  // 显式检定点：从场景类与 facts section 的 ownLines 提取；
  // facts 里的知识/调查类检定（如“通过【历史】或【神秘学】检定可知…”）同样入列。
  flat.scenarioCheckpoints = extractCheckpointsFromBlocks(
    sections
      .filter((section) => SCENE_KINDS.has(section.kind) || section.kind === "facts")
