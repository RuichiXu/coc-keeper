 ...
 * 将剧本原文按标题行切分为场景块。
 * 标题识别（优先级从高到低）：
 * 1. 编号标题：`5.6 约翰的书斋`、`附录 1.2.2 对手`、`7.9.5 可选遭遇：…`（不含句读、无点线目录特征）
 * 2. 独立结局行（如“结局1（BE）”）
 * 3. 严格冒号标题：以冒号结尾的短行，且包含章节/地点类标题词；正文里被换行截断的
 *    残句（如“下内容：”“行检查：”）不满足此条件，会被并入正文。
...
export function splitScenarioSections(text) {
  ...
  const isNumberedHeading = (line) => {
    if (!/^[0-9]+(?:\.[0-9]+)*\s+\S/.test(line)) return false;
    if (/\.{3,}/.test(line)) return false; // 目录点线
    if (/[。！？；，]/.test(line)) return false; // 带句读的是正文（如年份事件行）
    if (/^[0-9]{4}\s*[年月日]/.test(line)) return false; // 1892 年… 是正文
    const rest = line.replace(/^[0-9]+(?:\.[0-9]+)*\s+/, "");
    if (rest.length < 2) return false; // 排除“1 轮”“9 8”这类正文短行
    if (/\s/.test(rest)) return false; // 排除“18 祭司之主 轩 辕 …”这类表格行
    return line.length <= 40;
  };
  const isEndingHeading = (line) => /^(?:结局\s*[一二三四五六七八九十\d]*|END\s*\d*|BAD\s*END|GOOD\s*END|TRUE\s*END)(?:[（(].*[）)])?$/i.test(line) && line.length <= 20;
  const isAppendixHeading = (line) => /^附录(?:\s*[0-9]+(?:\.[0-9]+)*)?(?:\s+\S+)?$/.test(line) && line.length <= 30 && !/\.{3,}/.test(line);
  const isColonHeading = (line) => {
    if (!/^[^。！？…，,；;]{1,12}[：:]\s*$/.test(line) || line.length > 13) return false;
    // 只有明显像标题的短冒号行才切分；正文里的长冒号句交给换行合并还原整句。
    return /(?:章|幕|节|附录|房间|卧室|书房|走廊|客厅|餐厅|厨房|旅舍|酒店|酒馆|教堂|警局|医院|工厂|码头|仓库|宅邸|城堡|图书馆|书店|商店|古董|森林|湖|桥|塔|楼|层|地区|镇|村|城市|广场|剧院|报社|事务所)/.test(line);
  };
  const isHeading = (line) => isNumberedHeading(line) || isAppendixHeading(line) || isEndingHeading(line) || isColonHeading(line);
