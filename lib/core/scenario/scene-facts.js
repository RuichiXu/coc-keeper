...
import { looksLikeHeadingForSplit } from "./heading-rules.js";
...
// 战斗/对抗类括号检定不进“门禁检定点”，交给战斗与对抗规则处理。
const CHECKPOINT_EXCLUDED_SKILLS = new Set([
  "斗殴", "格斗", "力量", "POW", "射击", "投掷", "闪避",
]);
 ...
 * 将剧本原文按标题行切分为场景块。
 * 标题识别（优先级从高到低）：
 * 1. 编号标题：`5.6 约翰的书斋`、`附录 1.2.2 对手`、`7.9.5 可选遭遇：…`
 * 2. 独立结局行（如“结局1（BE）”）
 * 3. 严格冒号标题：以冒号结尾的短行，且包含章节/地点类标题词
 * 4. 项目符号式章节标题：`- ◆ 舞动的皮`
 * 5. 裸露短标题（如“胀妇之死”）：仅在下一非空行是标题/页码/页标记/另一裸露短标题时切分，
 *    避免把正文残句误切成节。
 * 非标题行先做换行合并：上一行不以句末标点结尾时，与下一行拼接，还原被 PDF 拆开的整句。
 *
 * @param {string} text
...
export function splitScenarioSections(text) {
  ...
  const nextSignificant = (index) => (index + 1 < rawLines.length ? rawLines[index + 1] : null);
  const isHeading = (line, index) =>
    looksLikeHeadingForSplit(line, nextSignificant(index), { requireKeyword: true, strict: true });

  const sections = [];
  let current = { heading: "导入/前置", lines: [], startLine: 0, endLine: 0 };
  let pending = "";
  ...
  const flushPending = () => {
    if (pending.length > 0) {
      ...
      current.endLine = pendingStartLine;
    }
    pending = "";
    pendingStartLine = 0;
  };
  ...
  for (let index = 0; index < rawLines.length; index += 1) {
    ...
    if (isHeading(line, index)) {
      ...
      current = { heading: line.replace(/[：:]\s*$/, ""), lines: [], startLine: lineNo, endLine: lineNo };
      continue;
    }
    ...
    const canMerge = !/[。！？；：…]$/.test(pending) && pending.length + line.length <= 120;
    if (canMerge) {
  ...
  flushPending();
  if (current.lines.length > 0 || current.heading !== "导入/前置") {
    if (current.startLine === 0) current.startLine = 1;
    if (current.endLine === 0) current.endLine = rawLines.length;
    sections.push(current);
  }

  return sections.filter((section) => section.lines.length > 0).map((section) => ({
    ...section,
    startLine: section.startLine || 1,
    endLine: section.endLine || section.startLine || rawLines.length,
  }));
}
...
/** 从括号式技能前的片段识别难度（“极难难度的”“困难成功的”“普通成功”）。 */
function matchDifficultyBeforeBracket(prefix) {
  const match = /(普通|困难|极难|极限|常规)\s*(?:难度)?\s*(?:成功|通过)?\s*(?:的)?\s*$/.exec(prefix);
  return match ? match[1] : null;
}

/**
 * 括号式检定点：`【侦查】检定`、`【医学】或【生物学】检定`、
 * `困难成功的【教育】检定`、`一个成功的【侦查】或【幸运】发现`。
 * 战斗/对抗类（斗殴/格斗/力量/POW/射击/投掷/闪避）不在门禁检定点体系内，排除。
 */
function parseBracketCheckpoints(line) {
  const found = [];
  if (!/【[^】]{1,12}】/.test(line)) return found;
  const hasGateAnchor = /(?:检定|鉴定|判定|检测)/.test(line);
  const hasDiscovery = /发现/.test(line) && /(?:成功|困难|极难|普通|极限|常规)/.test(line);
  if (!hasGateAnchor && !hasDiscovery) return found;
  ...
  while ((match = bracketRe.exec(line)) !== null) {
    ...
    const known = SKILL_ALIASES[raw] ?? SKILL_ALIASES[base] ?? (KNOWN_SKILLS.has(raw) ? raw : null);
    const isKnown = known !== null && (KNOWN_SKILLS.has(known) || known === raw);
    // 未知技能只在明确检定锚点行纳入（避免“在【书房】发现…”这类名词误提）。
    if (!isKnown && !hasGateAnchor) continue;
    const skill = isKnown ? known : raw;
    if (CHECKPOINT_EXCLUDED_SKILLS.has(skill) || CHECKPOINT_EXCLUDED_SKILLS.has(base) || CHECKPOINT_EXCLUDED_SKILLS.has(raw)) continue;
    const difficulty = matchDifficultyBeforeBracket(line.slice(0, match.index)) ?? "普通";
    found.push({
      skill,
...
function parseCheckpointLine(line) {
  ...
  // ⑤ 括号式检定点：【侦查】检定 / 【医学】或【生物学】检定 / 困难成功的【教育】检定。
  found.push(...parseBracketCheckpoints(line));
  ...
  // ⑥ SC 表达式：“SC 1/1d6”“SC 1d4/1d10”。这类 SAN 结算通常没有“理智检定”
  // 字样，旧解析会漏掉，导致直面沸核/外出阶段等 SC 在运行时完全缺席。
  const scRe = /SC\s*(\d+(?:d\d+)?)\s*[\/／]\s*(\d+(?:d\d+)?)/gi;
  let sc;
  while ((sc = scRe.exec(line)) !== null) {
    const sanLoss = `${sc[1].toLowerCase()}/${sc[2].toLowerCase()}`;
    const exists = found.some(
      (item) => item.skill === "理智" && String(item.trigger ?? "").trim() === line.trim()
    );
    if (!exists) pushSan(line, sanLoss);
  }

  // ⑦ “承受 1/1d4 的伤害”：把伤害表达式挂到本行解析出的检定点上，
  // 供 .ra 结算时按成功/失败自动扣 HP（成功扣 1，失败扣 1d4）。
  const dmg = line.match(/承受\s*(\d+)\s*[\/／]\s*(\d+(?:d\d+)?)\s*的伤害/);
  if (dmg !== null) {
    const damage = `${dmg[1]}/${dmg[2].toLowerCase()}`;
    for (const item of found) {
      if (item.skill !== "理智" && item.damage === undefined) item.damage = damage;
    }
  }
 ...
 */
/**
 * 从文本块（{heading, lines}）提取显式检定点。
 * 与 extractCheckpoints 的规则完全一致，但接受调用方切好的块，
 * 供结构分析管线按 section 的 ownLines 精确提取。
 *
 * @param {Array<{heading: string, lines: string[]}>} blocks
 * @returns {Array<object>}
 */
export function extractCheckpointsFromBlocks(blocks) {
  const out = [];
  const seen = new Set();

  for (const section of blocks) {
    const heading = nonEmptyHeading(section?.heading);
    ...
    for (const item of byTriggerSkill.values()) {
      ...
      out.push({
        ...
        scene: heading,
        floor: classifyCheckpointFloor(heading, lines.join("\n")),
...
function nonEmptyHeading(value) {
  return String(value ?? "").replace(/[：:]\s*$/, "").trim();
}

export function extractCheckpoints(text) {
  const sections = splitScenarioSections(text);
  return extractCheckpointsFromBlocks(
...
export function inferSceneFromText(text, facts) {
  ...
  // 无事实卡命中时不做场景推断：不再使用旧版《墨渊》楼层硬编码兜底
  // （“一层门厅/宅邸外围/三层书房”会被写进《对流》等其它剧本）。
  return null;
}
