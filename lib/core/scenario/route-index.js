...
const LEADING_TRAVEL_VERB_RE =
  /^(?:你|我|他|她|它|你们|他们|看到|看见|穿过|进入|来到|走向|沿着|沿|发现|出现|绕过|踏上|离开|前往|到达|通过|靠近|走进|回到|返回|去到|去往)+/;

/**
 * 提取文本中的地点候选短语：取后缀词及其前最多 6 个汉字，
 * 再剥掉“你穿过/进入/来到”等转移动词，留下“下层阀门舱/旋梯”这类地点词。
 * @param {string} text
 * @returns {string[]}
 */
export function extractRoomLikeTerms(text) {
  ...
  while ((match = ROOM_LIKE_SUFFIX_RE.exec(source)) !== null) {
    ...
    const start = Math.max(0, end - 6 - match[0].length);
    let term = source.slice(start, end);
    term = term.replace(LEADING_TRAVEL_VERB_RE, "").trim();
    // “下层阀门舱和旋梯”这类并列地点短语按并列连词拆开。
