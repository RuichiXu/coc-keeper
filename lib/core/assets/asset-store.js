...
export class AssetStore {
  ...
  /**
   * 宽松查找：按规范化姓名匹配（忽略空格/引号/间隔号等变体）。
   * 用于人物卡去重，避免「伊芙琳·“伊芙”·默瑟」与「伊芙琳·默瑟」重复建卡。
   */
  findByNameLoose(kind, name) {
    const normalize = (value) => String(value)
      .toLowerCase()
      .replace(/[\s·"'“”‘’.。,_-]/g, "");
    const target = normalize(name);
    return this.list(kind).find((asset) => normalize(asset.name) === target) ?? null;
  }
}
