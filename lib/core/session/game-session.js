...
export class GameSession {
  ...
  syncFromFlat(flat) {
    ...
    // 同名重建的新场次没有 core：必须把内存里旧场次的运行时账本全部清空，
    // 否则旧场次的 eventLog / plot / clues / trace 会污染新场次（game-delete 后 game-create）。
    if (flat.core === undefined || flat.core === null) {
      this.trace = [];
      this.eventLog = new EventLog();
      this.plot = new PlotGraph();
      this.clues = new ClueGraph();
    }
    // flags 只存在 core.world 中：旧 flat 没有该字段时，先重置为空 Map，
    // 避免同名重建后旧场次的世界 Flag 被带进新场次。
    const flagsSource = flat.flags ?? flat.core?.world?.flags;
    if (flagsSource === undefined) {
      this.world.flags = new Map();
    }
    this.world.hydrate({
      id: this.id,
      ...
      tasks: flat.tasks ?? [],
      ...(flagsSource !== undefined ? { flags: flagsSource } : {}),
