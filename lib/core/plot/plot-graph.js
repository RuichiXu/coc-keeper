...
export class PlotGraph {
   ...
   * @param {object|null} [world=null] - WorldState；提供时应用节点 consequences
   * @returns {object}
   */
  completeNode(id, outcome = "", world = null) {
    const node = this.findNode(id);
    if (!node) throw new Error(`剧情节点 ${id} 不存在`);
    node.status = "completed";
    node.completedAt = new Date().toISOString();
    node.outcome = outcome;
    if (world !== null && world !== undefined && node.consequences !== null && node.consequences !== undefined) {
      applyConsequences(world, node.consequences);
      node.consequencesApplied = true;
    }
  ...
  /**
   * 对所有已 completed 但尚未应用后果的节点，应用其 consequences 到 WorldState。
   * 幂等：每个节点只应用一次。
   * @param {object} world - WorldState 实例
   * @returns {string[]} 新应用后果的节点 id
   */
  applyCompletedConsequences(world) {
    const applied = [];
    for (const node of this.nodes) {
      if (node.status !== "completed" || node.consequencesApplied === true) continue;
      if (node.consequences === null || node.consequences === undefined) continue;
      applyConsequences(world, node.consequences);
      node.consequencesApplied = true;
  ...
  syncFromStory(story) {
    ...
    for (const kp of keyPoints) {
      ...
      // 默认后果：揭示关键点 → 写世界 Flag；后续 D 阶段可生成更丰富的后果。
      if (node.consequences === null || node.consequences === undefined) {
        node.consequences = { setFlags: { [`kp:${kp.id}:revealed`]: true } };
      }
    }
    ...
    for (const branch of branches) {
      ...
      if (node.consequences === null || node.consequences === undefined) {
        node.consequences = { setFlags: { [`branch:${branch.id}:chosen`]: String(branch.chosen ?? "") } };
      }
    }
    ...
    for (const branch of branches) {
      for (const option of branch.options ?? []) {
        ...
        // 结局节点：选项指向结局/END 时，建 ending 节点。
        if (/(?:结局|END|TE|BE|GE|TRUE)/i.test(leadsTo)) {
          const endId = `end:${branch.id}:${this.nodes.filter((n) => n.type === "ending" && n.branchId === branch.id).length + 1}`;
          let endingNode = this.findNode(endId);
          if (endingNode === undefined) {
            endingNode = this.addNode({
              id: endId,
              title: leadsTo,
              type: "ending",
              scene: branch.scene ?? "",
          ...
          endingNode.title = leadsTo;
          endingNode.branchId = branch.id;
          endingNode.optionLabel = option.label ?? "";
          endingNode.chosen = String(branch.chosen ?? "");
          endingNode.status =
            String(branch.chosen ?? "").length > 0 && branch.chosen === option.label ? "completed" : "inactive";
          if (endingNode.consequences === null || endingNode.consequences === undefined) {
            endingNode.consequences = { setFlags: { [`ending:${branch.id}:${leadsTo}`]: true } };
          }
          this.addEdge(`br:${branch.id}`, endingNode.id, { label: option.label ?? "", requires: [] });
        }
      }
    }
   ...
   * @returns {Array<object>}
   */
  completedEndingNodes() {
    return this.nodes.filter((node) => node.type === "ending" && node.status === "completed");
  }
...
/**
 * 把剧情后果应用到 WorldState（flags / 线索 / 实体状态 / 结局倾向）。
 * @param {object} world - WorldState 实例
 * @param {object|null} consequences
 * @returns {string[]} 已应用的后果描述
 */
export function applyConsequences(world, consequences) {
  if (consequences === null || consequences === undefined) return [];
  const applied = [];

  for (const [key, value] of Object.entries(consequences.setFlags ?? {})) {
    world.setFlag(key, value);
    applied.push(`flag:${key}`);
  }
  for (const key of consequences.clearFlags ?? []) {
    world.removeFlag(key);
    applied.push(`clear-flag:${key}`);
  }
  for (const clue of consequences.discoverClues ?? []) {
    const clueId = String(clue?.clueId ?? clue ?? "");
    if (clueId.length > 0) {
      world.discoverClue({ clueId, method: clue?.method ?? "plot", character: clue?.character ?? "", isCritical: clue?.isCritical === true });
      applied.push(`clue:${clueId}`);
    }
  }
  for (const change of consequences.setEntityState ?? []) {
    const name = String(change?.name ?? "");
    if (name.length === 0) continue;
    const entity = world.findEntityByName(name);
    if (entity !== undefined) {
      world.updateEntity(entity.id, { state: String(change?.state ?? "") });
      applied.push(`entity:${name}`);
    }
  }
  if (consequences.endingInfluence !== undefined && consequences.endingInfluence !== null) {
    world.setFlag("ending:influence", consequences.endingInfluence);
    applied.push("ending-influence");
  }
