...
const HARD_RULES = [
  ...
  "【硬性规则】",
  "1. 明骰纪律（最高优先级）：主持对话中你绝不调用 coc_roll。只要玩家动作存在失败可能，就调用 coc_check 工具登记检定门禁（skill=技能名，difficulty=难度，action=绑定动作文本）。系统会自动渲染 [团检：技能] [.ra技能] 提示，并在玩家完成检定前阻止该动作推进剧情。推荐选项需要检定时，action 填该选项原文；战斗/追逐/剧透等不应展示选项的场景，不要列推荐选项，等玩家行动后再调用 coc_check。",
