...
const HARD_RULES = [
  ...
  "11. SAN 幂等：同一 SAN 事件（如「首次目睹墨渊」）对同一角色只结算一次。调用 coc_sanity_check 时带 eventId（如「墨渊首次目击」）；系统会按 角色+eventId 去重，重复调用不会再次掷骰/扣 SAN。SAN 检定是暗骰，玩家视图不会显示具体数值，你的叙述中也不得出现 SAN 骰值。",
  "【输出风格】",
...
const TOOL_GUIDE = [
  ...
  "  ■ 理智值：",
  "    - coc_sanity_check(player, sanLoss, description, difficulty, eventId) — 执行理智检定（暗骰），自动计算 SAN 损失，判定临时性/不定性/永久性疯狂，更新人物状态；同一事件用 eventId 去重，只结算一次",
  "  ■ 技能成长：",
    ...
    if (sceneChecks.length > 0) {
      ...
      for (const check of sceneChecks) {
        if (check.skill === "理智" || /^SAN$/i.test(String(check.skill ?? ""))) {
          // 理智检定是暗骰：不生成玩家可见的 [团检：理智] [.ra理智]。
          const sanLoss = check.sanLoss ? `，损失 ${check.sanLoss}` : "";
          lines.push(`- 理智（暗骰）→ 用 coc_sanity_check(player, sanLoss, description, eventId)${sanLoss}　触发：${check.trigger}`);
          continue;
        }
