...
export function isRollVisible(roll, layer) {
  ...
  if (roll?.kind === "secret") return false;
  // SAN/理智检定是暗骰：玩家只应看到损失结果，不应看到出目与成功等级。
  const skill = String(roll?.skill ?? "");
  const label = String(roll?.label ?? "");
  if (skill === "理智" || /^SAN$/i.test(skill) || /理智/.test(label)) return false;
  return true;
}
