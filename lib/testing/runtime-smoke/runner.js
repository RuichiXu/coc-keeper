...
export function safeGameId(id) {
  // 与 shared/helpers.js 保持一致：保留中文，避免含中文 id 的场次读不到/删不掉。
...
export function createHarness(opts = {}) {
  ...
  const deps = {
    ...
    callLlmApi,
    enableSemanticSettlement: opts.enableSemanticSettlement === true,
...
export function makeLiveKpStream(opts = {}) {
  ...
  return async (options) => {
    const system = [options.system ?? "", ...parts].join("\n");
    const baseOptions = {
      ...options,
      system,
      model,
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      result = await callLlmApi(dataDir, retryMessages, {
        ...baseOptions,
