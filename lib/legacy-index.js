...
function stateFile(dataDir, gameId) {
	return join(dataDir, `${safeGameId(gameId)}.json`);
}
...
function saveState(dataDir, state) {
	...
	writeFileSync(stateFile(dataDir, state.id), JSON.stringify(state, null, 2), "utf8");
	return state;
}
...
function parseCharacters(text) {
	...
	if (trimmed.startsWith("[") || trimmed.startsWith("{")) {
		try {
			...
			if (list.every((item) => typeof item === "object" && item !== null)) return list;
		} catch {
			// 继续尝试行式解析
		}
	}
	...
	for (const raw of String(text).split(/\r?\n/)) {
		...
		if (nameMatch !== null) {
			current = { name: nameMatch[1].trim(), occupation: "", stats: {}, skills: {}, inventory: [], notes: "" };
		...
		if (key === "物品" || key === "装备" || key === "道具" || key === "随身物品") {
			current.inventory = value.split(/[、,，;；]/).map((s) => s.trim()).filter((s) => s.length > 0);
			continue;
		}
		const num = Number(value);
		const statKey = STAT_ALIASES[key] ?? key.toUpperCase();
		if (Number.isFinite(num) && (STAT_KEYS.has(statKey) || Object.prototype.hasOwnProperty.call(STAT_ALIASES, key))) {
...
function normalizeCharacter(raw, index) {
	...
	return {
		id: typeof raw.id === "string" && raw.id.length > 0 ? raw.id : `pc-${Date.now().toString(36)}-${index}`,
		name: nameText,
	...
	defs['coc_import'] = defineTool({
		...
		async execute(args, execCtx) {
			...
			if (kind === "auto") {
				...
				// 优先检测剧本/剧情关键词（含 NPC 标记）
				else if (/(剧本|模组|场景|scenario|module|chapter|故事|剧情|角色|npc|地点|物品|线索|秘密|调查)/i.test(head)) kind = "scenario";
			...
			let nameText = args.name ?? (args.source === "file" ? args.filePath.split(/[\\/]/).pop() : kind);
			let characters = 0;
			let keyPoints = 0;
			let branches = 0;
			let entities = 0;
...
${content}`;
				...
				const normalized = parsed.map((raw, index) => normalizeCharacter(raw, index));
	...
	// ── 自动导入内置规则（首次启动时自动加载） ──
	if (BUILTIN_RULES_TEXT && BUILTIN_RULES_TEXT.length > 0) {
		try {
