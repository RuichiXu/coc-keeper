			...
			let nameText =
				typeof args.name === "string" && args.name.trim().length > 0
					...
					: args.source === "file"
						...
						: kind;
			// 粘贴文本导入且未命名时，尝试从正文提取标题（【剧本】xxx / 标题：xxx / 第一行）
			if (nameText === kind && kind === "scenario") {
				const titleMatch = /【剧本】\s*([^\n|｜]+)|^#+\s*(.+)$|^(?:剧本|模组)[：:]\s*(.+)$/m.exec(content);
				if (titleMatch !== null) {
					nameText = (titleMatch[1] ?? titleMatch[2] ?? titleMatch[3]).trim() || kind;
				} else {
					const firstLine = content.split(/\r?\n/).map((l) => l.trim()).find((l) => l.length > 0);
					if (firstLine !== undefined && firstLine.length <= 40) nameText = firstLine;
				}
			}
			let characters = 0;
			let keyPoints = 0;
...
${content}`;
	...
	ctx.systemPrompt.section({
		...
		text: [
			...
			"2. 检定纪律：明骰由玩家发送 .ra 指令触发，你只输出 [团检：技能名] 提示，不要调用 coc_roll；暗骰调用 coc_roll_secret。严禁自行编造骰点或成功档位。",
...
//#region LLM 调用辅助（无 LLM 服务时的 API 直连与流式组装）
