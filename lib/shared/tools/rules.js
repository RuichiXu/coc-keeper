...
import { resolveRaTarget } from "../chat/check-command.js";

/**
 * 姓名查找归一化：LLM 可能把全角引号写成半角（伊芙琳·"伊芙"·默瑟），
 * 统一去掉引号与空白后再比较，避免“未找到调查员”阻断 SAN 结算。
 * @param {string} name
 * @returns {string}
 */
 ...
 */
function findCharacterLoose(world, name) {
  const target = normalizeLookupName(name);
  if (target.length === 0) return undefined;
  return (world.characters ?? []).find(
    (character) => normalizeLookupName(character.name) === target
  );
}
...
export function createRuleToolDefs(deps) {
  ...
  defs.push({
      ...
      execute(args) {
        ...
        const pc = session.world.findCharacter(playerName) ?? findCharacterLoose(session.world, playerName);
        if (pc === undefined) throw new Error(`未找到调查员「${playerName}」`);
        // 事件必须使用角色卡上的规范名，否则 applyEvent 会按精确名找不到角色，
        // 导致 SAN 变更不落库（半角/全角引号差异时尤其明显）。
        const characterName = pc.name ?? playerName;
        ...
        if (eventId.length > 0) {
          const existing = settled.find(
            (entry) =>
              normalizeLookupName(entry.player) === normalizeLookupName(characterName) &&
        ...
        // C-1：SAN 结算进入 EventLog（SanitySettled），flat.sanitySettled 由 WorldState 投影。
        const extraEvents = [];
        if (eventId.length > 0) {
          extraEvents.push({
            type: "SanitySettled",
        ...
        commitSession(deps, gameId, session, flat, [...result.events, ...extraEvents]);
  ...
  defs.push({
      ...
      parameters: {
        ...
        armor: { type: "string", description: "防御方护甲值（数字或骰式，如「1」「1d10」），从伤害中减免" },
      },
      output: {
        schema: {
          type: "object",
      ...
      execute(args) {
        ...
        return {
          result: `${result.hit ? "命中" : result.dodgeSuccess ? "被闪避" : "未命中"}${result.hit ? `，造成 ${result.damage} 点伤害` : ""}${result.majorWound ? "（重伤）" : ""}${result.dying ? "（濒死）" : ""}${result.dead ? "（死亡）" : ""}`,
          hit: result.hit,
  ...
  defs.push({
      ...
      execute(args) {
        ...
        // 当前技能值必须与 .ra 判定同源：人物卡优先，其次内置规则默认值。
        // 旧实现把未写卡技能当 0，导致“检定按默认 10、成长却从 0 起算”。
        const { target } = resolveRaTarget(flat, playerName, skillName);
        const currentValue = target ?? 0;

        // CoC 7e：只有成功使用过的技能才能在冒险结束成长。
        // 本局有该技能掷骰记录且全部失败时，确定性拒绝，不再把失败技能写进人物卡。
        const baseSkill = skillName.replace(/[（(][^）)]*[）)]/g, "").trim();
        const uses = (Array.isArray(flat.skillUseLog) ? flat.skillUseLog : [])
          .filter((entry) => String(entry?.skill ?? "").replace(/[（(][^）)]*[）)]/g, "").trim() === baseSkill);
        if (uses.length > 0 && !uses.some((entry) => entry?.passed === true)) {
          return {
            player: playerName,
            skill: skillName,
            before: currentValue,
            ...
            rolled: 0,
            grown: false,
            result: `「${skillName}」本局只有失败/未成功记录，不能成长（CoC 7e：只有成功使用过的技能才能在冒险结束时成长）`,
          };
        }
