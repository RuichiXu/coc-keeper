...
export async function runDeepParseGenerationLoop(flat, deps, onProgress = () => {}) {
  ...
  for (let round = 1; round <= maxRounds; round += 1) {
    ...
    try {
      const chunkResults = [];
      for (const item of chunkPrompts) {
        const raw = await callLlmText(deps, item.prompt + repairText, 6000);
        const parsed = parseDeepParseResult(raw, flat);
        chunkResults.push(parsed.deepParse);
      }
      const finalRaw = await callLlmText(deps, finalPrompt + repairText, 8000);
