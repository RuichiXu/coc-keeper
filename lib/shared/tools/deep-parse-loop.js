 ...
 */
function summarizeFinalSkeleton(flat) {
  const finalBranches = asArray(flat?.branches).filter(
...
export function buildReviewPrompt(flat, deepParse, preflightIssues) {
  ...
  const focusDraft = {
    branches: asArray(deepParse?.branches),
  ...
  return [
    ...
    `最终分支骨架：${JSON.stringify(summarizeFinalSkeleton(flat).finalBranches)}`,
    `关键点参考（id:标题）：${summarizeFinalSkeleton(flat).keyPointRef}`,
    `场景标题清单（scene 门控只能从这里选；出现次数>1 说明多条路线共用该标题）：${JSON.stringify(summarizeFinalSkeleton(flat).sceneHeadings)}`,
  ...
  for (let round = 1; round <= opts.maxRounds; round += 1) {
    ...
    if (round === 1) {
      ...
      const finalPart = {
        ...
        keyPoints: [],
        branches: asArray(currentDraft.deepParse?.branches),
