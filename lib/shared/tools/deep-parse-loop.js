...
/**
 * 当确定性门禁（preflight + 规则审校）完全干净时，跳过 LLM 语义审校与分块审校。
 * 目的：减少 LLM 调用轮次与 token 消耗，同时降低语义审校波动对结果的影响。
 * 只跳过“干净”情况；任何 high/medium 问题仍会触发语义审校兜底。
 */
export function shouldSkipSemanticReview(preflight, ruleReview) {
  const pf = preflight ?? { high: 0, medium: 0 };
  const rule = ruleReview ?? { high: 0, medium: 0 };
  return (pf.high ?? 0) === 0 && (pf.medium ?? 0) === 0 && (rule.high ?? 0) === 0 && (rule.medium ?? 0) === 0;
}
...
export async function runDeepParseGenerationLoop(flat, deps, onProgress = () => {}, loopOptions = {}) {
  ...
  for (let round = 1; round <= opts.maxRounds; round += 1) {
    ...
    currentReview = null;
    // 优化：确定性门禁完全干净时，跳过 LLM 语义审校与分块审校。
    // 这样干净剧本只烧“分块生成 + 最终分支生成”两段 LLM，既省 token，
    // 也降低审校模型波动对最终产物造成的差异。
    const skipSemantic = opts.runReview === false && opts.runChunkReview === false
      ? true
      : shouldSkipSemanticReview(currentPreflight, currentRuleReview);
    if (opts.runReview && !skipSemantic) {
      try {
    ...
    if (opts.runChunkReview && !skipSemantic) {
      if (round === 1) {
        // 第 1 轮：审校所有有内容的分块。
