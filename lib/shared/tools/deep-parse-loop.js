...
function makeScore(preflight, ruleReview, review, reviewGate) {
  ...
  const high = preflight.high + rule.high + reviewHigh;
  const medium = preflight.medium + rule.medium + reviewMedium;
  const low = preflight.low + rule.low + reviewLow;
  return {
...
export async function runDeepParseGenerationLoop(flat, deps, onProgress = () => {}, loopOptions = {}) {
  ...
  for (let round = 1; round <= opts.maxRounds; round += 1) {
    ...
    const score = makeScore(currentPreflight, currentRuleReview, currentReview, opts.reviewGate);
