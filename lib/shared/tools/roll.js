 ...
 */
export function createRollToolDefs(deps) {
  const defs = [];
  defs.push({
      name: "coc_roll",
      description:
      ...
      execute(args) {
        ...
        // 明骰结果对所有人可见：直接写入会话日志（kind: "roll"），
        // 玩家视图与 KP 视图都会渲染这行，LLM 不需要在叙述中重复骰值。
        const rollLine = `🎲【明骰】${outcome.player ? `${outcome.player} ` : ""}${outcome.label ? `· ${outcome.label} ` : ""}${renderRollLine(outcome)}`;
