...
export function findKeyPoint(flat, id) {
  return flat.keyPoints.find((k) => k.id === id) ?? null;
}

export function branchSceneMatches(branch, currentScene) {
  const branchScene = String(branch?.scene ?? "").trim();
  const scene = String(currentScene ?? "").trim();
  if (branchScene.length === 0) return true;
  if (scene.length === 0) return false;
  return scene.includes(branchScene) || branchScene.includes(scene);
}

/**
 * 从分支场景名派生出可检索的关键词：完整场景名 + 冒号后的尾部场景名。
 * 例如「房间3：设施总控室」→ [房间3：设施总控室, 设施总控室, 总控室]。
 */
export function branchSceneKeywords(branchScene) {
  const source = String(branchScene ?? "").trim();
  if (source.length === 0) return [];
  const parts = source.split(/[：:]/).map((part) => part.trim()).filter((part) => part.length > 0);
  const words = new Set([source, ...parts]);
  // 从长场景名中再截取尾部短词（如「设施总控室」→「总控室」），
  // 兼容 KP 叙述里只说“总控室”而不写完整场景名的情况。
  for (const part of parts) {
    if (part.length >= 4) {
      const tail = part.slice(-3);
...
/**
 * 最终分支场景门禁：满足任一即视为“已抵达过最终场景”：
 * 1. 当前场景匹配；
 * 2. SceneChanged 事件历史里切入过该场景；
 * 3. 对话日志（KP/玩家可见文本）中出现过该场景名——用于旧存档恢复与
 *    KP 漏调 coc_scene 的场景（Codex r3 存档暴露）。
 * 玩家在最终场景做出选择后，KP 可能在后续场景/尾声才补登记 reached/choose，
 * 不应再因“当前场景不匹配”拒绝；但开局无任何场景证据时仍会被拦下。
 */
export function branchSceneVisited(flat, branch) {
  const branchScene = String(branch?.scene ?? "").trim();
  if (branchScene.length === 0) return true;
  const currentScene = String(flat?.currentScene ?? "").trim();
  if (branchSceneMatches(branch, currentScene)) return true;
  const sceneChanges = Array.isArray(flat?.events) ? flat.events : [];
  const inHistory = sceneChanges.some((event) => {
    if (event?.type !== "SceneChanged") return false;
    const to = String(event?.to ?? "").trim();
    if (to.length === 0) return false;
    return to.includes(branchScene) || branchScene.includes(to);
  });
  if (inHistory) return true;
  const keywords = branchSceneKeywords(branchScene);
  if (keywords.length === 0) return false;
  const log = Array.isArray(flat?.log) ? flat.log : [];
  return log.some((entry) => {
    if (entry?.kind !== "kp" && entry?.kind !== "user") return false;
    const text = String(entry?.text ?? "");
    return keywords.some((keyword) => text.includes(keyword));
  });
}

export function isFinalBranch(branch) {
  return branch?.finalChoice === true || String(branch?.id ?? "").startsWith("br-final");
}

/**
 * 解析分支：优先按 id 精确匹配；模型常把分支标题当 id（如《对流》），
 * 依次按标题精确、标题包含、id 前缀兜底。多个候选时返回 null 并带候选说明。
 */
export function resolveBranch(flat, id) {
  const input = String(id ?? "").trim();
  if (input.length === 0) return { branch: null, error: "分支 ID 为空" };
  const branches = flat.branches ?? [];
  const exact = branches.find((b) => String(b.id ?? "") === input);
  if (exact !== undefined) return { branch: exact, error: null };
  const byTitle = branches.filter((b) => String(b.title ?? "").trim() === input);
  if (byTitle.length === 1) return { branch: byTitle[0], error: null };
  if (byTitle.length > 1) {
    return {
      branch: null,
      error: `分支标题「${input}」匹配到多个分支：${byTitle.map((b) => `${b.id}「${b.title}」`).join("、")}，请改用 id`,
    };
  }
  const byTitleIncludes = branches.filter((b) => String(b.title ?? "").includes(input));
  if (byTitleIncludes.length === 1) return { branch: byTitleIncludes[0], error: null };
  if (byTitleIncludes.length > 1) {
    return {
      branch: null,
      error: `分支「${input}」匹配到多个候选：${byTitleIncludes.map((b) => `${b.id}「${b.title}」`).join("、")}，请改用完整标题或 id`,
    };
  }
  const byIdPrefix = branches.filter((b) => String(b.id ?? "").startsWith(input));
  if (byIdPrefix.length === 1) return { branch: byIdPrefix[0], error: null };
  if (byIdPrefix.length > 1) {
    return {
      branch: null,
      error: `分支 id 前缀「${input}」匹配到多个：${byIdPrefix.map((b) => b.id).join("、")}，请使用完整 id`,
    };
  }
  return { branch: null, error: `分支 ${input} 不存在（可用 coc_branch list 查看全部 id 与标题）` };
}
      ...
      execute(args) {
        ...
        if (args.action === "add") {
          ...
          if (args.type === "keypoint") {
            ...
            const resolved = resolveBranch(flat, args.branchId);
            if (resolved.error !== null) throw new Error(resolved.error);
            const target = resolved.branch;
            const item = args.item ?? {};
            if (item.title !== undefined) target.title = String(item.title);
            if (item.desc !== undefined) target.desc = String(item.desc);
            if (item.scene !== undefined) target.scene = String(item.scene);
          ...
          if (args.type === "keypoint") {
            ...
            flat.branches = flat.branches.filter((b) => b.id !== resolved.branch.id);
            if (flat.branches.length === before) throw new Error(`分支 ${args.branchId} 不存在`);
            if (flat.currentBranchId === resolved.branch.id) flat.currentBranchId = "";
            message = `已删除分支 ${resolved.branch.id}`;
          }
          ...
          if (isFinalBranch(branch) && !branchSceneVisited(flat, branch)) {
            throw new Error(`尚未抵达最终分支场景「${branch.scene ?? "（未限定）"}」（当前场景：${flat.currentScene || "未设定"}），请先推进剧情到该场景再标记 reached`);
          }
          branch.reached = true;
          ...
          const branch = resolved.branch;
          if (isFinalBranch(branch) && !branchSceneVisited(flat, branch)) {
            throw new Error(`尚未抵达最终分支场景「${branch.scene ?? "（未限定）"}」（当前场景：${flat.currentScene || "未设定"}），请先推进剧情到该场景再记录选择`);
          }
          const option = branch.options.find((o) => o.label === args.optionLabel);
          if (option === undefined) {
            const optionList = (branch.options ?? []).map((o) => o.label).join(" / ");
            throw new Error(`分支「${branch.title}」没有选项「${args.optionLabel}」，可用选项：${optionList || "（无）"}`);
          }
