...
import { findRecentHpLoss, recordHpLoss } from "../chat/damage-ledger.js";
...
export function createStateToolDefs(deps) {
  ...
  defs.push({
      ...
      execute(args) {
        const gameId = gameIdOf(args, deps.defaultGame);
        const { session, flat } = loadSession(deps, gameId);
        // 单人团缺省人物名：模型偶尔只传 game 不传 name，此时唯一调查员即为目标。
        let name = typeof args.name === "string" && args.name.trim().length > 0 ? args.name.trim() : "";
        if (name.length === 0) {
          const characters = session.world.characters ?? [];
          if (characters.length === 1) {
            name = characters[0].name;
          } else if (characters.length === 0) {
            throw new Error("当前没有人物卡（可用 coc_character add 或 coc_import 导入）");
          } else {
            throw new Error("存在多个人物，请用 name 指定要更新的人物");
          }
        }
        const pc = session.world.findCharacter(name);
        if (pc === undefined) {
          throw new Error(`人物「${name}」不存在（可用 coc_character add 或 coc_import 导入）`);
        }

        const changed = [];
        const updates = {};
        if (typeof args.hp === "number") {
          const currentHp = Number(pc.hp ?? pc.stats?.HP ?? 10);
          const requestedLoss = currentHp - args.hp;
          if (requestedLoss > 0 && args._auto !== true) {
            const recent = findRecentHpLoss(flat, name, requestedLoss);
            if (recent !== null) {
              return {
                game: gameId,
                message: `「${pc.name}」HP 未重复扣减：${requestedLoss} 点损失已由系统自动结算（${recent.source}）。若这是新的独立伤害，请用不同数值或稍后再试。`,
                character: pc,
              };
            }
          }
          updates.hp = args.hp;
          changed.push(`HP→${args.hp}`);
          if (requestedLoss > 0) recordHpLoss(flat, name, requestedLoss, "kp-tool");
        }
        ...
        if (changed.length === 0) {
          return { game: gameId, message: `「${pc.name}」未提供可更新字段，状态未变更`, character: pc };
        }
