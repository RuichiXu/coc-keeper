...
import { checkKey } from "../chat/check-gates.js";
import {
  loadSession,
  commitSession,
  gameIdOf,
  ...
  defs.push({
      ...
      execute(args) {
        ...
        if (skill === "理智" || /^SAN$/i.test(skill)) {
          throw new Error("理智检定是暗骰，请调用 coc_sanity_check（带 eventId），不要登记玩家明骰门禁");
        }
        const difficulty =
        ...
        const exists = pending.some((check) => checkKey(check) === key);
        if (!exists) {
          pending.push({
        ...
        return {
          ...
          hidden,
          line,
          message: exists ? "该检定门禁已存在，未重复登记" : "已登记玩家明骰门禁",
        };
      },
  });
