...
import { ASSET_KINDS, KNOWLEDGE_LAYERS, buildKnowledgeView, compileByPattern, computeStoryFrontier, detectDeadEndScenes, ensureScenarioContract, extractCheckpoints, extractSceneFacts, extractStoryIntro, mergeDeepParseDraft, normalizeDeepParse, normalizeScenarioContract, sanitizeMetaText, storyFrontierText, validateDeepParse } from "../../core/index.js";
    ...
    if (req.method === "POST") {
      ...
      if (path === "/coc-api/deep-parse") {
        ...
        if (body.action === "confirm") {
          ...
          const normalized = normalizeDeepParse(flat.deepParse);
        ...
        if (body.deepParse !== undefined && body.deepParse !== null) {
          const normalized = normalizeDeepParse(body.deepParse);
          const issues = validateDeepParse(normalized, flat);
          if (issues.length > 0) {
            return sendJson(res, 400, { ok: false, error: `深度剧情解析校验未通过：${issues.join("；")}` });
          }
