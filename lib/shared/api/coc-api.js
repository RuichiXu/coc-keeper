...
import { ASSET_KINDS, KNOWLEDGE_LAYERS, applyStructureEdits, buildKnowledgeView, canonicalizeDeepParse, compileByPattern, computeStoryFrontier, detectDeadEndScenes, ensureScenarioContract, extractCheckpoints, extractSceneFacts, extractStoryIntro, mergeDeepParseDraft, normalizeScenarioContract, pickBetterDeepParse, sanitizeMetaText, storyFrontierText, validateDeepParse } from "../../core/index.js";
import { loadSession, commitSession, nowIso } from "../tools/helpers.js";
import { callLlmApi } from "../chat/index.js";
import { applyStoryPreset, exportStoryFixture, STORY_PRESET_NAMES } from "../testing/story-presets.js";
  ...
  try {
    if (req.method === "GET") {
      ...
      if (path === "/coc-api/contract") {
        ...
        return sendJson(res, 200, {
          ...
          data: {
            game: existing.id,
            status: existing.scenarioContract?.status ?? "none",
            source: existing.scenarioContract?.source ?? "none",
            reviewed: existing.scenarioContract?.reviewed === true,
            contract: existing.scenarioContract ?? null,
          },
        });
      }
      if (path === "/coc-api/deep-parse") {
        // 剧情网络与剧本资产绑定：优先按 asset 读取；按 game 读取时，
        // 若场次已绑定剧本资产且资产带有深度解析，则返回资产版本（同一剧本
        // 在任意场次看到同一张网络），资产无解析时才回退场次旧数据。
        const assetIdParam = params.get("asset")?.trim() ?? "";
        if (assetIdParam.length > 0) {
          const asset = deps.assetStore.load(ASSET_KINDS.SCENARIO, assetIdParam);
          if (asset === null) return sendJson(res, 404, { ok: false, error: "剧本资产不存在：" + assetIdParam });
          const dp = asset.deepParse ?? null;
          return sendJson(res, 200, {
            ok: true,
            data: {
              asset: asset.id,
              ...
              game: null,
              status: dp?.status ?? asset.deepParseStatus ?? "none",
              source: dp?.source ?? "none",
              reviewed: dp?.reviewed === true,
              deepParse: dp,
              // 资产自有剧情结构：解析时落盘的完整网络，与具体场次无关。
              // 前端始终以此为图模型基础；场次只叠加执行状态。
              assetKeyPoints: Array.isArray(asset.keyPoints) ? asset.keyPoints : [],
              assetBranches: Array.isArray(asset.branches) ? asset.branches : [],
              assetCheckpoints: Array.isArray(asset.checkpoints) ? asset.checkpoints : [],
            },
          });
        }
        const existing = deps.persistence.load(key);
        if (existing === null) return sendJson(res, 200, { ok: true, data: null });
        if (typeof existing.scenarioId === "string" && existing.scenarioId.length > 0) {
          const asset = deps.assetStore.load(ASSET_KINDS.SCENARIO, existing.scenarioId);
          if (asset !== null) {
            // 资产与场次各有一份草稿时读取更优者，保证解析页不再展示被资产
            // 旧稿压住的较新场次数据（写入方向由 confirm/setup 的择优保护）。
            const dp = pickBetterDeepParse(asset.deepParse ?? null, existing.deepParse ?? null);
            if (dp !== null) {
              const fromAsset = dp === asset.deepParse;
              return sendJson(res, 200, {
                ok: true,
                data: {
                  asset: asset.id,
                  assetName: asset.name ?? asset.id,
                  game: existing.id,
                  status: dp?.status ?? (fromAsset ? asset.deepParseStatus : existing.deepParse?.status) ?? "none",
                  source: dp?.source ?? "none",
                  reviewed: dp?.reviewed === true,
                  deepParse: dp,
                  assetKeyPoints: Array.isArray(asset.keyPoints) ? asset.keyPoints : [],
        ...
        return sendJson(res, 200, {
          ...
          data: {
            game: existing.id,
            status: existing.deepParse?.status ?? "none",
            source: existing.deepParse?.source ?? "none",
            reviewed: existing.deepParse?.reviewed === true,
            deepParse: existing.deepParse ?? null,
          },
        });
      }
      if (path === "/coc-api/page-image") {
        const page = Math.max(1, Number(params.get("page") ?? 1));
        const assetIdParam = params.get("asset")?.trim() ?? "";
        let scenarioId = assetIdParam;
        if (scenarioId.length === 0) {
          const existing = deps.persistence.load(key);
          scenarioId = existing?.scenarioId ?? "";
        }
        if (scenarioId.length === 0) return sendJson(res, 404, { ok: false, error: "缺少剧本资产 id" });
        const pageFile = join(
          deps.assetStore.rootDir,
          ASSET_KINDS.SCENARIO,
          scenarioId,
          "pages",
          `page-${String(page).padStart(3, "0")}.png`
        );
        if (!existsSync(pageFile)) return sendJson(res, 404, { ok: false, error: `页图不存在：${pageFile}` });
        const image = readFileSync(pageFile);
      ...
      if (path === "/coc-api/player-view") {
        ...
        const limit = limitParam === null ? null : Math.min(1000, Math.max(1, Number(limitParam) || 20));
        const log = limit === null
          ? (existing.log ?? []).slice(after)
          : (existing.log ?? []).slice(after, after + limit);
        ...
        return sendJson(res, 200, {
          ...
          data: {
            ...
            logHasMore: limit !== null && after + log.length < (existing.log ?? []).length,
    ...
    if (req.method === "POST") {
      ...
      if (path === "/coc-api/contract") {
        ...
        const key = deps.stateKey(gameId);
        const flat = deps.persistence.load(key);
        if (flat === null) return sendJson(res, 400, { ok: false, error: "游戏数据不存在" });
        const { session } = loadSession(deps, gameId);

        if (body.action === "confirm") {
          if (flat.scenarioContract === null || flat.scenarioContract === undefined) {
            return sendJson(res, 400, { ok: false, error: "契约不存在，无法确认生效" });
          }
          flat.scenarioContract.status = "confirmed";
          flat.scenarioContract.reviewed = true;
          commitSession(deps, gameId, session, flat);
          return sendJson(res, 200, { ok: true, data: { status: "confirmed" } });
        }

        if (body.contract !== undefined && body.contract !== null) {
          const contract = normalizeScenarioContract(body.contract);
          contract.status = body.status === "confirmed" ? "confirmed" : "draft";
          contract.source = String(body.source ?? contract.source ?? "manual");
          contract.reviewed = contract.status === "confirmed" || body.reviewed === true;
          flat.scenarioContract = contract;
          commitSession(deps, gameId, session, flat);
          return sendJson(res, 200, { ok: true, data: { status: contract.status, source: contract.source } });
        }

        return sendJson(res, 400, { ok: false, error: "缺少 contract 或 action=confirm" });
      }
      if (path === "/coc-api/deep-parse") {
        const gameId = body.game?.trim() || deps.defaultGame;
        const key = deps.stateKey(gameId);
        ...
        // 剧情网络与剧本资产绑定：场次绑定了剧本资产时，深度解析写入资产
        // 并镜像回当前场次（运行期保持一致）；显式编辑其他资产时只写资产，
        // 不污染当前场次。未绑定资产时维持场次级旧行为。
        const assetId = String(body.asset ?? flat.scenarioId ?? "").trim();
        let asset = null;
        if (assetId.length > 0) {
          asset = deps.assetStore.load(ASSET_KINDS.SCENARIO, assetId);
          if (asset === null) return sendJson(res, 404, { ok: false, error: "剧本资产不存在：" + assetId });
        }
        const ownAsset = asset === null || assetId === String(flat.scenarioId ?? "");
        // 合并/校验骨架：编辑自家资产时用场次数据（保留运行时字段），
        // 编辑其他资产时用资产自身结构（场次数据完全不参与）。
        const mergeBase = ownAsset
          ? flat
          : asset !== null
            ? {
                keyPoints: (asset.keyPoints ?? []).map((kp) => ({ ...kp })),
                branches: (asset.branches ?? []).map((branch) => ({ ...branch })),
                scenario: { name: asset.name },
                scenarioId: asset.id,
              }
            : flat;
        // 资产与场次各有一份 deepParse 时，取门禁更优的一方，避免旧资产覆盖新场次草稿。
        const sourceDp = ownAsset
          ? (asset !== null
              ? pickBetterDeepParse(asset.deepParse ?? null, flat.deepParse ?? null)
              : flat.deepParse)
          : (asset !== null ? asset.deepParse : flat.deepParse);

        if (body.action === "confirm") {
          if (sourceDp === null || sourceDp === undefined) {
            return sendJson(res, 400, { ok: false, error: "深度剧情解析不存在，无法确认生效" });
          }
          const normalized = canonicalizeDeepParse(sourceDp).deepParse;
          const issues = validateDeepParse(normalized, mergeBase);
          if (issues.length > 0) {
            return sendJson(res, 400, { ok: false, error: `深度剧情解析校验未通过：${issues.join("；")}` });
          }
          sourceDp.status = "confirmed";
          sourceDp.reviewed = true;
          if (asset !== null) {
            deps.assetStore.update(ASSET_KINDS.SCENARIO, assetId, { deepParse: sourceDp, deepParseStatus: "confirmed" });
          }
          if (ownAsset) {
            flat.deepParse = sourceDp;
            commitSession(deps, gameId, session, flat);
          }
          return sendJson(res, 200, { ok: true, data: { status: "confirmed", asset: assetId.length > 0 ? assetId : null } });
        }

        if (body.deepParse !== undefined && body.deepParse !== null) {
          const normalized = canonicalizeDeepParse(body.deepParse).deepParse;
          const issues = validateDeepParse(normalized, mergeBase);
          if (issues.length > 0) {
            return sendJson(res, 400, { ok: false, error: `深度剧情解析校验未通过：${issues.join("；")}` });
          }
          const merged = mergeDeepParseDraft(mergeBase, normalized);
          const nextDp = {
            status: body.status === "confirmed" ? "confirmed" : "draft",
            source: String(body.source ?? "manual"),
            reviewed: body.status === "confirmed" || body.reviewed === true,
            generatedAt: new Date().toISOString(),
            ...(body.quality !== undefined && body.quality !== null ? { quality: body.quality } : {}),
            ...merged.deepParse,
          };
          nextDp.graphIssues = detectDeadEndScenes(
            { keyPoints: mergeBase.keyPoints ?? [], branches: mergeBase.branches ?? [] },
            nextDp
          );
          if (asset !== null) {
            const assetPatch = { deepParse: nextDp, deepParseStatus: nextDp.status };
            if (ownAsset) {
              // mergeDeepParseDraft 会把新增节点并入 flat；按 id 合并回资产，保证
              // 新建场次也能拿到同一套 keyPoints/branches（防回退）。
              assetPatch.keyPoints = mergeByIdList(asset.keyPoints ?? [], flat.keyPoints ?? []);
              assetPatch.branches = mergeByIdList(asset.branches ?? [], flat.branches ?? []);
            } else {
              assetPatch.keyPoints = mergeBase.keyPoints ?? [];
              assetPatch.branches = mergeBase.branches ?? [];
            }
            deps.assetStore.update(ASSET_KINDS.SCENARIO, assetId, assetPatch);
          }
          if (ownAsset) {
            flat.deepParse = nextDp;
            commitSession(deps, gameId, session, flat);
          }
          return sendJson(res, 200, { ok: true, data: { status: nextDp.status, source: nextDp.source, asset: assetId.length > 0 ? assetId : null } });
        }

        return sendJson(res, 400, { ok: false, error: "缺少 deepParse 或 action=confirm" });
      }
      if (path === "/coc-api/structure") {
        const gameId = body.game?.trim() || deps.defaultGame;
        const key = deps.stateKey(gameId);
        const flat = deps.persistence.load(key);
        if (flat === null) return sendJson(res, 400, { ok: false, error: "游戏数据不存在" });
        const { session } = loadSession(deps, gameId);
        if (!Array.isArray(body.sections)) return sendJson(res, 400, { ok: false, error: "缺少 sections 数组" });
        const stats = applyStructureEdits(flat, body.sections);
        commitSession(deps, gameId, session, flat);
        return sendJson(res, 200, { ok: true, data: { stats, structure: flat.scenarioStructure } });
      }
      if (path === "/coc-api/debug") {
        const gameId = body.game?.trim() || deps.defaultGame;
        const key = deps.stateKey(gameId);
        const flat = deps.persistence.load(key);
        if (flat === null) return sendJson(res, 400, { ok: false, error: "游戏数据不存在" });
        if (body.action === "removeGate") {
          const gateId = String(body.gateId ?? "");
          flat.pendingChecks = (flat.pendingChecks ?? []).filter((gate) => String(gate.id ?? "") !== gateId);
          syncAndSave(deps, gameId, flat);
          return sendJson(res, 200, { ok: true, data: { pendingChecks: flat.pendingChecks } });
        }
        if (body.action === "clearGates") {
          flat.pendingChecks = [];
          flat.pendingChoice = null;
          syncAndSave(deps, gameId, flat);
          return sendJson(res, 200, { ok: true, data: { pendingChecks: [] } });
        }
        if (body.action === "clearChoice") {
          flat.pendingChoice = null;
          syncAndSave(deps, gameId, flat);
          return sendJson(res, 200, { ok: true, data: { pendingChoice: null } });
        }
        if (body.action === "clearResolved") {
          flat.resolvedChecks = [];
          syncAndSave(deps, gameId, flat);
          return sendJson(res, 200, { ok: true, data: { resolvedChecks: [] } });
        }
        if (body.action === "gotoPreset") {
          const presetName = String(body.preset ?? "").trim();
          if (!STORY_PRESET_NAMES.includes(presetName)) {
            return sendJson(res, 400, { ok: false, error: `未知剧情点预设：${presetName}（可用：${STORY_PRESET_NAMES.join(" / ")}）` });
          }
          applyStoryPreset(flat, presetName);
          syncAndSave(deps, gameId, flat);
          return sendJson(res, 200, {
            ok: true,
            data: {
        ...
        if (body.action === "exportFixture") {
          return sendJson(res, 200, { ok: true, data: exportStoryFixture(flat) });
        }
        if (body.action === "dumpCore") {
          return sendJson(res, 200, {
            ok: true,
            data: {
              eventLog: flat.core?.eventLog?.entries ?? [],
              plot: {
                nodes: flat.core?.plot?.nodes ?? [],
                edges: flat.core?.plot?.edges ?? [],
              },
              flags: flat.core?.world?.flags ?? {},
              world: {
                currentScene: flat.core?.world?.currentScene ?? flat.currentScene ?? "",
                pendingChecks: flat.core?.world?.pendingChecks ?? flat.pendingChecks ?? [],
                sanitySettled: flat.core?.world?.sanitySettled ?? flat.sanitySettled ?? [],
              },
            },
          });
        }
        return sendJson(res, 400, { ok: false, error: "未知 debug action" });
      }
      ...
      if (path === "/coc-api/game-setup") {
        ...
        if (scenarioId.length > 0) {
          ...
          // 场景事实卡与显式检定点：始终从剧本原文确定性重算（规则为主）。
          try {
            flat.scenarioFacts = extractSceneFacts(flat.scenario.text ?? "");
            flat.scenarioCheckpoints = extractCheckpoints(flat.scenario.text ?? "");
          } catch (e) {
            console.error("[coc-api/game-setup] 场景事实/检定点提取失败:", e.message);
          }

          // 剧本执行契约：优先复用导入时生成的草拟（LLM/确定性），否则现场草拟。
          try {
            const cachedContract = asset.scenarioContract ?? null;
            flat.scenarioContract = ensureScenarioContract(cachedContract, flat);
            if (flat.scenarioContract.status === undefined) {
              flat.scenarioContract.status = "draft";
              flat.scenarioContract.source = "deterministic";
            }
            if (flat.scenarioContract.reviewed === undefined) {
              flat.scenarioContract.reviewed = flat.scenarioContract.status === "confirmed";
            }
          } catch (e) {
            console.error("[coc-api/game-setup] 剧本执行契约草拟失败:", e.message);
          }

          // D-2：深度剧情解析草稿随剧本资产复用；无草稿时保持 none。
          // 防御性择优：若 flat 已有更优草稿（门禁更好/更新），不因资产回退。
          flat.deepParse = pickBetterDeepParse(flat.deepParse ?? null, asset.deepParse ?? null);
        }
        ...
        // 开场前提关键点（scene=导入，如“委托到来”）随开场白确定性揭示。
        for (const kp of flat.keyPoints ?? []) {
          if (kp?.revealed === true) continue;
          if (String(kp?.scene ?? "").trim() === "导入") kp.revealed = true;
        }
        deps.persistence.save(key, flat);
        ...
        if (body.action === "list") {
          ...
          const data = items.map((asset) => {
            ...
            if (kind === ASSET_KINDS.SCENARIO) {
              ...
              // 深度解析与结构大字段由专门接口按需读取，列表只留状态标记。
              delete item.deepParse;
              delete item.keyPoints;
              delete item.branches;
              delete item.entities;
              delete item.scenarioFacts;
      ...
      if (path === "/coc-api/kp-command") {
        ...
        try {
          ...
          if (Array.isArray(parsed)) calls = parsed.filter((call) => call !== null && typeof call === "object" && typeof call.name === "string");
        } catch {
          // 模型偶尔会在 JSON 前后加说明文字：截取第一个 [ ... ] 再解析。
...
function mergeByIdList(existingList, incomingList) {
  const out = [];
  const seen = new Set();
...
function listGames(deps) {
  ...
  for (const file of readdirSync(gamesDir)) {
    ...
    const key = join("games", file);
    const flat = deps.persistence.load(key);
    if (flat === null) continue;
    // 自愈：文件内 id 必须等于文件名（stateKey 由 id 推导），
    // 否则前端按下拉框 id 加载/删除时会落到不存在的文件（本地曾出现
    // verify-对流.json / verify-盲愚之眼.json 内部 id 均为 verify 的情况）。
    if (String(flat.id ?? "") !== fileId) {
      flat.id = fileId;
      if (flat.core !== null && typeof flat.core === "object" && flat.core.id !== undefined) {
        flat.core.id = fileId;
      }
      deps.persistence.save(key, flat);
    }
...
function stateDigestOf(flat) {
  return {
    ...
    debug: {
      busy: flat.busy === true,
      structure: flat.scenarioStructure ?? null,
      ...
      pendingChecks: Array.isArray(flat.pendingChecks) ? flat.pendingChecks : [],
      pendingChoice: flat.pendingChoice ?? null,
      resolvedChecks: Array.isArray(flat.resolvedChecks) ? flat.resolvedChecks : [],
      ...
      skippedChecks: Array.isArray(flat.skippedChecks) ? flat.skippedChecks : [],
      firedNightEventIds: Array.isArray(flat.firedNightEventIds) ? flat.firedNightEventIds : [],
      endingReached: flat.endingReached === true,
      endedAt: flat.endedAt ?? null,
      frontier: storyFrontierText(computeStoryFrontier(flat)),
      facts: {
        flags: flat.core?.world?.flags ?? flat.flags ?? {},
        discoveredClues: flat.core?.world?.discoveredClues ?? flat.discoveredClues ?? [],
        entityStates: (flat.entities ?? []).filter((entity) => String(entity?.state ?? "").length > 0).map((entity) => `${entity.name}:${entity.state}`),
        time: flat.time ?? "",
      },
      events: (flat.core?.trace ?? []).slice(-40).reverse(),
    },
  };
}
...
export function createCocApiHandler(deps) {
  return (req, res) => handleCocApi(req, res, deps);
}
