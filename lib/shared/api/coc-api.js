  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (path === "/coc-api/deep-parse") {
        ...
        const sourceDp = asset !== null && (asset.deepParse ?? null) !== null ? asset.deepParse : flat.deepParse;
        ...
        if (body.action === "confirm") {
          ...
          const issues = validateDeepParse(normalized, flat);
          if (issues.length > 0) {
        ...
        if (body.deepParse !== undefined && body.deepParse !== null) {
          ...
          const merged = mergeDeepParseDraft(flat, normalized);
          const nextDp = {
          ...
          if (asset !== null) {
            ...
            // mergeDeepParseDraft 会把新增节点并入 flat；同步回资产，保证新建
            // 场次也能拿到同一套 keyPoints/branches（按 id 合并防回退）。
            if (assetId === String(flat.scenarioId ?? "")) {
