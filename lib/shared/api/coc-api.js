    ...
    if (req.method === "POST") {
      ...
      if (path === "/coc-api/deep-parse") {
        ...
        if (body.action === "confirm") {
          ...
          const normalized = canonicalizeDeepParse(flat.deepParse).deepParse;
          const issues = validateDeepParse(normalized, flat);
          if (issues.length > 0) {
            return sendJson(res, 400, { ok: false, error: `深度剧情解析校验未通过：${issues.join("；")}` });
          }
          flat.deepParse.status = "confirmed";
          flat.deepParse.reviewed = true;
          commitSession(deps, gameId, session, flat);
          return sendJson(res, 200, { ok: true, data: { status: "confirmed" } });
        }
        ...
        if (body.deepParse !== undefined && body.deepParse !== null) {
          ...
          commitSession(deps, gameId, session, flat);
          return sendJson(res, 200, { ok: true, data: { status: flat.deepParse.status, source: flat.deepParse.source } });
        }
