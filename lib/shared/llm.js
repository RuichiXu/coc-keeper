...
export function loadLlmConfig(dataDir) {
...
export function toOpenAiMessages(messages, system) {
  ...
  for (const message of messages ?? []) {
    // 统一内容为块数组：兼容部分调用方直接传字符串 content 的旧用法，
    // 避免在发出 HTTP 请求前因 .find 不是函数而失败（Codex r6 语义裁决 0 请求根因）。
    ...
    const converted = {
      ...
      content: textOf(contentBlocks) || (calls.length > 0 ? null : ""),
    };
    if (calls.length > 0) {
...
export async function callLlmApi(dataDir, messages, options = {}) {
  ...
  // 部分 OpenAI 兼容推理模型（如 deepseek-v4-flash）默认会把大量
  // completion 预算花在 reasoning tokens 上导致 content 为空。
  // 需要时由调用方显式传入 reasoningEffort（如 "low"），模型无关透传。
  if (options.reasoningEffort !== undefined && options.reasoningEffort !== null && String(options.reasoningEffort).length > 0) {
