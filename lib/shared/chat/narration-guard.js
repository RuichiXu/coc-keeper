...
/**
 * 检测叙述正文里是否混入了原始工具调用标签（模型把工具调用当正文输出）。
 * 采用结构化标签扫描，不依赖具体协议字符串。
 * @param {string} text
 * @returns {boolean}
 */
export function containsRawToolCallSyntax(text) {
  const source = String(text ?? "");
  let index = source.indexOf("<");
  while (index !== -1) {
    const end = source.indexOf(">", index + 1);
    if (end === -1) break;
    const tag = source.slice(index + 1, end).trim().toLowerCase();
    const name = tag.split(/\s+/)[0] ?? "";
    if (
      name.includes("tool") ||
      name.includes("invoke") ||
      name.includes("parameter") ||
      name.startsWith("coc_")
    ) {
      return true;
    }
