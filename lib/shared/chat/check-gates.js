...
export function mergeCheckGates(existing, incoming) {
  ...
  for (const check of incoming ?? []) {
    ...
    if (existingIndex !== undefined) {
      ...
      old.target = target;
      if ((old.checkpointId === undefined || old.checkpointId === null || String(old.checkpointId).length === 0) &&
          check.checkpointId !== undefined && check.checkpointId !== null && String(check.checkpointId).length > 0) {
        old.checkpointId = check.checkpointId;
      }
      continue;
    }
