...
// ── 通用判定（C-4：统一由 Core Trigger Engine 提供） ──────
...
export { evaluatePrerequisites, evaluateRequiresAnyOf };
...
export function findFinalBranch(flat) {
  ...
  // deepParse 结局显式挂接的分支（如《对流》br-final-1）即使选项 leadsTo
  // 不含“结局/END”字样，也应视为最终分支。
  const endingBranchIds = new Set(
    asArray(flat?.deepParse?.endings)
      .map((ending) => String(ending?.branchId ?? ""))
      .filter((id) => id.length > 0)
  );
  const candidates = branches.filter((branch) => {
    if (branch?.finalChoice === true) return true;
    if (String(branch?.id ?? "").startsWith("br-final")) return true;
    if (endingBranchIds.has(String(branch?.id ?? ""))) return true;
    return asArray(branch?.options).some((option) => /(?:结局|END|TE|BE|GE|TRUE)/i.test(String(option?.leadsTo ?? "")));
  });
  if (candidates.length === 0) return null;
  return candidates.find((branch) => /(?:最终|结局|仪式|抉择)/.test(String(branch?.title ?? ""))) ?? candidates[0];
}
...
export function draftKeyPointPrerequisites(kp, flat) {
  ...
  // 场景节点（scene/scene_event）不能用“发生在同一场景内”的检定点作为前置：
  // 那会形成“必须先完成场景内检定才能进入该场景”的死锁。
  const isSceneNode = kp?.kind === "scene" || kp?.kind === "scene_event";
  const sameSceneCheckpoint = (check) => {
    if (!isSceneNode) return false;
    const checkScene = String(check?.scene ?? "").trim();
    return checkScene.length > 0 && (scene === checkScene || scene.includes(checkScene) || checkScene.includes(scene));
  };
  ...
  for (const term of terms) {
    for (const check of checkpoints) {
      ...
      if (sameSceneCheckpoint(check)) continue;
  ...
  for (const term of terms) {
    const hits = checkpoints.filter((check) => {
      ...
      if (sameSceneCheckpoint(check)) return false;
