 ...
 * @param {object} input
 * @returns {Promise<{ok:boolean, verdicts?: Map, raw?: string, error?: string}>}
 */
export async function judgeSettlements(deps, { settlements, playerText, narration, currentScene, pcName }) {
  ...
  if (candidates.length === 0) return { ok: true, verdicts: new Map() };
  ...
  try {
    ...
    if (parsed === null) return { ok: false, error: "empty-parse", raw };
