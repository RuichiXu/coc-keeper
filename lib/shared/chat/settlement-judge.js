...
export async function judgeSettlements(deps, { settlements, playerText, narration, currentScene, pcName }) {
  ...
  try {
    let messages = buildSettlementJudgeMessages(prompt);
    let response = await callLlmApi(deps.dataDir, messages, {
    ...
    let raw = (response.blocks ?? [])
      ...
      .join("");
    let parsed = parseSettlementJudgeOutput(raw);
    // 一次空解析重试：要求模型只输出 JSON，降低“空解析→回退固定字段”的扰动（r8 唯一回退）。
    if (parsed === null) {
      messages.push({
        role: "assistant",
        content: raw.trim().length > 0
          ? [{ type: "text", text: raw.slice(0, 400) }]
          : [{ type: "text", text: "（空输出）" }],
      });
      messages.push({
        role: "user",
        content: [{
          type: "text",
          text: '（系统）你没有输出可解析的 JSON。请只输出 JSON，格式：{"verdicts":[{"id":"结算点id","happened":true,"subject":"pc","confidence":0.9,"evidence":"原句摘录"}]}',
        }],
      });
      response = await callLlmApi(deps.dataDir, messages, {
        system: buildSettlementJudgeSystem(),
        temperature: 0.1,
        max_tokens: 800,
        reasoningEffort: "low",
      ...
      raw = (response.blocks ?? [])
        ...
        .map((block) => block.text ?? "")
        .join("");
      parsed = parseSettlementJudgeOutput(raw);
    }
