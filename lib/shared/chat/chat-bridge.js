...
export function resolvedCheckKey(skill, action) {
  return `${String(skill ?? "").trim()}::${String(action ?? "").trim()}`;
}
 ...
 */
export function applyEventDrivenLanding(flat) {
...
const ITEM_STRICT_DENY = /的|那|这|其|些|隔着|传来|凉|冰|并排|摊开|贴身|齐整|收好|纸页|与|及|和|或|以及|放下|叠|空白|仔细|翻看|又|再|仍|继续|了|着|过|手套|抄录纸|皮面册子|之后/;
...
export function createSharedChatBridge(deps) {
  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      // 叙述词面启发式仅作兜底。
      const eventLanded = applyEventDrivenLanding(flat);
