...
export function createSharedChatBridge(deps) {
  ...
  async function runNarrationLoop(gameId, flat, messages) {
    let narration = "";
    let rounds = 0;
    let lastFinish = null;
    let calledRollTool = false;
    let pendingChecks = [];
    ...
    try {
      ...
      if (ra !== null) {
        ...
        messages.push({
          ...
          content: [{
            type: "text",
            text: `【系统检定】${rollLine}。请据此继续叙述，只写效果，不要出现成功档位词或骰值。`,
          }],
        });
      } else {
        messages = buildLoopMessages(flat.log ?? [], deps.maxChatLog);
      ...
      const loopResult = await runNarrationLoop(gameId, flat, messages);
      const narration = stripResultPhrases(formatNarration(loopResult.narration, loopResult.lastFinish));
