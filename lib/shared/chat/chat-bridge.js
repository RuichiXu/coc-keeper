...
/**
 * 关键点自动揭示：叙述正文里明确出现未揭示关键点的标题时，
 * 视为该剧情点已被玩家知晓，直接揭示（保守：标题 ≥ 2 字且完整包含）。
...
export function revealKeyPointsFromNarration(keyPoints, narration) {
  ...
  for (const kp of keyPoints ?? []) {
    ...
    if (title.length >= 2 && text.includes(title)) {
      kp.revealed = true;
      changed += 1;
    }
  }
...
const ITEM_ACQUIRE_RE =
  /(?:拿起|带上|取得|拿到|获得|得到|取出|掏出|拾起|捡起|带走|收起|揣进|装进|收好|放进背包|放入包内|塞进|接过|借到)了?(?:那|这)?(?:一把|一张|一份|一本|一个|一叠|一串|一枚|一部|一台|一柄|一根|一盏|一支)?([^，。；、！？\n]{1,12})/g;
const ITEM_ABSTRACT_DENY = /信任|线索|消息|结论|进展|真相|机会|灵感|情报|优势|先机|头绪|眉目|主动权|把握|风声|口风|情况|状况|动静|味道|气味/;

/**
 * 物品获取自动入栏：从验证后的叙述中保守提取“获得/拿起”物品，
 * 写入第一个非 AI 调查员的物品栏（已存在则跳过）。
 * @param {object} flat
 * @param {string} narration
...
export function autoTrackInventory(flat, narration) {
  ...
  while ((match = ITEM_ACQUIRE_RE.exec(text)) !== null) {
    let item = String(match[1] ?? "").trim();
    item = item.replace(/^(?:一把|一张|一份|一本|一个|一叠|一串|一枚|一部|一台|一柄|一根|一盏|一支|那|这)/, "");
    item = item.replace(/(?:递给|交给|放到|放在|塞到|塞进|递了|给了|拿给)[^，。；、！？]{0,8}$/, "");
    item = item.replace(/[，。；、！？\s]+$/g, "");
    if (item.length < 2 || item.length > 12) continue;
    if (ITEM_ABSTRACT_DENY.test(item)) continue;
