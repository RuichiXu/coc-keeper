...
const ITEM_QUANTIFIER_RE =
  /^(?:[一二三四五六七八九十两数几]+[张份本个叠串枚部台柄根盏支把件]|那|这)/;
const ITEM_ACQUIRE_RE =
  /(?:拿起|带上|取得|拿到|获得|得到|取出|掏出|拾起|捡起|带走|收起|揣进|装进|装入|收进|放进|放入|收好|携带|随身携带|挎上|放进背包|放入包内|塞进|接过|借到)了?(?:那|这)?(?:[一二三四五六七八九十两数几]+[张份本个叠串枚部台柄根盏支把件])?([^，。；、！？\n]{1,12})/g;
const ITEM_BA_RE =
  /(?:把|将)([^，。；、！？\n]{1,12}?)(?:装入|装进|放进|放入|收进|收好|带上|带走|拿起|取出|塞进|塞入|揣进)/g;
const ITEM_ABSTRACT_DENY = /信任|线索|消息|结论|进展|真相|机会|灵感|情报|优势|先机|头绪|眉目|主动权|把握|风声|口风|情况|状况|动静|味道|气味/;
const ITEM_CONTAINER_DENY = /文件夹|背包|包内|口袋|衣袋|裤袋|箱子|盒子|抽屉|柜子|书柜|壁橱|行囊|挎包|皮包|提包|匣子/;
const ITEM_ALIASES = { 原稿: "手稿", 稿纸: "手稿", 稿件: "手稿" };

/**
 * 把提取出的原始物品名规范化：去数量词、去尾部动词、拒绝抽象词、套别名。
 * @param {string} raw
 * @returns {string|null}
 */
export function normalizeAcquiredItem(raw) {
  ...
  item = item.replace(/(?:按顺序|依次|小心地|轻轻地|慢慢|全部|一起|统统|都|整齐地)$/, "");
...
export function autoTrackInventory(flat, narration) {
  ...
  const pc =
    ...
    null;
  if (pc === null) return [];
  const inventory = Array.isArray(pc.inventory) ? pc.inventory : (pc.inventory = []);
  const candidates = [];
  let match;
  ...
  for (const item of candidates) {
    if (inventory.includes(item)) continue;
    inventory.push(item);
    added.push(item);
  }
  return added;
...
export function createSharedChatBridge(deps) {
  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      // 叙事候选校验：线索门禁泄露 + 危险推荐动作。
      // 有问题时追加纠正消息并重写一次；重写仍不合格也接受（避免空转），
      // 但至少阻止了“无检定先给线索”的叙述直接落盘。
      {
        ...
        if (guardIssues.some((issue) => issue.kind === "clue-leak") && sideEffectSnapshot !== null) {
        ...
        if (guardIssues.length > 0) {
          messages.push({
            ...
            content: [{
              type: "text",
              text: `（系统）${guardIssues.map((issue) => issue.message).join(" ")}`,
            }],
          });
      ...
      if (sideEffectRollbackNeeded && sideEffectSnapshot !== null) {
        if (restoreSideEffects(flat, sideEffectSnapshot)) {
          syncSession(flat);
          deps.session.recordTrace({ kind: "side-effect-rollback", at: nowIso(), reason: "clue-leak-guard" });
          flat.core = deps.session.toJSON();
          flat.updatedAt = nowIso();
          deps.persistence.save(deps.stateKey(gameId), flat);
        }
