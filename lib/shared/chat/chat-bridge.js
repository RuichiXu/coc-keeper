...
export function createSharedChatBridge(deps) {
  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      if (finalBranch?.reached === true && finalChosen.length > 0 && flat.endingReached !== true) {
        ...
        const deterministicEndingFallback = () =>
          ...
          /(?:尾声|后日谈|葬礼|本次跑团到此|画下句点|就此作结|圆满结束|——终——|终幕|幕落)/.test(narration) ||
          /(?:结束本次跑团|结束本次冒险|结束行动|完成结局结算|收束并记录结局|就此作结|画下句点)/.test(text);

        if (deps.enableSemanticSettlement === true) {
          // 语义结局裁决：到达结局节点（最终分支已选）后，每轮一次 flash 小调用
          // 判断所选结局是否已在叙述/玩家输入中被确认；固定收束语只作回退。
          ...
          if (endingJudge.ok) {
            if (endingJudge.ended === true) stats.endedTrue += 1;
            ...
            endingReached = endingJudge.ended === true;
          } else {
            stats.failures += 1;
