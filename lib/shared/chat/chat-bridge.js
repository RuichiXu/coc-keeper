 ...
 * @param {object} flat
 * @param {string} combinedText - 玩家输入 + 叙述
 * @returns {number} 新落地分支数
 */
export function autoLandBranches(flat, combinedText) {
  const source = String(combinedText ?? "");
  let changed = 0;
...
export function applyEventDrivenLanding(flat) {
  ...
  for (const kp of flat.keyPoints ?? []) {
    ...
    // 1) 场景切入：currentScene 精确等于关键点场景（“三层书房”不匹配“三层书房门外”）。
    if (scene.length >= 4 && !scene.includes("/") && scene !== "导入" && currentScene === scene) {
      hit = true;
    }
  ...
  // 分支事件落地：掀开地毯（ai-br-2）由“墨渊首次目击”或极限侦查通过驱动。
  for (const branch of flat.branches ?? []) {
    if (branch?.reached === true) continue;
    const id = String(branch.id ?? "");
    const title = String(branch.title ?? "");
    if (id === "ai-br-2" || title.includes("掀开地毯")) {
      const kp5Revealed = (flat.keyPoints ?? []).some(
        (entry) => (entry.id === "ai-kp-5" || String(entry.title ?? "").includes("发现墨渊")) && entry.revealed === true
      );
      if (kp5Revealed || sanitySettledFor("chk-8") || passedAny(["chk-7"])) {
...
const ITEM_STRICT_DENY = /的|那|这|其|些|隔着|传来|凉|冰|并排|摊开|贴身|齐整|收好|纸页|与|及|和|或|以及|放下|叠|空白|仔细|翻看|又|再|仍|继续|了|着|过/;
...
export function normalizeAcquiredItem(raw) {
  ...
  item = item.replace(/(?:按顺序|依次|小心地|轻轻地|慢慢|全部|一起|统统|都|整齐地|盘好|盘着|卷好|卷着|折好|折着|斜挎|过肩|沉甸甸地|沉甸甸)$/, "");
...
export function autoTrackInventory(flat, narration) {
  ...
  for (const rawItem of candidates) {
    const canonicalNames = canonicalItemsFromEntities(rawItem, flat.entities ?? []);
...
export function createSharedChatBridge(deps) {
  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      // 防止 LLM 把叙述回退到掀地毯/书房调查等更早场景。
      {
        ...
        if (finalRiteSucceeded) {
          ...
          for (const kp of flat.keyPoints ?? []) {
            ...
            if (kpId === "ai-kp-7" || kpTitle.includes("十二字咒文") || kpId === "ai-kp-8" || kpTitle.includes("最终抉择")) {
              kp.revealed = true;
            }
          }
      ...
      const combinedForLanding = `${text}\n${narration}`;
      const landedBranches = autoLandBranches(flat, combinedForLanding) + eventLanded.branches;
