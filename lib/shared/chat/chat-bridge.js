...
export function createSharedChatBridge(deps) {
  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      // 开场前提关键点：第一段 KP 叙述落盘时，scene=导入 的关键点（如“委托到来”）
      ...
      if (kpLogCountBefore === 0 && narration.trim().length > 0) {
      ...
      const revealedCount = revealKeyPointsFromNarration(flat.keyPoints, narration) + openingRevealed;
