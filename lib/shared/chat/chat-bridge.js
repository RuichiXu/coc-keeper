...
import { judgeSettlements } from "./settlement-judge.js";
import { judgeEndingReached } from "./ending-judge.js";
export { callLlmApi };
import {
  branchLandedEvent,
...
import { recordHpLoss } from "./damage-ledger.js";
import {
...
import { containsRawToolCallSyntax, validateNarrationCandidate } from "./narration-guard.js";
...
import { applyEndingResolvedEvent, createEndingResolvedEvent, endingKeywordsFor } from "./ending.js";
...
export function findEarlyDiaryLeak(narration, flat) {
  const diaryKp = (flat?.keyPoints ?? []).find(
    (kp) => String(kp?.title ?? "").includes("日记") || String(kp?.title ?? "").includes("手稿")
  );
  if (diaryKp === undefined || diaryKp?.revealed === true) return [];
  ...
  // 从关键点自带文本里提取受保护句子；没有自带文本时不做任何剧本专属拦截。
  ...
  return protectedLines
    ...
    .map((line) => `叙述提前泄露关键点「${String(diaryKp.title ?? "")}」的受保护内容「${line}」；该线索受检定点保护，请先给团检，不要直接写出。`);
}

/**
 * 剔除叙述末尾的推荐选项/菜单行（以 -/* /数字序号开头）。
 * 结算匹配只关心“已发生的叙述与玩家行动”，不关心系统给出的可选项。
 * @param {string} text
 * @returns {string}
 */
...
/**
 * 在 KP 叙述中查找具体的 HP/SAN 数值变化声明（如“林晚 HP -3”）。
 * 真实数值应由系统状态行/骰点行展示，叙述里出现这类声明会造成
 * “叙述扣 3 点、账本扣 5 点”的状态不一致（Codex r7 房间2）。
 * @param {string} text
 * @returns {string|null} 命中的声明片段
 */
export function findPcStateClaim(text) {
  const source = String(text ?? "");
  const match = source.match(/(?:HP|SAN)\s*[-−]\s*\d+/);
  return match !== null ? match[0] : null;
}

/**
 * 删除叙述中的括号式 HP/SAN 数值声明（如“（林晚 HP -3）”）；
 * 括号外的正文保留，只去掉状态数值括号组。
 * @param {string} text
 * @returns {string}
 */
export function stripPcStateClaims(text) {
  return String(text ?? "")
    .replace(/[（(][^（）()]*(?:HP|SAN|生命值|理智值|生命|理智)[^（）()]*[）)]/g, "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
}
...
export function autoLandBranches(flat, playerText, narration = null) {
  ...
  // 玩家选择型最终分支（finalChoice / br-final-*）：聊天 UI 里玩家的明确选择
  // 就是文本输入，等价于 UI 点击选项。只有玩家输入精确命中选项原文、
  // 且当前场景与分支场景一致（或分支未限定场景）时才落地，避免叙述词代选结局。
  // 若 KP 已通过 coc_branch reached/choose 落地，这里不会重复处理。
  for (const branch of flat.branches ?? []) {
    if (branch?.reached === true) continue;
    if (branch?.finalChoice !== true && !String(branch?.id ?? "").startsWith("br-final")) continue;
    const branchScene = String(branch?.scene ?? "").trim();
    const currentScene = String(flat?.currentScene ?? "").trim();
    const sceneOk =
      branchScene.length === 0 ||
      (currentScene.length > 0 && (currentScene.includes(branchScene) || branchScene.includes(currentScene)));
    if (!sceneOk) continue;
    const options = Array.isArray(branch?.options) ? branch.options : [];
    const source = String(playerText ?? "");
    let bestMatch = null;
    let bestLength = -1;
    for (const option of options) {
      const label = String(option?.label ?? "").trim();
      if (label.length < 2) continue;
      if (source.includes(label) && label.length > bestLength) {
        bestMatch = option;
  ...
  for (const branch of flat.branches ?? []) {
    if (branch?.reached === true) continue;
    // 玩家选择型最终分支已在上方专门处理；这里保留跳过，避免叙述文本代选。
    // 检定点分支：必须由 .ra 检定门禁推进，不能靠叙述词代选。
    if (branch?.finalChoice === true || String(branch?.id ?? "").startsWith("br-final") || branch?.checkpointBranch === true) continue;
    const options = Array.isArray(branch?.options) ? branch.options : [];
...
export function applyEventDrivenLanding(flat, playerText = "", narration = "") {
  // 同一轮内落地使用快照：本回合新揭示的关键点/新选择的分支，不会在同一轮
  // 继续满足后续节点的前置条件，避免 keyPointIds 链/分支链在单轮内 cascade 塌缩。
  const snapshotKeyPoints = (Array.isArray(flat.keyPoints) ? flat.keyPoints : []).map((kp) => ({ ...kp }));
  const snapshotBranches = (Array.isArray(flat.branches) ? flat.branches : []).map((branch) => ({ ...branch }));
  const ctx = {
  ...
  // 分支事件落地：满足结构化前置条件的分支标记 reached；只有明确给出
  // autoChooseLabel 或只有一个选项时才代选，否则保留 chosen 为空等待玩家选择。
  for (const branch of flat.branches ?? []) {
    if (branch?.reached === true) continue;
    ...
    branch.reached = true;
    if (options.length === 1 && branch?.checkpointBranch !== true && branch?.finalChoice !== true && !String(branch?.id ?? "").startsWith("br-final")) {
      branch.chosen = String(options[0]?.label ?? "");
    } else if (label.length > 0) {
      const matched = options.find((entry) => String(entry.label ?? "").includes(label));
      branch.chosen = String(matched?.label ?? "");
    } else {
      branch.chosen = "";
    }
    if (flat.currentBranchId === undefined || flat.currentBranchId === null || flat.currentBranchId.length === 0) {
...
const ITEM_PARTICLE_DENY = /从|往|在|被|将|把|它|你|熟悉|位置/;
...
const ITEM_JUNK_EXTRA = /隔层|两样|一并|手套|抄录纸|皮面册子|之后|可用装备/;
...
export function createSharedChatBridge(deps) {
  ...
  const enrichScenarioFacts = (flat) => {
    ...
    if (!Array.isArray(flat.scenarioSettlements)) {
      flat.scenarioSettlements = extractSettlements(text);
    }
    if (!Array.isArray(flat.settledSettlementIds)) flat.settledSettlementIds = [];
    if (!Array.isArray(flat.skillUseLog)) flat.skillUseLog = [];
    return flat;
  };
  ...
  // C-3/D-4：每轮用 flat 剧情结构 + 已确认 deepParse 同步 PlotGraph，并重算可达路线集合（frontier）。
  const refreshPlotFrontier = (flat) => {
    const routes = computeStoryFrontier(flat);
    const story = { keyPoints: flat.keyPoints ?? [], branches: flat.branches ?? [] };
    deps.session.plot.syncFromStory(story);
    syncPlotGraphFromDeepParse(deps.session.plot, flat.deepParse ?? null, story);
  ...
  async function runNarrationLoop(gameId, flat, messages, opts = {}) {
    ...
    for (; rounds < deps.maxChatRounds; rounds += 1) {
      ...
      const kpSystem = buildKpSystemPrompt({
        ...flat,
        endingStatus: summarizeReachability(deps.session.plot),
        frontier: storyFrontierText(computeStoryFrontier(flat)),
      });
      let response = await streamBlocks({
        system: kpSystem,
        messages,
        tools: toolsForThisRound,
        max_tokens: 1200,
      });
      // 输出预算被 reasoning 耗尽（finish=length 且无正文/工具调用）时，
      // 追加“直接输出正文”的纠正消息并提高预算重试一次，避免空叙述落盘。
      {
        const blocks = response.blocks ?? [];
        const hasContent =
          blocks.some((block) => block?.type === "text" && String(block.text ?? "").trim().length > 0) ||
          blocks.some((block) => block?.type === "tool-call");
        if (!hasContent && response.finish?.kind === "length") {
          messages.push({
            role: "user",
            source: { kind: "system" },
            content: [{
              type: "text",
              text: "（系统）你上一轮只输出了思考、没有输出正文或工具调用。请直接输出剧情叙述正文，不要再进行内部推理。",
            }],
          });
          response = await streamBlocks({
            system: kpSystem,
            messages,
            tools: toolsForThisRound,
            max_tokens: 2000,
      ...
      if (decision.kind === "narrate") {
        ...
        // 工具标记泄漏守卫：模型把工具调用标签当正文输出时，不落盘，纠正后重试。
        if (containsRawToolCallSyntax(text)) {
          toolSyntaxLeaks += 1;
          messages.push({
            role: "user",
            source: { kind: "system" },
            content: [{
              type: "text",
              text: "（系统）你的正文里出现了原始工具调用标签。请直接输出剧情叙述文本；需要操作工具时使用工具调用功能，不要把标签写进正文。",
            }],
          });
          continue;
        }
    ...
    return { narration, lastFinish, rounds, pendingChecks, calledRollTool, toolChecks, rollToolCalls, sanityCheckLines, toolSyntaxLeaks };
  }
  ...
  async function runKpTurn(gameId, text, player) {
    ...
    // D-4：已确认的深度解析覆盖/补充关键点与分支的结构化条件；
    // 未确认的 deepParse 只作草稿，不参与运行时。
    if (flat.deepParse?.status === "confirmed") {
      const applied = applyConfirmedDeepParse(flat);
      if (applied.keyPointsApplied > 0 || applied.branchesApplied > 0) {
        syncSession(flat);
        deps.session.recordTrace({ kind: "deep-parse-applied", ...applied, at: nowIso() });
      }
    }
    ...
    try {
      ...
      if (ra !== null || choiceAction !== null) {
        ...
        {
          const useLog = Array.isArray(flat.skillUseLog) ? flat.skillUseLog : (flat.skillUseLog = []);
          useLog.push({ skill, passed: result.passed === true, action, at: nowIso() });
          if (useLog.length > 120) flat.skillUseLog = useLog.slice(-120);
          // 立即落盘，防止后续叙事循环里工具执行从磁盘重载 flat 时
          // 丢掉刚记录的成功/失败技能使用日志。
          saveFlat(gameId, flat);
        }
        if (result.passed) {
        ...
        // 检定点条件伤害：如“体质检定，随后承受1/1d4的伤害”。
        // 成功扣左值、失败扣右值，由 Rule Engine 掷伤害并走 coc_pc 落盘。
        {
          const damageExpr = selectedGate?.damage ?? checkpointBefore?.damage ?? null;
          if (damageExpr !== null && damageExpr !== undefined && String(damageExpr).length > 0) {
            const parts = String(damageExpr).split(/[\/／]/);
            const lossExpr = result.passed ? parts[0] ?? "0" : parts[1] ?? "0";
            if (lossExpr !== "0") {
              flat = deps.persistence.load(deps.stateKey(gameId)) ?? flat;
              const pcForDamage = (flat.characters ?? []).find((c) => c.name === name) ??
                (flat.characters ?? []).find((c) => c.aiControlled !== true) ??
                (flat.characters ?? [])[0] ?? null;
              if (pcForDamage !== null) {
                const hpBefore = Number(pcForDamage.hp ?? pcForDamage.stats?.HP ?? 10);
                const loss = rollExpr(lossExpr);
                const hpAfter = Math.max(0, hpBefore - loss);
                const pcDef = deps.toolDefs.get("coc_pc");
                if (pcDef !== undefined) {
                  await executeToolForLoop(pcDef, { game: gameId, name: pcForDamage.name, hp: hpAfter, _auto: true });
                  flat = deps.persistence.load(deps.stateKey(gameId)) ?? flat;
                  appendLog(flat, "roll", `[场景伤害] ${pcForDamage.name} 损失 ${loss} HP（${hpBefore} → ${hpAfter}）`, pcForDamage.name);
                  recordHpLoss(flat, pcForDamage.name, loss, `checkpoint:${String(selectedGate?.checkpointId ?? checkpointBefore?.id ?? "")}`);
                  saveFlat(gameId, flat);
                }
              }
            }
        ...
        // 最终仪式：意志/SAN 成功必须按已选分支推进结局，禁止回退到更早场景。
        // 结局具体内容由剧本原文与已选分支决定，这里只做流程约束，不注入任何剧本专属文本。
        ...
        if (finalBranch?.reached === true && String(finalBranch?.chosen ?? "").length > 0 && /^(意志|POW|理智|SAN)$/i.test(skill)) {
          rollSystemText += `\n这是最终仪式轮（已选「${String(finalBranch.chosen)}」）。${result.passed ? "检定成功：直接推进到该选择的结局，不要再回退到更早场景。" : "检定失败：按失败后果继续，不要回退到更早场景。"}`;
        }
        messages.push({
          role: "user",
          source: { kind: "system" },
        ...
        messages = buildLoopMessages(flat.log ?? [], deps.maxChatLog);
        const spellRecognition = buildSpellRecognition(extractSpellGroups(flat)?.groups ?? null);
        if (spellRecognition !== null && (spellRecognition.forward.test(text) || spellRecognition.inverse.test(text))) {
          messages.push({
            role: "user",
            source: { kind: "system" },
            content: [{
      ...
      // 状态数值守卫：叙述里出现“林晚 HP -3”这类具体 HP/SAN 数值声明时，
      // 先要求 LLM 重写一次（只描述伤势与效果，不写数值）；仍残留则删除括号声明。
      // 真实数值由系统状态行/骰点行展示，避免叙述 -3 而账本 -5 的不一致（Codex r7）。
      {
        const stateClaim = findPcStateClaim(narration);
        if (stateClaim !== null) {
          messages.push({
            role: "user",
            source: { kind: "system" },
            content: [{
              type: "text",
              text: `（系统）你的叙述里出现了具体 HP/SAN 数值变化（${stateClaim}）。请不要在正文或选项行中写出具体数值；真实数值已由系统状态行/骰点行展示，你只需描述伤势与效果。请重写这一段。`,
            }],
          });
          ...
          if (retryNarration.trim().length > 0 && findPcStateClaim(retryNarration) === null) {
            loopResult = retryResult;
            narration = retryNarration;
          } else {
            narration = stripPcStateClaims(narration);
          }
        }
      }
      ...
      // 路线守卫：叙述中出现剧本原文与地点白名单里都没有的地点词
      // （如《对流》复测里的“下层阀门舱/旋梯/拱道”）时，追加纠正并重写一次。
      {
        const routeIndexForGuard = buildRouteIndex(flat);
        const allowedLocationTerms = collectAllowedLocationTerms(flat, routeIndexForGuard);
        const unknownLocationTerms = findUnknownLocationTerms(narration, allowedLocationTerms);
        if (unknownLocationTerms.length > 0) {
          deps.session.recordTrace({ kind: "route-guard", terms: unknownLocationTerms, at: nowIso() });
          messages.push({
            role: "user",
            source: { kind: "system" },
            content: [{
              type: "text",
              text: `（系统）你的叙述出现了剧本原文没有的地点词：${unknownLocationTerms.join("、")}。请以【当前场景原文】和【本剧本地点清单】为准重写，不要新增地点/房间/通道/舱/层/台。`,
            }],
          });
          const retryResult = await runNarrationLoop(gameId, flat, messages, { calledRollTool: loopResult.calledRollTool });
          const retryNarration = stripResultPhrases(formatNarration(retryResult.narration, retryResult.lastFinish));
      ...
      {
        // C-4/D-4：结局由 PlotGraph 结局节点完成时发布——先同步剧情图
        // （含已确认 deepParse 的边与结局节点），再取已完成的结局节点来构造 EndingResolved 事件。
        const plotStory = { keyPoints: flat.keyPoints ?? [], branches: flat.branches ?? [] };
        deps.session.plot.syncFromStory(plotStory);
        syncPlotGraphFromDeepParse(deps.session.plot, flat.deepParse ?? null, plotStory);
        ...
        const finalBranchFromPlot =
          endingNode !== null
            ? {
                ...
                ...(endingNode.endingKeywords !== undefined ? { endingKeywords: endingNode.endingKeywords } : {}),
                ...(endingNode.endingRequires !== undefined ? { endingRequires: endingNode.endingRequires } : {}),
                ...(endingNode.endingBlockers !== undefined ? { endingBlockers: endingNode.endingBlockers } : {}),
              }
            : null;
      ...
      if (flat.endingReached !== true) {
        // 路线约束优先：玩家输入里有“去/前往/进入/返回 + 白名单地点”，
        // 且该地点是当前场景的邻接地点（或当前场景为空）时，确定性切换；
        // 叙述推断只作兜底。
        // 叙述必须先用 stripMenuLines 去掉推荐选项行——选项里的“前往/返回”
        // 会被 hasSceneMovementPhrase 当成位置转移，把“教授的手稿”等词
        // 误判成布莱克的房间（Codex r5 第 20 轮泄压阀场景漂移根因）。
        const narrationBody = stripMenuLines(narration);
        const routeIndexForScene = buildRouteIndex(flat);
        const route = routeNeighbors(routeIndexForScene, flat.currentScene ?? "");
        const travelTarget = findTravelIntent(routeIndexForScene.aliasIndex, text);
        const travelOk =
          travelTarget !== null &&
          (route.current === null || route.neighbors.includes(travelTarget));
        const inferred = travelOk
          ? travelTarget
          : inferSceneTransition(narrationBody, flat.currentScene ?? "", flat.scenarioFacts ?? []);
        if (inferred !== null && inferred.length > 0 && inferred !== flat.currentScene) {
          ...
          const mentionsCurrent = currentKeywords.some((keyword) => narrationBody.includes(keyword));
          // 玩家明确表达移动意图（且路线合法）时，直接采纳，不再用
          // “叙述仍提到当前场景词”拦下正确切换（r5 第 32 轮前往外部控制室被拦）。
          if (travelOk || (flat.currentScene ?? "") === "" || !mentionsCurrent) {
            const previousScene = flat.currentScene ?? "";
            flat.currentScene = inferred;
            // 确定性推断的场景切换也要落 SceneChanged 事件，
            // 否则场景历史缺失，最终分支补登记会因“从未到过该场景”被拒。
            sceneEvents.push({
              type: "SceneChanged",
              at: nowIso(),
              gameId,
      ...
      const landingEvents = [...sceneEvents];
      for (const kp of flat.keyPoints ?? []) {
      ...
      // 剧本结算点：旧检定点提取会漏掉的 SC/HP 事件（如“直面沸核SC 1d4/1d10”、
      // “房间里的全员HP-1d6”）在此确定性结算。SC 走 coc_sanity_check（明骰并去重），
      // HP 走 coc_pc 扣血；若结算点关联了体质/技能门禁（承受1/1d4的伤害），
      // 一并登记，玩家 .ra 后由检定点伤害逻辑扣血。
      {
        const settlements = Array.isArray(flat.scenarioSettlements) ? flat.scenarioSettlements : [];
        if (settlements.length > 0) {
          flat = deps.persistence.load(deps.stateKey(gameId)) ?? flat;
          const settled = new Set(Array.isArray(flat.settledSettlementIds) ? flat.settledSettlementIds : []);
          const pendingSettlements = settlements.filter((settlement) => {
            const id = String(settlement.id ?? "");
            return id.length > 0 && !settled.has(id);
          });
          // 推荐选项/菜单行不是“已发生的事件”，结算匹配前必须剔除，
          // 否则“（如山坡、矿洞方向）”会误触外出结算（Codex r3 第 1 轮）。
          const narrationBody = stripMenuLines(narration);
          const settlementText = `${text}\n${narrationBody}`;
          const pc = (flat.characters ?? []).find((c) => c.aiControlled !== true) ??
            (flat.characters ?? [])[0] ?? null;
          const currentScene = flat.currentScene ?? "";
          let dirty = false;

          // 到达型结算点：当前场景已抵达目的地即命中，不需要语义裁决。
          const arrivalHit = (settlement) => {
            const sceneTokens = settlement?.sceneTokens ?? sceneTokensFor(settlement?.scene);
            return (
              settlement?.matchRules?.arrival === true &&
              sceneTokens.length > 0 &&
              sceneTokens.some((token) => currentScene.includes(token))
            );
          };

          // 语义裁决层：固定字段匹配降级为召回/回退，实际“事件是否发生”
          // 由一次非流式小调用判定；数值结算仍走确定性工具。
          // judge 调用数/失败数/回退数与每条 verdict 先写入本地 observations，
          // 待全部工具执行完成后再落到 session.trace + flat.judgeStats，
          // 避免循环内工具重载状态覆盖 trace（Codex r6 回退日志丢失根因）。
          let judge = null;
          let judgeInvoked = false;
          const judgeObservations = [];
          if (deps.enableSemanticSettlement === true && pendingSettlements.length > 0) {
            const candidates = pendingSettlements.filter((settlement) => !arrivalHit(settlement));
            if (candidates.length > 0) {
              judgeInvoked = true;
              judge = await judgeSettlements(deps, {
              ...
              if (!judge.ok) {
                judgeObservations.push({
                  kind: "settlement-judge-fallback",
                  error: judge.error ?? "",
                  at: nowIso(),
                });
              }
          ...
          for (const settlement of pendingSettlements) {
            const id = String(settlement.id ?? "");
            let hit = false;
            if (arrivalHit(settlement)) {
              hit = true;
            } else if (judge?.ok) {
              const verdict = judge.verdicts.get(id) ?? null;
              if (verdict !== null) {
                judgeObservations.push({ kind: "settlement-judge", id, ...verdict, at: nowIso() });
                hit = verdict.happened === true && verdict.subject === "pc";
              }
            } else {
              hit = settlementMatches(settlement, settlementText, currentScene);
            }
            if (!hit) continue;

            if (settlement.kind === "san" && pc !== null && typeof settlement.sanLoss === "string" && settlement.sanLoss.length > 0) {
              const sanDef = deps.toolDefs.get("coc_sanity_check");
              if (sanDef !== undefined) {
                const outcome = await executeToolForLoop(sanDef, {
                  game: gameId,
                  player: pc.name,
                  sanLoss: settlement.sanLoss,
                  description: String(settlement.trigger ?? "").slice(0, 80),
                  eventId: `scenario:${id}`,
                });
                flat = deps.persistence.load(deps.stateKey(gameId)) ?? flat;
                if (outcome.ok) {
                  const line = sanitizeSanityLine(outcome.text);
                  if (line.length > 0) appendLog(flat, "roll", line, pc.name);
                }
                dirty = true;
              }
            } else if (settlement.kind === "hp" && pc !== null && typeof settlement.damage === "string" && settlement.damage.length > 0) {
              const pcDef = deps.toolDefs.get("coc_pc");
              if (pcDef !== undefined) {
                const hpBefore = Number(pc.hp ?? pc.stats?.HP ?? 10);
                const loss = rollExpr(settlement.damage);
                const hpAfter = Math.max(0, hpBefore - loss);
                const outcome = await executeToolForLoop(pcDef, { game: gameId, name: pc.name, hp: hpAfter, _auto: true });
                flat = deps.persistence.load(deps.stateKey(gameId)) ?? flat;
                if (outcome.ok) {
                  appendLog(flat, "roll", `[场景伤害] ${pc.name} 损失 ${loss} HP（${hpBefore} → ${hpAfter}）`, pc.name);
                  recordHpLoss(flat, pc.name, loss, `settlement:${id}`);
                }
                dirty = true;
              }
            }

            // 注意：这里不再 reload flat，避免把上一段刚 append 的 SAN/HP 日志行冲掉
            // （Codex r3 第 1 轮 SAN 损失没进玩家可见日志的根因）。
            if (settlement.linkedGate?.skill && settlement.linkedGate?.damage) {
              const pending = Array.isArray(flat.pendingChecks) ? flat.pendingChecks : (flat.pendingChecks = []);
              const exists = pending.some(
                (gate) => gate.skill === settlement.linkedGate.skill &&
                  String(gate.damage ?? "") === settlement.linkedGate.damage
              );
              if (!exists) {
                const linkedGate = {
                  id: nextId("chk", pending, "id"),
                  skill: settlement.linkedGate.skill,
                  difficulty: settlement.linkedGate.difficulty ?? "regular",
                  action: settlement.linkedGate.action ?? "",
                  hidden: false,
                  damage: settlement.linkedGate.damage,
                  target: gateTargetKey(String(settlement.linkedGate.action ?? "")),
                  at: nowIso(),
                  scene: flat.currentScene ?? "",
                  source: "settlement-linked",
                };
                // 只入 pending：本轮末尾的统一门禁渲染/事件提交会负责
                // 生成 GateCreated 事件与 [团检] 提示，避免重复提交。
                pending.push(linkedGate);
                dirty = true;
              }
            }
          ...
          // 语义裁决观测统一在工具执行完成后落盘（此时 flat 已是最终版本）。
          if (judgeInvoked || judgeObservations.length > 0) {
            const stats = flat.judgeStats ?? {
              calls: 0,
              failures: 0,
              verdicts: 0,
              fallbacks: 0,
            ...
            if (judgeInvoked) {
              stats.calls += 1;
              stats.lastDurationMs = Number(judge?.durationMs ?? 0);
              if (judge?.ok) {
                stats.verdicts += judge.verdicts.size;
              } else {
                stats.failures += 1;
                stats.fallbacks += 1;
                stats.lastError = String(judge?.error ?? "");
              }
            }
            flat.judgeStats = stats;
            for (const observation of judgeObservations) {
              deps.session.recordTrace(observation);
            }
            dirty = true;
          }
          ...
          if (dirty) {
            if (!Array.isArray(flat.settledSettlementIds)) flat.settledSettlementIds = [];
            flat.settledSettlementIds = [...new Set([...(flat.settledSettlementIds ?? []), ...settled])];
            saveFlat(gameId, flat);
          }
        }
      }
      ...
      // 否则 LLM 可能只写“十二个字已抄下”而始终不给出可核验的原文。
      {
        ...
        if (spell !== null && flat.spellShown !== true && (spellKp?.revealed === true || finalBranchChosen)) {
          appendLog(flat, "check", formatSpellDisplay(spell.groups));
      ...
      // 结局已确认（最终分支已选 + 叙述/玩家输入出现结局关键词或收束语）后，
      // 不再生成任何新的动作门禁；防止结局叙述里的“手稿/书房”等词又推导出侦查团检。
      const finalBranch = findFinalBranch(flat);
      const finalChosen = String(finalBranch?.chosen ?? "");
      const endingKeywords = endingKeywordsFor(flat, finalBranch);
      const endingOptions = (finalBranch?.options ?? []).map((option) => ({
        label: String(option?.label ?? ""),
        leadsTo: String(option?.leadsTo ?? ""),
      }));
      const endingRecall = () => {
        if (finalBranch?.reached === true && finalChosen.length > 0) return true;
        if (/(?:尾声|后日谈|葬礼|落幕|终幕|幕落|本次跑团到此|画下句点|就此作结|圆满结束|——终——|模组结束)/.test(narration)) return true;
        if (/(?:结束本次跑团|结束本次冒险|结束行动|完成结局结算|收束并记录结局|就此作结|画下句点)/.test(text)) return true;
        return endingOptions.some(
          (option) =>
            option.leadsTo.length > 0 &&
            (narration.includes(option.leadsTo) || text.includes(option.leadsTo) || narration.includes(option.label) || text.includes(option.label)),
        );
      };
      const matchEndingOption = (label) => {
        const source = String(label ?? "").trim();
        if (source.length === 0) return null;
        return endingOptions.find((option) => {
          const a = option.leadsTo;
          const b = option.label;
          return a === source || b === source || a.includes(source) || b.includes(source) || source.includes(a) || source.includes(b);
        }) ?? null;
      };
      const deterministicEndingFallback = () =>
        finalBranch?.reached === true &&
        finalChosen.length > 0 &&
        (endingKeywords.some((keyword) => narration.includes(keyword) || text.includes(keyword)) ||
          /(?:尾声|后日谈|葬礼|落幕|终幕|幕落|本次跑团到此|画下句点|就此作结|圆满结束|——终——|模组结束)/.test(narration) ||
          /(?:结束本次跑团|结束本次冒险|结束行动|完成结局结算|收束并记录结局|就此作结|画下句点)/.test(text));

      let endingReached = false;
      // 上一轮已语义判定“终局成立”但最终分支尚未标记：本轮一旦标记立即收束，不再等新叙述。
      if (
        flat.endingReached !== true &&
        finalBranch?.reached === true &&
        finalChosen.length > 0 &&
        (flat.pendingEnding?.detectedAt ?? null) !== null
      ) {
        flat.pendingEnding = null;
        endingReached = true;
      } else if (flat.endingReached !== true && endingRecall()) {
        if (deps.enableSemanticSettlement === true) {
          // 语义结局裁决：只要出现终局候选（最终分支已选，或叙述/输入里出现
          // 落幕语/结局选项关键词），就用一次 flash 小调用判断是否真的落幕，
          // 并尽量识别对应哪个最终分支选项；固定收束语只作回退。
          const endingJudge = await judgeEndingReached(deps, {
            finalBranch,
            playerText: text,
            narration,
            currentScene: flat.currentScene ?? "",
            pcName: (flat.characters ?? []).find((c) => c.aiControlled !== true)?.name ??
              (flat.characters ?? [])[0]?.name ?? "",
          });
          const stats = flat.endingJudgeStats ?? { calls: 0, endedTrue: 0, failures: 0, lastDurationMs: 0 };
          stats.calls += 1;
          stats.lastDurationMs = Number(endingJudge.durationMs ?? 0);
          if (endingJudge.ok) {
            deps.session.recordTrace({
              kind: "ending-judge",
              ended: endingJudge.ended === true,
              endingLabel: endingJudge.endingLabel ?? "",
              confidence: endingJudge.confidence ?? 0,
              evidence: endingJudge.evidence ?? "",
              at: nowIso(),
            });
            if (endingJudge.ended === true) {
              stats.endedTrue += 1;
              if (finalBranch?.reached === true && finalChosen.length > 0) {
                endingReached = true;
              } else {
                const matched = matchEndingOption(endingJudge.endingLabel ?? "");
                if (matched !== null && finalBranch !== null) {
                  finalBranch.reached = true;
                  finalBranch.chosen = matched.label;
                  flat.currentBranchId = finalBranch.id ?? flat.currentBranchId;
                  flat.pendingEnding = null;
                  deps.session.recordTrace({
                    kind: "ending-judge-auto-land",
                    branchId: finalBranch.id,
                    chosen: matched.label,
                    at: nowIso(),
                  });
                  endingReached = true;
                } else {
                  flat.pendingEnding = {
                    label: endingJudge.endingLabel ?? "",
                    evidence: endingJudge.evidence ?? "",
                    detectedAt: nowIso(),
                  };
                  appendLog(flat, "check", "[系统] 检测到终局叙述，但尚未确定最终分支选项；待最终分支标记后自动收束。");
                }
              }
            }
          } else {
            stats.failures += 1;
            deps.session.recordTrace({ kind: "ending-judge-fallback", error: endingJudge.error ?? "", at: nowIso() });
            endingReached = deterministicEndingFallback();
          }
          flat.endingJudgeStats = stats;
        } else {
          endingReached = deterministicEndingFallback();
        }
      }
      ...
      // 空叙述不落成 KP 正文，改为系统提示行；metrics 通过返回的 emptyNarration 计数。
      const emptyNarration = String(loopResult.narration ?? "").trim().length === 0;
      if (emptyNarration) {
        appendLog(flat, "check", "（本轮未产生叙述，请再说一次你的行动）");
      } else {
      ...
      // C-3/D-4：同步剧情图节点/边（含已确认 deepParse），应用已完成节点的后果（写 flags/线索/实体状态）。
      syncSession(flat);
      const plotStory = { keyPoints: flat.keyPoints ?? [], branches: flat.branches ?? [] };
      deps.session.plot.syncFromStory(plotStory);
      syncPlotGraphFromDeepParse(deps.session.plot, flat.deepParse ?? null, plotStory);
      ...
      return {
        ...
        busy: false,
        narration: emptyNarration ? "" : narration,
        emptyNarration,
        toolSyntaxLeaks: loopResult.toolSyntaxLeaks ?? 0,
