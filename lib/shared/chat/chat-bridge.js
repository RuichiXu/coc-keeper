  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      if (ra !== null) {
        ...
        // 找到与本次 .ra 对应的待处理团检（含其动作选项），用于绑定剧情。
        const pendingMatch = (flat.pendingChecks ?? []).find(
        ...
        const boundHint =
          pendingMatch !== null && typeof pendingMatch.hint === "string" && pendingMatch.hint.trim() !== ""
            ? pendingMatch.hint.trim()
            : "";
        messages.push({
          role: "user",
          source: { kind: "system" },
          content: [{
            type: "text",
            text: boundHint !== ""
              ? `【系统检定】${rollLine}。玩家执行的动作是：「${boundHint}」。请据此直接叙述该动作的剧情结果，只写效果，不要出现成功档位词或骰值。`
