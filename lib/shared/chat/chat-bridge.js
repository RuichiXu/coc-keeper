  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      // 防止 LLM 把叙述回退到掀地毯/书房调查等更早场景。
      {
        ...
        const endingKeywords = (finalBranch?.options ?? [])
          .map((option) => String(option?.leadsTo ?? "").replace(/(?:的)?(?:坏|好|真|假)?结局$|END$/i, "").trim())
          .filter((keyword) => keyword.length >= 2);
        const endingMentioned =
          endingKeywords.some((keyword) => narration.includes(keyword)) ||
          /(?:尾声|后日谈|葬礼)/.test(narration);
        if (finalRiteSucceeded) {
          ...
          // 结局达成即证明咒文已被拼凑、最终抉择已做出：把结构化前置条件里
          // 引用最终分支的关键点，以及咒文关键点，全部补记为已揭示。
          const spellKp = findSpellKeyPoint(flat);
          ...
          for (const kp of flat.keyPoints ?? []) {
            if (kp?.revealed === true) continue;
            if (endingKeyPoints.has(kp)) kp.revealed = true;
          }
