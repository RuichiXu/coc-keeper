...
/**
 * 为 .ra 结果寻找应揭示/应隐藏的剧本原文（显式检定点 trigger）。
 * 成功时把原文交给 LLM 改写成现场叙述，失败时则明确禁止写出成功结果。
 * @param {object} flat
 * @param {string} skill
...
/**
 * 场景失效门禁清理：门禁登记时绑定了 scene；当前场景已切走且两者互不包含时，
 * 该门禁的目标已不在现场，移入 skippedChecks（reason=scene-invalid）。
 * 纯函数（不落盘），返回移除数量。
 * @param {object} flat
 * @param {string} currentScene
 * @returns {number}
 */
export function expireSceneGates(flat, currentScene) {
  ...
  flat.pendingChecks = pending.filter((gate) => {
    ...
    if (gateScene === scene || scene.includes(gateScene) || gateScene.includes(scene)) return true;
    skipped.push({ ...gate, skippedAt: nowIso(), reason: "scene-invalid" });
    removed += 1;
    return false;
  });
...
export function findCheckpointMatch(flat, skill, difficulty, action) {
  const checks = (flat.scenarioCheckpoints ?? []).filter(
    (check) => String(check?.skill ?? "").trim() === skill &&
...
export function revealKeyPointsForBranchChoices(flat) {
  ...
  for (const kp of flat.keyPoints ?? []) {
    ...
    // “最终抉择”必须满足 最终分支已选 + 十二字咒文已揭示，由 applyEventDrivenLanding 统一判定，
    // 这里不再按 branch.leadsTo 提前揭示，避免 ai-kp-8 先于 ai-kp-7 落地。
    if (kp?.id === "ai-kp-8" || title.includes("最终抉择")) continue;
...
export function applyEventDrivenLanding(flat, playerText = "", narration = "") {
  const passed = new Set(Array.isArray(flat.passedCheckpointIds) ? flat.passedCheckpointIds : []);
  const sanitySettled = Array.isArray(flat.sanitySettled) ? flat.sanitySettled : [];
  const currentScene = String(flat.currentScene ?? "");
  const combined = `${String(playerText ?? "")}\n${String(narration ?? "")}`;
  let revealed = 0;
  let branches = 0;

  const passedAny = (ids) => ids.some((id) => passed.has(id));
  const sanitySettledFor = (checkpointId) =>
    sanitySettled.some((entry) => {
      const eventId = String(entry.eventId ?? "");
      return eventId.includes(checkpointId) || eventId === `scenario:${checkpointId}`;
    });
  ...
  for (const kp of flat.keyPoints ?? []) {
    ...
    // 1) 场景切入：仅空间/动作型关键点（“进入书房/打开暗门”）在 currentScene 精确
    //    等于其场景时揭示；发现型关键点（“发现一层墨渍”）不能因为身处该场景就揭示。
    //    “进入书房”额外要求本轮文本里有实际进门证据，避免 LLM 在门外就把 currentScene
    //    提前设为“三层书房”导致提前揭示。
    const spatialTitle = /^(进入|来到|抵达|打开)/.test(title);
    if (!hit && spatialTitle && scene.length >= 4 && !scene.includes("/") && scene !== "导入" && currentScene === scene) {
      if (id === "ai-kp-3" || title === "进入书房") {
        const entryVariants = ["进入书房", "进到书房", "走进书房", "踏进书房", "迈入书房", "来到书房内"];
        const entryEvidence = entryVariants.some((variant) => {
          let index = combined.indexOf(variant);
          while (index !== -1) {
            const before = combined.slice(Math.max(0, index - 4), index);
            if (!/(?:没|未|不|没有|无法|未能|别|莫|拒绝)/.test(before)) return true;
            index = combined.indexOf(variant, index + 1);
          }
          return false;
        });
        hit = combined.trim().length > 0 ? entryEvidence : true;
      } else {
        hit = true;
      }
    }
    // 2) 发现日记与手稿：日记检定点 + 手稿检定点都已通过。
    if (!hit && (id === "ai-kp-4" || title.includes("日记与手稿"))) {
      hit = passedAny(["chk-3", "chk-4"]) && passedAny(["chk-5", "chk-6"]);
    }
    // 3) 发现墨渊：仅由“墨渊首次目击”SAN 结算驱动（chk-9），
    //    确认接缝（chk-7）通过不算目击，不能提前揭示。
    if (!hit && (id === "ai-kp-5" || title.includes("发现墨渊"))) {
      hit = sanitySettledFor("chk-9");
    }
    // 4) 拼凑十二字咒文：智力极难解读通过（发现手稿上 12 个字）。
    if (!hit && (id === "ai-kp-7" || title.includes("十二字咒文"))) {
      hit = passedAny(["chk-13"]);
    }
    // 5) 克罗斯临终提示：与咒文同源，咒文已揭示或最终分支已选即视为已交代。
    if (!hit && (id === "ai-kp-6" || title.includes("临终提示"))) {
      const spellRevealed6 = (flat.keyPoints ?? []).some(
        (entry) => (entry.id === "ai-kp-7" || String(entry.title ?? "").includes("十二字咒文")) && entry.revealed === true
      );
      const finalChosen6 = (flat.branches ?? []).some(
        (branch) => branch?.id === "ai-br-3" && branch?.reached === true && String(branch?.chosen ?? "").length > 0
      );
      hit = spellRevealed6 || finalChosen6;
    }
    // 6) 最终抉择：最终分支已选，且十二字咒文关键点已揭示。
    if (!hit && (id === "ai-kp-8" || title.includes("最终抉择"))) {
      const finalChosen = (flat.branches ?? []).some(
        (branch) => branch?.id === "ai-br-3" && branch?.reached === true && String(branch?.chosen ?? "").length > 0
      );
      const spellRevealed = (flat.keyPoints ?? []).some(
        (entry) => (entry.id === "ai-kp-7" || String(entry.title ?? "").includes("十二字咒文")) && entry.revealed === true
      );
      hit = finalChosen && spellRevealed;
    }
  ...
  // 分支事件落地：掀开地毯（ai-br-2）由“墨渊首次目击”SAN 结算驱动，
  // 确认接缝（chk-7）通过不算掀开，不能提前落地。
  for (const branch of flat.branches ?? []) {
    if (branch?.reached === true) continue;
    const id = String(branch.id ?? "");
    const title = String(branch.title ?? "");
    if (id === "ai-br-2" || title.includes("掀开地毯")) {
      if (sanitySettledFor("chk-9")) {
        const option = (branch.options ?? []).find((entry) => String(entry.label ?? "").includes("掀开地毯")) ??
          (branch.options ?? [])[0];
        branch.reached = true;
    ...
    try {
      ...
      if (ra !== null || choiceAction !== null) {
        ...
        if (result.passed) {
          ...
          // 咒文解读兜底：智力在解谜语境下成功，即使门禁动作与 chk-13 的匹配词不完全一致，
          // 只要日记与手稿关键点已揭示，就确定性记录 chk-13 通过，驱动 ai-kp-7 与十二字展示。
          // 避免 LLM 在智力大成功后不落地咒文、反而生成错误十二字。
          {
            const baseSkill = String(skill ?? "").replace(/[（(][^）)]*[）)]/g, "").trim();
            const aiKp4Revealed = (flat.keyPoints ?? []).some(
              (kp) => (kp?.id === "ai-kp-4" || String(kp?.title ?? "").includes("日记与手稿")) && kp?.revealed === true
            );
            ...
            if (
              ...
              aiKp4Revealed &&
              decodeContext &&
              !(flat.passedCheckpointIds ?? []).includes("chk-13")
            ) {
              recordPassedCheckpoint(flat, "chk-13");
              syncSession(flat);
              deps.session.recordTrace({ kind: "checkpoint-pass", checkpointId: "chk-13", skill, action, at: nowIso(), via: "spell-decode-fallback" });
            }
          }
        ...
        const finalBranch = (flat.branches ?? []).find(
          (branch) => branch?.id === "ai-br-3" || String(branch?.scene ?? "").includes("结局")
        );
      ...
      // 防止 LLM 把叙述回退到掀地毯/书房调查等更早场景。
      {
        ...
        const finalRiteSucceeded =
          finalBranch?.reached === true &&
          String(finalBranch?.chosen ?? "").length > 0 &&
          /^(意志|POW|理智|SAN)$/i.test(String(rolledRaSkill ?? "")) &&
          lastRoll?.passed === true;
        const endingKeywords = /(?:墨渊消散|夏拉卡拉布降临|尾声|后日谈|葬礼)/;
        if (finalRiteSucceeded) {
          const chosen = String(finalBranch.chosen);
          if (!endingKeywords.test(narration)) {
            const endingSentence =
              chosen.includes("逆序") || chosen.includes("送神")
                ? "墨渊消散。夏拉卡拉布被逐回虚空，书房重新归于寂静。"
                : "夏拉卡拉布降临。仪式完成，整座宅邸沉入不可名状的死寂。";
          ...
          // 结局达成即证明咒文已被拼凑、最终抉择已做出，补记关键点避免 ai-kp-7/8 缺位。
          for (const kp of flat.keyPoints ?? []) {
            if (kp?.revealed === true) continue;
            const kpId = String(kp.id ?? "");
            const kpTitle = String(kp.title ?? "");
            if (kpId === "ai-kp-6" || kpTitle.includes("临终提示") || kpId === "ai-kp-7" || kpTitle.includes("十二字咒文") || kpId === "ai-kp-8" || kpTitle.includes("最终抉择")) {
              kp.revealed = true;
            }
          }
          abandonGates(undefined, "ending-short-circuit");
          flat.pendingChoice = null;
          syncSession(flat);
          deps.session.recordTrace({ kind: "ending-short-circuit", skill: rolledRaSkill, chosen, at: nowIso() });
        }
      }
      ...
      {
        const spellKp = (flat.keyPoints ?? []).find(
          (kp) => (kp?.id === "ai-kp-7" || String(kp?.title ?? "").includes("十二字咒文")) && kp?.revealed === true
        );
        const finalBranchChosen = (flat.branches ?? []).some(
          (branch) => branch?.id === "ai-br-3" && branch?.reached === true && String(branch?.chosen ?? "").length > 0
        );
        if (flat.spellShown !== true && (spellKp !== undefined || finalBranchChosen)) {
          appendLog(
            flat,
            "check",
      ...
      // 结局已确认（最终分支已选 + 叙述出现结局关键词）后，不再生成任何新的
      // 动作门禁；防止结局叙述里的“手稿/书房”等词又推导出侦查团检。
      const endingReached =
        (flat.branches ?? []).some(
          (branch) => branch?.id === "ai-br-3" && branch?.reached === true && String(branch?.chosen ?? "").length > 0
        ) &&
        /(?:墨渊消散|夏拉卡拉布降临|尾声|后日谈|葬礼)/.test(narration);
      if (endingReached) {
        abandonGates(undefined, "ending-reached");
        flat.pendingChoice = null;
        flat.endingReached = true;
