...
export function keypointTitleMatched(title, text) {
  ...
  for (const variant of keypointTitleVariants(title)) {
    ...
    // 4 字以上标题的 CJK 双字组兜底：“发现一层墨渍”→正文出现“一层”“墨渍”即命中。
    if (variant.length >= 4) {
      const grams = cjkBigrams(variant);
      const sourceCjk = source.replace(/[^\u4e00-\u9fff]/g, "");
      let hits = 0;
      for (const gram of grams) {
        if (sourceCjk.includes(gram)) hits += 1;
      }
...
export function findCheckpointReveal(flat, skill, difficulty, action) {
  ...
  for (const check of pool) {
    ...
    for (const key of check?.keys ?? []) {
      ...
      if (word.length >= 2 && actionText.length > 0 && actionText.includes(word)) score += 3;
    }
  ...
  return best !== null && typeof best?.trigger === "string" && best.trigger.trim().length > 0
    ? best.trigger
    : null;
}
...
export function autoLandBranches(flat, combinedText) {
  ...
  for (const branch of flat.branches ?? []) {
    ...
    const matched = options.find((option) => {
      ...
      return (core.length >= 2 && source.includes(core)) || (label.length >= 2 && source.includes(label));
    });
...
export function revealKeyPointsForBranchChoices(flat) {
  ...
  for (const kp of flat.keyPoints ?? []) {
    ...
    for (const branch of chosenBranches) {
      const branchScene = String(branch?.scene ?? "");
      ...
      if (
        (branchScene.length > 0 && scene.includes(branchScene)) ||
        (leadsTo.length > 0 && (scene.includes(leadsTo) || String(kp?.title ?? "").includes(leadsTo)))
      ) {
        kp.revealed = true;
        changed += 1;
        break;
...
const ITEM_STRICT_DENY = /的|那|这|其|些|隔着|传来|凉|冰|并排|摊开|贴身|齐整|收好|纸页|与|及/;
...
/**
 * 把候选物品归一到剧本实体物品名：叙述说“纸页/原稿”时，若剧本实体里有
 * “四张手稿”，则入栏名用实体名，避免产生“纸页”这类非规范物品。
 * @param {string} item
 * @param {Array<object>} entities
 * @returns {string|null} 实体名；无匹配返回 null
 */
export function canonicalItemFromEntities(item, entities) {
  const raw = String(item ?? "").trim();
  if (raw.length === 0) return null;
  const value = ITEM_ALIASES[raw] ?? raw;
  for (const entity of entities ?? []) {
    ...
    if (
      ...
      name.includes(value) ||
      value.includes(name) ||
      (core.length >= 2 && value.includes(core)) ||
      (core.length >= 2 && core.includes(value))
    ) {
      return name;
    }
  }
...
export function autoTrackInventory(flat, narration) {
  ...
  for (const rawItem of candidates) {
    let item = canonicalItemFromEntities(rawItem, flat.entities ?? []) ?? rawItem;
    // 未命中剧本实体名的候选必须通过更严格校验，过滤“的那齐整纸页”等句子残片。
    if (item === rawItem && ITEM_STRICT_DENY.test(item)) continue;
...
export function createSharedChatBridge(deps) {
  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      // 关键点/分支/物品自动落地：不依赖 LLM 自觉调用 coc_branch / coc_pc。
      const combinedForLanding = `${text}\n${narration}`;
      const landedBranches = autoLandBranches(flat, combinedForLanding);
      const branchRevealed = revealKeyPointsForBranchChoices(flat);
      const revealedCount = revealKeyPointsFromNarration(flat.keyPoints, narration) + openingRevealed + branchRevealed;
      ...
      // SAN 暗骰结算提示：具体数值仅 KP 可见，聊天只给非数值的结算说明。
      if ((loopResult.rollToolCalls ?? []).includes("coc_sanity_check")) {
        appendLog(flat, "check", "[SAN 检定] 系统已完成一次理智检定（暗骰），SAN 损失已结算；具体数值仅 KP 可见。");
      }
