...
/**
 * 当前剧本是否定义了《墨渊》十二字咒文。
 * 只用于决定《墨渊》特化的咒文展示/仪式提示是否允许注入；
 * 其它剧本绝不出现“启墨渊/引魂夜/临神名/归字主”这类跨剧本文本。
 * @param {object} flat
 * @returns {boolean}
 */
export function scenarioHasMoyuanSpell(flat) {
  const text = String(flat?.scenario?.text ?? "");
  return text.includes("启墨渊") && text.includes("归字主");
}
...
export function findEarlyDiaryLeak(narration, flat) {
  // 有明确剧本全文且不是《墨渊》时，不启用《墨渊》特化的日记核心句防护；
  // 旧测试/旧存档缺少 scenario.text 时保持原行为。
  if (!scenarioHasMoyuanSpell(flat) && String(flat?.scenario?.text ?? "").trim().length > 0) return [];
  ...
  async function runKpTurn(gameId, text, player) {
    ...
    try {
      ...
      if (ra !== null || choiceAction !== null) {
          ...
          if (damageExpr !== null && damageExpr !== undefined && String(damageExpr).length > 0) {
            ...
            if (lossExpr !== "0") {
              ...
              if (pcForDamage !== null) {
                ...
                if (pcDef !== undefined) {
                  await executeToolForLoop(pcDef, { game: gameId, name: pcForDamage.name, hp: hpAfter });
                  flat = deps.persistence.load(deps.stateKey(gameId)) ?? flat;
        ...
        // 《墨渊》特化的“逆序/正序念诵”后果只在剧本确实定义十二字咒文时注入。
        ...
        if (finalBranch?.reached === true && String(finalBranch?.chosen ?? "").length > 0 && /^(意志|POW|理智|SAN)$/i.test(skill)) {
          const hasMoyuan = scenarioHasMoyuanSpell(flat);
          rollSystemText += hasMoyuan
        ...
        messages = buildLoopMessages(flat.log ?? [], deps.maxChatLog);
        if (scenarioHasMoyuanSpell(flat) && (FULL_FORWARD_SPELL_RE.test(text) || FULL_INVERSE_SPELL_RE.test(text))) {
          messages.push({
            role: "user",
            source: { kind: "system" },
            content: [{
      ...
      // 一并登记，玩家 .ra 后由检定点伤害逻辑扣血。
      {
        ...
        if (settlements.length > 0) {
          ...
          for (const settlement of settlements) {
            ...
            if (!settlementMatches(settlement, combinedText)) continue;
            ...
            if (settlement.kind === "san" && pc !== null && typeof settlement.sanLoss === "string" && settlement.sanLoss.length > 0) {
              ...
              if (pcDef !== undefined) {
                ...
                const hpAfter = Math.max(0, hpBefore - loss);
                const outcome = await executeToolForLoop(pcDef, { game: gameId, name: pc.name, hp: hpAfter });
                flat = deps.persistence.load(deps.stateKey(gameId)) ?? flat;
                if (outcome.ok) appendLog(flat, "roll", `[场景伤害] ${pc.name} 损失 ${loss} HP（${hpBefore} → ${hpAfter}）`, pc.name);
                dirty = true;
              }
            }
      ...
      // 否则 LLM 可能只写“十二个字已抄下”而始终不给出可核验的原文。
      {
        ...
        if (scenarioHasMoyuanSpell(flat) && flat.spellShown !== true && (spellKp?.revealed === true || finalBranchChosen)) {
          appendLog(
            flat,
            "check",
