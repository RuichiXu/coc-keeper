...
export function confirmedEndingForBranch(flat, finalBranch, chosen = "") {
  ...
  if (picked.length > 0) {
    const exact = endings.find((ending) => String(ending?.optionLabel ?? "").trim() === picked);
    if (exact !== undefined) return exact;
    const fuzzy = endings.find((ending) => {
  ...
  return endings.find((ending) => String(ending?.optionLabel ?? "").trim().length === 0) ?? endings[0] ?? null;
}
