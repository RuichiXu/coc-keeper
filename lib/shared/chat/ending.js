 ...
 * 《墨渊》特化的“夏拉卡拉布/书房/宅邸”模板只在剧本确实定义十二字咒文时使用；
 * 其它剧本一律用通用句，避免跨剧本注入固定结局文本。
 * @param {object} finalBranch
 * @param {string} chosen
 * @param {string} [scenarioText=""]
 * @returns {string}
 */
export function endingSentenceFor(finalBranch, chosen, scenarioText = "") {
  const keyword = buildEndingKeywords(finalBranch)[0] ?? "";
  const hasMoyuan = String(scenarioText ?? "").includes("启墨渊") && String(scenarioText ?? "").includes("归字主");
  if (!hasMoyuan) {
...
export function createEndingResolvedEvent(flat, narration, opts = {}) {
  ...
  const sentence = mentioned ? "" : endingSentenceFor(finalBranch, chosen, flat?.scenario?.text ?? "");
