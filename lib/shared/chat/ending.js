...
export function endingKeywordsFor(flat, finalBranch, ending = null) {
  ...
  return buildEndingKeywords(finalBranch, String(finalBranch?.chosen ?? ""));
}

/**
 * 从最终分支选项派生结局关键词（去掉“的结局/END”等尾缀）。
 * 已选分支时只取该选项的 leadsTo，避免把其它结局的关键词当成当前结局。
 * @param {object|null} finalBranch
 * @param {string} [chosen] - 已选选项 label
 * @returns {string[]}
 */
export function buildEndingKeywords(finalBranch, chosen = "") {
  const options = finalBranch?.options ?? [];
  const picked = String(chosen ?? "").trim();
  const source = picked.length > 0 ? options.filter((option) => String(option?.label ?? "") === picked) : options;
  const list = source.length > 0 ? source : options;
  return list
...
/**
 * 结局句兜底：只使用最终分支选项派生的关键词，或最通用的收束句。
 * 不包含任何剧本专属文本（结局的具体内容由 KP 按剧本原文与已选分支叙述）。
 * @param {object} finalBranch
 * @param {string} chosen
 * @returns {string}
 */
export function endingSentenceFor(finalBranch, chosen) {
  const keyword = buildEndingKeywords(finalBranch, String(chosen ?? ""))[0] ?? "";
