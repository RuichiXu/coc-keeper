...
### 隐藏新剧本最终门禁（3 个必过 + 1 个可选长剧本）

| 剧本 | 轮数 | preflight | 审校 | B级 |
|---|---|---|---|---|
| 2001：太空漫游 | 3 | h0/m0 | h0/m1 | ✅ pass |
| 无心漫谈 | 3 | h0/m0 | h0/m0/l1 | ✅ pass |
| 星影泠—坍圮之梦 | 2 | h0/m0 | h0/m1 | ✅ pass |

- 最终门禁 3/3 通过；星孩v1.0（117 页长剧本）作为可选尝试，正在解析。
- 门禁前额外修复：最终分支 scene 回退真实场景标题、列表型最后选择提取、
  分阶段结局指引。

### 可选长剧本尝试

- 星孩v1.0（117 页，166k 字符）：3 轮 preflight h0/m12，审校 h2/m0，未达 B 级。
...
## Session（2026-09-05 后续）：网络拓扑保真讨论 + 设计文档

- 用户反馈：解析出的剧情网络基本单线，仅结局附近分支；《星孩》的并行幕/可选遭遇/支线被拉成单链。
- 根因归纳：chunk 阶段不能生成中途分支节点；`repairDeepParseConnectivity` 按扁平 order 串主链；preflight 门禁奖励连通、惩罚悬空；分块 prompt 看不到全局幕结构。
- 讨论结论（详见 `NETWORK-TOPOLOGY.md`）：
  - 节点增加 `reachability`（strict/conditional/optional），先宽松条件可达闭包，后按需收紧。
  - 确定性后处理用结构树搭骨架：LLM 边优先；同级兄弟默认 hub-and-spoke；编号标题默认顺序；幕级 chapter 默认并行（全剧 hub → 各幕 → 终幕 hub）。
  - 返回点用独立虚拟枢纽节点，避免重访场景重新注入原文；一次性边/已消费边语义；跳过边带条件。
...
## Session（2026-09-06 第五次）：KP 上下文注入全局事实卡与全局检定点

- 背景：Codex 对 r3 存档的完整评价仍为 NO-GO（剧情还原 2、规则与检定 2、状态一致 1）。根因之一是 KP 系统提示只注入“当前场景原文”，从未注入世界设定（facts 章节），场景漂移时连检定点也消失。
- 实现（`context-builder.js`）：
  1. 注入 `scenarioStructure.sections` 中 `kind === "facts"` 的 `ownText/bodyText` 作为“世界设定/事实卡（全局权威）”，总量 5000 字/节 1600 字截断。
...
## Session（2026-09-07）：场景/路线约束——地点白名单、邻接图与未登记地点守卫

- 背景：Codex 复测中 KP 发明“下层阀门舱/矿洞钥匙/旋梯/拱道”等原文没有的地点与捷径，遗迹五室环形结构被改写；scene 长期误落。用户要求把场景/路线约束做了再一起测试。
- 实现：
  1. 新增 `lib/core/scenario/route-index.js`：从 keyPoints/branches 的 scene、deepParse.plotEdges、location 实体收集地点白名单与别名；生成地点邻接图；识别玩家“去/前往/进入/返回 + 白名单地点”的转移意图；提取叙述中的地点候选词（房间/室/厅/舱/旋梯/拱道/通道/走廊等后缀），检测原文与白名单都没有的未登记地点。
  2. `context-builder.js`：注入「本剧本地点清单（白名单）」与「路线约束（当前场景 → 可前往地点）」；明确“没有列出的移动默认不可达，不得新增地点”。
  3. `chat-bridge.js`：
     - 场景同步优先玩家转移意图（目标地点是当前场景邻接点或当前场景为空时直接切换并写 SceneChanged 事件），叙述推断降为兜底。
     - 新增路线守卫：叙述出现未登记地点词时追加纠正并重写一次，记录 route-guard trace。
  4. 回归测试：`tests/unit/route-index.test.mjs`（白名单/邻接/转移意图/未登记地点检测）。
- 验证：全量测试 64/64；《对流》DB 地点 26 个，邻接图正确（遗迹入口→房间1/2/3；房间3→房间4/5）；r3 发明词“旋梯/下层阀门舱”被守卫识别；正常叙述“沿走廊进入能量分流室”不误报；系统提示已包含地点清单与路线约束。

---

## Session（2026-09-07）：r4 复测修复——外出结算实际事件约束、路线守卫降误报、条件句/问路、母语检定

- 背景：Codex r4（6f0b79e）规则与检定 3/5、剧情还原 3/5，但路线守卫 26 次误报导致玩家体验回退；新误触：读地图提到“外部控制室”就扣 6 SAN；问路/条件句被误判为移动意图；旧版“一层门厅”楼层兜底写入《对流》；`.ra语言（母语）` 解析缺右括号导致两轮空叙述；成长在“无记录”时仍允许。
- 修复：
  1. `settlements.js` set-4 外出结算 anyOf 改为“离开/走出/出去/前往/到达/站到/岩台/山脉侧翼/夜风”等实际移动/抵达动作，读图/计划仅提到外部控制室不再命中。
  2. `route-index.js` 地点候选提取改为**只取后缀本身**（房间/控制室/舱/旋梯/拱道/通道/走廊…），不再截前文，根除“装置竖直贯穿房间/们沿环形走廊”误报。
  3. `findTravelIntent` 跳过“如果/若是…”条件句与“请/让/叫/问…路线/怎么走/地图”问路请求。
  4. `scene-facts.js` 删除 `inferSceneFromText` 的旧版《墨渊》楼层硬编码兜底（一层门厅/宅邸外围/三层书房）。
...
## Session（2026-09-07）：修复本地《对流》《盲愚之眼》场次 id 与文件名不一致

- 现象：重启 DSH web 后，《对流》《盲愚之眼》场次无数据，删除场次报“场次不存在”。
- 根因：`~/.dsh/coc/games/verify-对流.json` 与 `verify-盲愚之眼.json` 内部 `id` 都是 `verify`，与文件名不一致；前端按 `flat.id` 加载/删除时落到不存在的 `games/verify.json`。
- 修复：
