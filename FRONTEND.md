...
解析页工具栏有**独立的剧本切换**下拉框：默认「跟随当前场次」，也可直接选择任意剧本资产查看/编辑其网络，不必切换场次。查看的资产与当前场次剧本一致时，`createNetModel` 才叠加场次运行时摘要（revealed/reached 等）；查看其他资产时只显示资产纯结构，且「剧本结构编辑」折叠区隐藏（它写入当前场次）。资产列表接口不再返回 `deepParse`/`keyPoints` 等大字段，网络数据一律走 `/coc-api/deep-parse?asset=`。
