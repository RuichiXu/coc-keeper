...
前端结构与稳定 DOM 入口见 [FRONTEND.md](FRONTEND.md)，真实操作路径见 [USER_GUIDE.md](USER_GUIDE.md)。以 `686a3fc` 的覆盖为基础，2026-09-06 加入内置手册验收。
...
[tests/ui-check.mjs](tests/ui-check.mjs) 当前完整运行应为 **32/32**：
...
| 内置手册 | 打开、目录切章与关键校对说明、Esc 关闭恢复焦点和工作区、重开后按钮关闭 |
| 浏览器错误 | `pageerror` 为 0 |

其中部分旧子按钮通过 DOM click 触发；面板坞恢复已在 `686a3fc` 改回真实鼠标点击。因此 32/32 仍不能代替完整的指针交互测试。冒烟也不创建最终场次、不验证真实 LLM 主持/导入质量、不覆盖大图的完整数据及流畅度。UI 改动导致选择器失效时可以更新测试，但不能缩减上述覆盖来换取通过。
