...
describe("Shared 聊天桥", () => {
  ...
  it("KP 叙述给出团检标记时，写入 kp 与 check 日志并移除标记", async () => {
    ...
    expect(result.pendingChecks).toEqual([{ skill: "聆听", difficulty: "regular" }]);
    const flat = JSON.parse(readFileSync(join(dataDir, "games", "g1.json"), "utf8"));
    ...
    expect(flat.pendingChecks).toEqual([{ skill: "聆听", difficulty: "regular", at: flat.pendingChecks[0].at, scene: "" }]);
    expect(flat.busy).toBeFalse();
  });
