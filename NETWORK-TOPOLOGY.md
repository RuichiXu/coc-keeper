...
> 状态：2026-09-05 的拓扑设计记录，已有部分实现。前端当前基线为合并后的 `main`（`228e32f`）；前端实现见 [FRONTEND.md](FRONTEND.md)，后续工作见 [PLAN.md](PLAN.md)。

## 当前前端与原设计的对应关系

前端已实现骨架总览和场景总览：左向右推进、主线金色、分支/汇聚、场景聚合及场景间关系、hover 高亮、节点/边详情。返回语义主要显示为 `↩` 角标，独立返回节点收敛，返回边默认隐藏且可在检查栏打开。虚拟节点与推断连接有独立样式，结局和被折叠检定可从索引访问。

以下保留问题提出时的分析、schema 设计与实施建议。它不是最新后端实现清单，也不是尚待全部实施的前端计划；特别是一次性边、条件可达与运行时状态语义，应先核对当前源码及测试。前端展示了某条关系，不代表后端自动完成相关状态迁移。修改网络图需保留上述范式并执行 [TESTING.md](TESTING.md) 的四种图组合探针。

---

## 1. 背景与问题

提出本设计时，deepParse 生成的剧情网络在视觉上接近单线：只有最终分支/结局附近出现分支，中段大量场景被串成一条链。
